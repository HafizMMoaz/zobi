...
to zobi/examples/ and loaded via the example loading system.

Usage:
    zobi export-example --dashboard-id 123 --name my_example
    zobi export-example --dashboard-slug my-dashboard --name my_example
"""
...
@click.option(
    "--output-dir",
    "-o",
    default="zobi/examples",
    help="Output directory (default: zobi/examples)",
)
@click.option(
...
def export_example(  # noqa: C901
    ...
    Creates a folder structure in zobi/examples/ that can be loaded
    ...
    \b
        ...
        zobi export-example -d 1 -n my_example

    \b
        # Export by slug, limit data to 1000 rows
        zobi export-example -s my-dashboard -n my_example --sample-rows 1000

    \b
        # Export metadata only (no data)
        zobi export-example -d 1 -n my_example --no-export-data
    """
    ...
    if dashboard.json_metadata:
        try:
            meta = zobi_json.loads(dashboard.json_metadata)
    ...
    click.echo("\nTo load this example, ensure the folder is in zobi/examples/")
