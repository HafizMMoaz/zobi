...
from zobi.db_engine_specs.lib import (
    ADVANCED_FEATURES,
    BASIC_FEATURES,
    DATABASE_DETAILS,
    diagnose,
...
user_prefs = Table(
    "tmp_zobi_test_table_user_prefs",
    metadata_obj,
    Column("pref_id", Integer, primary_key=True),
    Column(
        "user_id",
        Integer,
        ForeignKey("tmp_zobi_test_table_user.user_id"),
        nullable=False,
    ),
...
def test_db_engine_spec(  # noqa: C901
    ...
    for spec in load_engine_specs():
        ...
        if supported:
            if spec.__module__.startswith("zobi.db_engine_specs"):
                console.print(
        ...
        console.print(
            ...
            "database can still be used with Zobi, but some functionality may be "
            "limited."
        )
...
def test_sqlalchemy_dialect(
    ...
    """
    Test the SQLAlchemy dialect, making sure it supports everything Zobi needs.
    """
