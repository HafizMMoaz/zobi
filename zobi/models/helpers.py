...
from zobi.advanced_data_type.types import AdvancedDataTypeResponse
from zobi.common.db_query_status import QueryStatus
from zobi.common.utils import dataframe_utils
from zobi.common.utils.time_range_utils import (
    get_since_until_from_query_object,
    get_since_until_from_time_range,
)
from zobi.constants import CacheRegion, EMPTY_STRING, NULL_STRING, TimeGrain
from zobi.db_engine_specs.base import TimestampExpression
...
def validate_adhoc_subquery(
    ...
    :raise ZobiSecurityException if sql contains sub-queries or
    ...
    if parsed_statement.has_subquery():
        if not is_feature_enabled("ALLOW_ADHOC_SUBQUERY"):
            raise ZobiSecurityException(
                ZobiError(
                    error_type=ZobiErrorType.ADHOC_SUBQUERY_NOT_ALLOWED_ERROR,
...
class ExploreMixin:  # pylint: disable=too-many-public-methods
    ...
    def query(self, query_obj: QueryObjectDict) -> QueryResult:
        ...
        try:
            ...
            # Re-raise ZobiErrorException (includes OAuth2RedirectError)
            # to bubble up to API layer
            if isinstance(ex, (ZobiErrorException, ZobiErrorsException)):
                raise
            df = pd.DataFrame()
    ...
    def validate_expression(
        ...
        from zobi.sql_validators.base import SQLValidationAnnotation
