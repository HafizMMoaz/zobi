...
from zobi_core.common.models import Database as CoreDatabase

from zobi import db, db_engine_specs, is_feature_enabled
from zobi.commands.database.exceptions import DatabaseInvalidError
from zobi.constants import LRU_CACHE_MAX_SIZE, PASSWORD_MASK
from zobi.databases.utils import make_url_safe
from zobi.db_engine_specs.base import MetricType, TimeGrain
...
if TYPE_CHECKING:
    from zobi_core.queries.types import AsyncQueryHandle, QueryOptions, QueryResult
...
class Database(CoreDatabase, AuditMixinNullable, ImportExportMixin):  # pylint: disable=too-many-public-methods
    ...
    def function_names(self) -> list[str]:
        try:
            ...
            # more info in: https://github.com/HafizMMoaz/zobi/issues/9678
            logger.error(
    ...
    def is_oauth2_enabled(self) -> bool:
        ...
        admins to create custom OAuth2 clients from the Zobi UI, and assign them to
        specific databases.
        """
        try:
    ...
    def execute(
        ...
        """
        from zobi.sql.execution import SQLExecutor
...
class Log(Model):  # pylint: disable=too-few-public-methods
    """ORM object used to log Zobi actions to the database"""
