...
from zobi.exceptions import ZobiError, ZobiErrorsException
...
class ReportScheduleSystemErrorsException(CommandException, ZobiErrorsException):
    errors: list[ZobiError] = []
    message = _("Report schedule system error")


class ReportScheduleClientErrorsException(CommandException, ZobiErrorsException):
    status = 400
    errors: list[ZobiError] = []
