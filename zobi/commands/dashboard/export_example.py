...
zobi/examples/ and loaded via the example loading system.
"""
...
def get_referenced_tables(sql: str, engine: str = "base") -> set[Table]:
    """Extract table references from SQL using Zobi's SQL parser.
...
def export_dataset_data(
    ...
    from zobi import db  # pylint: disable=import-outside-toplevel
...
def export_dashboard_yaml(
    ...
    from zobi.utils import (
        json as zobi_json,  # pylint: disable=import-outside-toplevel
    )
    ...
    if dashboard.json_metadata:
        try:
            json_metadata = zobi_json.loads(dashboard.json_metadata)
        except Exception:
