...
from zobi.commands.dashboard.exceptions import DashboardAccessDeniedError
...
from zobi.commands.explore.form_data.get import GetFormDataCommand
...
from zobi.extensions import async_query_manager, cache_manager
...
class Zobi(BaseZobiView):
    ...
    def _generate_xlsx(viz_obj: BaseViz) -> FlaskResponse:
        ...
        from zobi.utils.excel import apply_column_types, df_to_excel
    ...
    def explore_json_data(self, cache_key: str) -> FlaskResponse:
        ...
        try:
            ...
            # Let structured Zobi errors (e.g. OAuth2RedirectError) propagate
    ...
    def explore_json(  # noqa: C901
        ...
        if response_type in (
            ...
            if is_feature_enabled("GRANULAR_EXPORT_CONTROLS"):
                can_export = security_manager.can_access("can_export_data", "Zobi")
            else:
                can_export = security_manager.can_access("can_csv", "Zobi")
            if not can_export:
    ...
    def get_redirect_url() -> str:
        ...
        to the cache layer.
        """
        redirect_url = request.url.replace("/zobi/explore", "/explore")
        form_data_key = None
    ...
    def explore(  # noqa: C901
        ...
        if request.method == "GET":
            return redirect(Zobi.get_redirect_url())
        ...
        except (ZobiException, SQLAlchemyError):
    ...
    def dashboard_permalink(
        ...
        url = url_for(
            "Zobi.dashboard", dashboard_id_or_slug=dashboard_id, permalink_key=key
        )
