...
- ✅ **Access Control**: RBAC and RLS enforcement through Zobi's security manager
...
```python
MCP_JWT_ISSUER = "https://keycloak.yourcompany.com/realms/zobi"
MCP_JWT_AUDIENCE = "zobi-mcp"
MCP_JWKS_URI = "https://keycloak.yourcompany.com/realms/zobi/protocol/openid-connect/certs"
```
...
Zobi's existing RBAC is automatically enforced. Ensure roles are configured:

```python
# In Zobi UI: Security → List Roles
...
```python
# Future implementation - add to zobi_config.py
...
graph TB
    ...
    subgraph "Application Tier"
        ...
        MCP2[MCP Instance 2]
        MCP3[MCP Instance 3]
        Zobi[Zobi Web Server]
    end

    subgraph "Data Tier"
        DB[(PostgreSQL<br/>Zobi Metadata)]
...
python3.11 -m venv /opt/zobi/venv
source /opt/zobi/venv/bin/activate

# Install Zobi with MCP support
pip install zobi[fastmcp]
...
```python
# /opt/zobi/zobi_config.py

# Database connection
SQLALCHEMY_DATABASE_URI = "postgresql://user:pass@db-host:5432/zobi"
...
# Zobi webserver address (for screenshot generation)
ZOBI_WEBSERVER_ADDRESS = "https://zobi.yourcompany.com"
WEBDRIVER_BASEURL = "https://zobi.yourcompany.com/"
...
```bash
# /opt/zobi/.env
export ZOBI_CONFIG_PATH=/opt/zobi/zobi_config.py
...
```ini
# /etc/systemd/system/zobi-mcp.service
[Unit]
Description=Zobi MCP Service
...
User=zobi
Group=zobi
WorkingDirectory=/opt/zobi

Environment="ZOBI_CONFIG_PATH=/opt/zobi/zobi_config.py"
Environment="FLASK_APP=zobi"

ExecStart=/opt/zobi/venv/bin/zobi mcp run --port 5008
...
systemctl enable zobi-mcp

# Start service
systemctl start zobi-mcp

# Check status
systemctl status zobi-mcp

# View logs
journalctl -u zobi-mcp -f
```
...
```ini
# /etc/supervisor/conf.d/zobi-mcp.conf
[program:zobi-mcp]
command=/opt/zobi/venv/bin/zobi mcp run --port 5008
directory=/opt/zobi
user=zobi
autostart=true
autorestart=true
redirect_stderr=true
stdout_logfile=/var/log/zobi/mcp.log
stdout_logfile_maxbytes=50MB
stdout_logfile_backups=10
environment=ZOBI_CONFIG_PATH="/opt/zobi/zobi_config.py",FLASK_APP="zobi"
```
...
supervisorctl start zobi-mcp
supervisorctl status zobi-mcp
```
...
```dockerfile
FROM HafizMMoaz/zobi:latest

# Install MCP dependencies
RUN pip install zobi[fastmcp]

# Copy production config
COPY zobi_config.py /app/pythonpath/

# Expose MCP port
EXPOSE 5008

# Run MCP service
CMD ["zobi", "mcp", "run", "--port", "5008"]
```
...
```bash
# Build image
docker build -t zobi-mcp:latest .

# Run container
docker run -d \
  --name zobi-mcp \
  -p 5008:5008 \
  -v /opt/zobi/zobi_config.py:/app/pythonpath/zobi_config.py:ro \
  -e ZOBI_CONFIG_PATH=/app/pythonpath/zobi_config.py \
  zobi-mcp:latest

# View logs
docker logs -f zobi-mcp
```
  ...
  template:
    ...
    spec:
      ...
      - name: mcp
        ...
        command: ["zobi", "mcp", "run", "--port", "5008"]
        ports:
        - containerPort: 5008
          name: mcp
        env:
        - name: ZOBI_CONFIG_PATH
          value: /app/pythonpath/zobi_config.py
        - name: FLASK_APP
          value: zobi
        volumeMounts:
        - name: config
...
```bash
# Create ConfigMap from zobi_config.py
kubectl create configmap zobi-config \
  --from-file=zobi_config.py=/opt/zobi/zobi_config.py

# Apply deployment
kubectl apply -f zobi-mcp-deployment.yaml

# Check status
kubectl get pods -l app=zobi-mcp
kubectl logs -l app=zobi-mcp -f
```
  ...
  "logs": {
    "logs_collected": {
      "files": {
        "collect_list": [
          {
            "file_path": "/var/log/zobi/mcp.log",
            "log_group_name": "/aws/zobi/mcp",
...
- [ ] Zobi webserver address updated for production URL
...
1. **Immediate Rollback**:
   ```bash
   # Systemd
   systemctl stop zobi-mcp
   # Restore previous configuration
   cp /opt/zobi/zobi_config.py.backup /opt/zobi/zobi_config.py
   systemctl start zobi-mcp

   # Kubernetes
   kubectl rollout undo deployment/zobi-mcp
   ```
...
**Diagnosis**:
```bash
# Check if user exists in Zobi
zobi fab list-users | grep username
```

**Solution**:
- Create user in Zobi: `zobi fab create-user`
- Ensure JWT `sub` claim matches Zobi username
...
journalctl -u zobi-mcp | grep -i pool
...
- Test database connection: `zobi db upgrade`
...
```bash
# Check user's roles
zobi fab list-users | grep -A 5 username

# Check role permissions in Zobi UI
...
# Zobi webserver request timeout
ZOBI_WEBSERVER_TIMEOUT = 60
```

## References

- **Zobi Configuration**: https://zobi.dev/docs/configuration/configuring-zobi
- **Zobi Installation**: https://zobi.dev/docs/installation/installing-zobi-from-scratch
