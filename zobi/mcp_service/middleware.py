...
from zobi.mcp_service.constants import (
    DEFAULT_TOKEN_LIMIT,
    DEFAULT_WARN_THRESHOLD_PCT,
)
from zobi.mcp_service.utils.token_utils import (
...
def _sanitize_error_for_logging(error: Exception) -> str:
    ...
    error_str = re.sub(
        r"/[a-zA-Z0-9_\-/.]{1,200}/zobi/", "/[REDACTED]/zobi/", error_str
    )
...
def _is_user_error(error: Exception) -> bool:
    ...
    if isinstance(error, _USER_ERROR_TYPES):
        return True
    # ZobiException and CommandException have a .status attribute.
    # 4xx = user error, 5xx = system error.
    if isinstance(error, ZobiException):
...
class GlobalErrorHandlerMiddleware(Middleware):
    ...
    async def _handle_error(  # noqa: C901
        ...
        if isinstance(error, ToolError):
            ...
            # Zobi command: not found (404) or validation (422)
            raise ToolError(
                f"Invalid request for {tool_name}: {_sanitize_error_for_logging(error)}"
            ) from error
        elif isinstance(error, (ForbiddenError, ZobiSecurityException)):
            # Zobi access denied — agent tried a tool it can't use
            raise ToolError(
                f"Permission denied for {tool_name}: "
                f"{_sanitize_error_for_logging(error)}"
            ) from error
        elif isinstance(error, ZobiException):
            # Other Zobi errors — .status determines severity (already
...
class FieldPermissionsMiddleware(Middleware):
    ...
    def _filter_response(self, response: Any, object_type: str, user: Any) -> Any:
        ...
        Returns:
            Filtered response
        """
        from zobi.mcp_service.utils.permissions_utils import filter_sensitive_data
...
class ResponseSizeGuardMiddleware(Middleware):
    ...
    Configuration via MCP_RESPONSE_SIZE_CONFIG in zobi_config.py:
    ...
    def _rewrap_as_tool_result(payload: dict[str, Any], original: Any) -> Any:
        ...
        from zobi.utils.json import dumps as json_dumps
...
def create_response_size_guard_middleware() -> ResponseSizeGuardMiddleware | None:
    ...
    """
    try:
        from zobi.mcp_service.flask_singleton import get_flask_app
