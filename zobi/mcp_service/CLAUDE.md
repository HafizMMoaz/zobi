...
This guide helps LLM agents understand the Zobi MCP (Model Context Protocol) service architecture and development conventions.
...
The MCP service provides programmatic access to Zobi via the Model Context Protocol, allowing AI assistants to interact with dashboards, charts, datasets, databases, SQL Lab, and instance metadata.
...
The `@tool` and `@prompt` decorators are defined as stubs in the `core` package (`zobi_core.mcp.decorators`). At startup, `app.py` calls `initialize_core_mcp_dependencies()` which replaces these stubs with concrete implementations that register tools/prompts with the FastMCP instance. This avoids circular imports between `zobi_core` and `zobi`.
...
2. **Decorate with `@tool`** using the decorator from `zobi_core.mcp.decorators`
...
```python
# zobi/mcp_service/chart/tool/my_new_tool.py
...
**Then add to app.py**:
```python
# zobi/mcp_service/app.py (at the bottom, after initialize_core_mcp_dependencies())
from zobi.mcp_service.chart.tool import (  # noqa: F401, E402
    get_chart_info,
    list_charts,
...
```python
# zobi/mcp_service/chart/resources/my_new_resource.py
from zobi.mcp_service.app import mcp
from zobi.mcp_service.auth import mcp_auth_hook  # REQUIRED for resources

@mcp.resource("zobi://chart/my_resource")
...
The `@tool` decorator from `zobi_core.mcp.decorators` accepts:
...
**ToolAnnotations** (from `zobi_core.mcp.decorators`):
```python
annotations=ToolAnnotations(
...
**Use Zobi's DAO (Data Access Object) layer** instead of direct database queries:
...
```python
from zobi.commands.dataset.exceptions import DatasetInvalidError, DatasetCreateFailedError
...
```python
from zobi.mcp_service.chart.chart_utils import validate_chart_dataset
...
`ModelListCore` handles JSON string vs. native object parsing automatically via utilities in `zobi.mcp_service.utils.schema_utils`:
...
Default configuration is in `mcp_config.py`. Override in `zobi_config.py`:
...
async def test_my_tool_success(mcp_server):
    ...
    with patch("zobi.daos.chart.ChartDAO.find_by_id", return_value=mock_obj):
...
- **Patch at the DAO level**: `patch("zobi.daos.chart.ChartDAO.find_by_id", ...)`
...
**Solution**: Use `from zobi_core.mcp.decorators import tool, ToolAnnotations` (NOT `zobi_core.api.mcp`).
...
**Problem**: Bypasses Zobi's security and caching layers.
...
**Solution**: Use `from zobi_core.mcp.decorators import tool` for tools/prompts. Only import `from zobi.mcp_service.app import mcp` in resource files.

### 9. Missing event_logger Instrumentation
...
- [ ] Import: `from zobi_core.mcp.decorators import tool, ToolAnnotations`
...
- [ ] Added ASF license header
- [ ] Used `@prompt("prompt_name")` from `zobi_core.mcp.decorators`
...
- [ ] Added ASF license header
- [ ] Used `@mcp.resource("zobi://{path}")` decorator
...
- See `CLAUDE.md` in project root for general Zobi development guidelines
- Consult Zobi documentation: https://zobi.dev/docs/
