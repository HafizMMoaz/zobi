...
from zobi.mcp_service.dashboard.constants import (
    generate_id,
    GRID_COLUMN_COUNT,
...
from zobi.mcp_service.privacy import user_can_view_data_model_metadata
...
def _collect_tabs_groups(layout: Dict[str, Any]) -> list[list[str]]:
    ...
    Zobi dashboards can place TABS under either ROOT_ID or GRID_ID
...
def _ensure_layout_structure(
    ...
    if "ROOT_ID" in layout:
        ...
        # child of ROOT_ID.  Real Zobi dashboards with tabs place a
...
def add_chart_to_existing_dashboard(
    ...
    try:
        ...
        with event_logger.log_context(action="mcp.add_chart_to_dashboard.validation"):
            ...
            from zobi.mcp_service.auth import check_chart_data_access
        ...
        dashboard_info = DashboardInfo(
            ...
            url=f"{get_zobi_base_url()}/zobi/dashboard/{updated_dashboard.id}/",
