...
DEFAULT_SUPPORT_CONTACT = (
    "your Zobi administrator or the Zobi community "
    "(https://github.com/HafizMMoaz/zobi/issues)"
)
...
def _collect_environment() -> dict[str, str]:
    ...
    env: dict[str, str] = {
        ...
        "zobi_version": "unknown",
        "service": "Zobi MCP Service",
    }

    try:
        version_metadata = get_version_metadata()
        env["zobi_version"] = _safe_str(
            version_metadata.get("version_string", "unknown")
        )
    except Exception:  # noqa: BLE001
        logger.warning("bug_report: unable to read Zobi version", exc_info=True)
...
def _format_report(
    ...
    lines: list[str] = [
        ...
        f"- **Zobi version:** {environment['zobi_version']}",
    ...
    lines += [
        ...
        "---",
        (
            "_This report was generated by the Zobi MCP service. "
...
async def generate_bug_report(
    ...
    ``MCP_BUG_REPORT_CONTACT`` setting in ``zobi_config.py`` so each
    deployment can point users at the right channel. The default points
    at the user's Zobi administrator and the Zobi issue
    tracker.
