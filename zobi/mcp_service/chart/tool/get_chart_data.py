...
from zobi.exceptions import OAuth2Error, OAuth2RedirectError, ZobiException
...
from zobi.mcp_service.utils.cache_utils import get_cache_status_from_result
...
# Maps Zobi viz_type strings to canonical categories so we can
...
async def get_chart_data(  # noqa: C901
    ...
    try:
        ...
        # HafizMMoaz/zobi#39206 for the analogous database eager-load fix.
            ...
            if query_context_json is None and not using_unsaved_state:
                ...
                if viz_type.startswith("deck_"):
                    await ctx.warning(
                        ...
                        "re-saving the chart in Zobi." % (chart.id, viz_type)
                    )
                    return ChartError(
                        error=(
                            ...
                            f"Please open this chart in Zobi and re-save "
                ...
                if all(
                    ...
                    return ChartError(
                        error=(
                            ...
                            f"Please open this chart in Zobi and "
        ...
        except (CommandException, ZobiException, ValueError) as data_error:
            await ctx.error(
...
async def _query_from_form_data(
    ...
    except (CommandException, ZobiException, ValueError) as e:
...
def _export_data_as_csv(
    ...
    from zobi.mcp_service.chart.schemas import ChartData
