...
from zobi.mcp_service.chart.compile import (
    _compile_chart,
    CompileResult,
    validate_and_compile,
)
from zobi.mcp_service.chart.preview_utils import SUPPORTED_FORM_DATA_PREVIEW_FORMATS
...
from zobi.mcp_service.utils import sanitize_for_llm_context
from zobi.mcp_service.utils.oauth2_utils import (
...
async def generate_chart(  # noqa: C901
    ...
    """Create a chart preview in Zobi, optionally saving it permanently.
    ...
    VALIDATION:
    - 5-layer pipeline: Schema, business logic, dataset, Zobi compatibility, runtime
    ...
    try:
        ...
        with event_logger.log_context(action="mcp.generate_chart.validation"):
            from zobi.mcp_service.chart.validation import ValidationPipeline
        ...
        # Map the simplified config to Zobi's form_data format
        ...
        if request.save_chart:
            ...
            if not dataset:
                ...
                from zobi.mcp_service.common.error_schemas import (
            ...
            explore_url = f"{get_zobi_base_url()}/explore/?slice_id={chart.id}"
            ...
            try:
                with event_logger.log_context(
                    ...
                    from zobi.commands.explore.form_data.parameters import (
                        CommandParameters,
                    )
                    from zobi.mcp_service.commands.create_form_data import (
            ...
            from zobi.mcp_service.chart.chart_utils import generate_explore_link
                ...
                with event_logger.log_context(action="mcp.generate_chart.preview"):
                    for format_type in request.preview_formats:
                        ...
                        if chart_id:
                            ...
                            from zobi.mcp_service.chart.tool.get_chart_preview import (  # noqa: E501
                            ...
                            if format_type in SUPPORTED_FORM_DATA_PREVIEW_FORMATS:
                                ...
                                from zobi.mcp_service.chart.preview_utils import (
        ...
        if request.save_chart and chart:
            ...
            from zobi import db
            from zobi.daos.chart import ChartDAO
            from zobi.mcp_service.chart.schemas import serialize_chart_object
        ...
        result = {
            ...
            "api_endpoints": {
                "data": f"{get_zobi_base_url()}/api/v1/chart/{chart.id}/data/"
                if chart
                else None,
                "export": f"{get_zobi_base_url()}/api/v1/chart/{chart.id}/export/"
                if chart
                else None,
            }
            if chart
        ...
        from zobi.mcp_service.utils.error_builder import ChartErrorBuilder
