...
def _sql_from_saved_query_context(
    ...
    except (ZobiException, CommandException, TypeError, ValueError):
        return None
...
def _resolve_datasource_name(
    ...
    try:
        ...
        from zobi.daos.exceptions import (
            DatasourceNotFound,
...
async def get_chart_sql(
    ...
    feature in the Zobi UI. Useful for understanding, debugging, or
    auditing the SQL generated by a chart's configuration.

    Note: The returned SQL is RLS-scoped to the caller and contains
    table/schema names. Same disclosure surface as the Zobi UI
    ...
    except ZobiException as e:
        logger.exception("Zobi error in get_chart_sql")
        return ChartError(
            error=f"Zobi error: {e}",
            error_type="ZobiError",
        )
...
async def _handle_chart_sql_request(
    ...
    with event_logger.log_context(action="mcp.get_chart_sql.build_query"):
        ...
        except (ZobiException, CommandException, ValueError) as e:
