...
# Zobi's query builder, so genuine SQL/config errors are very unlikely.
...
def _compile_chart(
    ...
    from zobi.mcp_service.chart.chart_utils import adhoc_filters_to_query_filters
    from zobi.mcp_service.chart.preview_utils import _build_query_columns
...
def _adhoc_filter_column_valid(
    ...
    reference a saved metric because Zobi resolves metric names there.
    """
    if clause == "HAVING":
...
def _classify_as_database_error(exc: BaseException, dataset_id: int) -> bool:
    ...
    the same classification the Zobi UI uses.
    """
