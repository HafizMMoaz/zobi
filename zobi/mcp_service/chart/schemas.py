...
from zobi.daos.base import ColumnOperator, ColumnOperatorEnum
from zobi.mcp_service.common.cache_schemas import (
    CacheStatus,
    CreatedByMeMixin,
...
from zobi.mcp_service.common.error_schemas import ChartGenerationError, MCPBaseError
from zobi.mcp_service.constants import DEFAULT_PAGE_SIZE, MAX_PAGE_SIZE
from zobi.mcp_service.privacy import filter_user_directory_fields
from zobi.mcp_service.system.schemas import (
    PaginationInfo,
    TagInfo,
)
from zobi.mcp_service.utils import (
...
def serialize_chart_object(chart: ChartLike | None) -> ChartInfo | None:
    ...
    from zobi.utils import json as utils_json

    chart_id = getattr(chart, "id", None)
    chart_url = None
    if chart_id:
        chart_url = f"{get_zobi_base_url()}/explore/?slice_id={chart_id}"
...
class PieChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        None,
        description=(
            "Zobi color scheme ID (e.g. 'zobiColors', 'lyftColors', "
            "'googleCategory10c', 'd3Category10'). Defaults to 'zobiColors'."
        ),
        max_length=100,
    )
...
class MixedTimeseriesChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        ...
        description=(
            "Zobi color scheme ID (e.g. 'zobiColors', 'lyftColors'). "
            "When omitted, Zobi's default scheme is used."
        ),
        max_length=100,
    )
    currency_format: CurrencyFormat | None = Field(
...
class BigNumberChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        None,
        description=(
            "Zobi color scheme ID for the trendline (e.g. 'zobiColors'). "
            "When omitted, Zobi's default scheme is used."
        ),
        max_length=100,
    )
...
class TableChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        None,
        description=(
            "Zobi color scheme ID applied to conditional/cell formatting "
            "(e.g. 'zobiColors')."
        ),
        max_length=100,
    )
...
class XYChartConfig(UnknownFieldCheckMixin):
    ...
    color_scheme: str | None = Field(
        ...
        description=(
            "Zobi color scheme ID (e.g. 'zobiColors', 'lyftColors', "
            "'googleCategory10c', 'd3Category10'). When omitted, Zobi's "
        ...
        if self.group_by:
            for i, col in enumerate(self.group_by):
                if self.x is not None and col.name == self.x.name:
                    ...
                    # to prevent Zobi "duplicate label" errors, so
...
class ListChartsRequest(OwnedByMeMixin, CreatedByMeMixin, MetadataCacheControl):
    ...
    def parse_filters(cls, v: Any) -> List[ChartFilter]:
        ...
        """
        from zobi.mcp_service.utils.schema_utils import parse_json_or_model_list
    ...
    def parse_select_columns(cls, v: Any) -> List[str]:
        ...
        See: https://github.com/anthropics/claude-code/issues/5504
        """
        from zobi.mcp_service.utils.schema_utils import parse_json_or_list
...
class GenerateChartRequest(QueryCacheControl):
    ...
    save_chart: bool = Field(default=False, description="Save permanently in Zobi")
...
class GenerateExploreLinkRequest(FormDataCacheControl):
    ...
    config: ChartConfig | None = Field(
        ...
        description=(
            ...
            "explore URL that opens the dataset in Zobi without a "
            "preconfigured chart."
        ),
    )
...
class GenerateChartResponse(BaseModel):
    ...
    explore_url: str | None = Field(None, description="Edit chart in Zobi")
...
class ChartPreview(BaseModel):
    ...
    explore_url: str = Field(description="URL to open chart in Zobi for editing")
