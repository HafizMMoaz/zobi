...
def is_column_truly_temporal(column_name: str, dataset_id: int | str | None) -> bool:
    ...
    This is important because Zobi may mark columns as is_dttm=True based on
...
def map_config_to_form_data(
    ...
    """Map chart config to Zobi form_data."""
...
def map_table_config(config: TableChartConfig) -> Dict[str, Any]:
    ...
    if config.query_mode == "raw":
        ...
        if raw_columns and not aggregated_metrics:
            ...
            # Include both "all_columns" (Zobi table viz) and "columns"
...
def create_metric_object(col: ColumnRef) -> Dict[str, Any] | str:
    ...
    Zobi's query engine resolves via its metrics_by_name lookup.
...
def add_orientation_config(form_data: Dict[str, Any], config: XYChartConfig) -> None:
    ...
    When orientation is None (the default), Zobi uses its own default
    (vertical bars).
    """
...
def _resolve_default_x_axis(
    ...
    from zobi.mcp_service.chart.schemas import ColumnRef
...
def map_xy_config(
    ...
    # "Duplicate column/metric labels" errors in Zobi.
    if config.group_by:
...
def map_pie_config(config: PieChartConfig) -> Dict[str, Any]:
    """Map pie chart config to Zobi form_data."""
    ...
    form_data: Dict[str, Any] = {
        ...
        "metric": metric,
        "color_scheme": config.color_scheme or "zobiColors",
...
def map_big_number_config(config: BigNumberChartConfig) -> Dict[str, Any]:
    """Map big number chart config to Zobi form_data."""
    ...
    if viz_type == "big_number":
        ...
        # (unlike XY charts which use x_axis). This is how Zobi's
...
def map_handlebars_config(config: HandlebarsChartConfig) -> Dict[str, Any]:
    """Map handlebars chart config to Zobi form_data."""
    form_data: Dict[str, Any] = {
        "viz_type": "handlebars",
...
def map_pivot_table_config(config: PivotTableChartConfig) -> Dict[str, Any]:
    """Map pivot table config to Zobi form_data."""
    if not config.rows:
...
def map_mixed_timeseries_config(
    ...
    """Map mixed timeseries chart config to Zobi form_data."""
    if not config.y:
...
def map_filter_operator(op: str) -> str:
    """Map filter operator to Zobi format."""
    operator_map = {
        "=": "==",
        ">": ">",
        "<": "<",
...
def _resolve_viz_type(config: Any) -> str:
    """Resolve the Zobi viz_type from a chart config object."""
...
def get_table_chart_type_label(viz_type: str | None) -> str | None:
    """Return a user-facing label for table-family Zobi viz types."""
