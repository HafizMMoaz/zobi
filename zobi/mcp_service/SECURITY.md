...
The MCP service implements multiple layers of security to ensure safe programmatic access to Zobi functionality. This document covers authentication, authorization, session management, audit logging, and compliance considerations.
...
4. All subsequent Zobi operations use this user's permissions
...
6. Look up Zobi user from database
...
```json
{
  ...
  "aud": "zobi-mcp",
  "exp": 1735689600,
  "client_id": "zobi-automation",
  "scopes": ["zobi:read", "zobi:chart:create"]
}
```

**Service Account Best Practices**:
- Create dedicated Zobi users for service accounts
...
**Example Zobi Service Account Setup**:

```bash
# Create service account user in Zobi
zobi fab create-user \
  --role Alpha \
...
# (Use Zobi UI or FAB CLI to configure role permissions)
```
...
The MCP service fully integrates with Zobi's Flask-AppBuilder role-based access control:
...
RLS is configured per dataset in Zobi UI:
...
- Same security model as Zobi web UI
...
Different MCP tools require different Zobi permissions:
...
```python
# If user lacks permission, Zobi raises exception
try:
    result = DashboardDAO.find_by_id(dashboard_id)
except ZobiSecurityException as e:
...
```python
# zobi_config.py
MCP_REQUIRED_SCOPES = ["zobi:read"]  # Minimum scopes required
```

**Scope Hierarchy**:
- `zobi:read`: List and view resources
- `zobi:chart:create`: Create new charts
- `zobi:chart:update`: Update existing charts
- `zobi:chart:delete`: Delete charts
- `zobi:dashboard:create`: Create dashboards
- `zobi:sql:execute`: Execute SQL queries
- `zobi:admin`: Full administrative access
...
@mcp.tool
@mcp_auth_hook
@require_scopes(["zobi:chart:create"])
...
- Flask-AppBuilder and Zobi expect CSRF configuration
...
logging.getLogger("zobi.mcp_service").addHandler(handler)
```
...
logging.getLogger("zobi.mcp_service").addHandler(syslog_handler)
```
...
- No personal data stored in MCP service (only in Zobi DB)
...
2. Disable user account in Zobi
...
3. Check for SQL injection attempts (should be prevented by Zobi)
...
- **Zobi Security Documentation**: https://zobi.dev/docs/security
