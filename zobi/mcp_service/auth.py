...
- RBAC permission checking aligned with Zobi's REST API permissions
...
def check_tool_permission(func: Callable[..., Any]) -> bool:
    ...
    and uses Zobi's security_manager to verify access.

    Controlled by the ``MCP_RBAC_ENABLED`` config flag (default True).
    Set to False in zobi_config.py to disable RBAC checking.
...
def _resolve_user_from_jwt_context(app: Any) -> User | None:
    ...
    from zobi.mcp_service.mcp_config import default_user_resolver
    ...
    if not user:
        ...
        raise ValueError(
            f"JWT authenticated user '{username}' not found in Zobi database. "
    ...
    if username := current_app.config.get("MCP_DEV_USERNAME"):
        ...
        if not user:
            raise ValueError(
                ...
                f"Please create admin user with: zobi fab create-admin"
            )
        return user
...
def has_dataset_access(dataset: "SqlaTable") -> bool:
    ...
    has permission to access the given dataset using Zobi's security manager.
    ...
    try:
        ...
        if hasattr(g, "user") and g.user:
            # Use Zobi's security manager to check dataset access
