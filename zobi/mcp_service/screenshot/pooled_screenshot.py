...
from zobi.mcp_service.screenshot.webdriver_pool import get_webdriver_pool
from zobi.mcp_service.utils.retry_utils import retry_screenshot_operation
from zobi.utils.screenshots import BaseScreenshot, WindowSize
...
class PooledChartScreenshot(PooledBaseScreenshot):
    ...
    def __init__(
        ...
        from zobi.utils.webdriver import ChartStandaloneMode
    ...
    def _take_screenshot(self, driver: Any, user: User) -> bytes | None:
        """Take screenshot of chart with standard Zobi chart handling"""
        try:
...
class PooledDashboardScreenshot(PooledBaseScreenshot):
    ...
    def __init__(
        ...
        from zobi.utils.urls import modify_url_query
        from zobi.utils.webdriver import DashboardStandaloneMode
    ...
    def _take_screenshot(self, driver: Any, user: User) -> bytes | None:
        """Take screenshot of dashboard with standard Zobi dashboard handling"""
        try:
            # Wait for page to load
            selenium_headstart = current_app.config["SCREENSHOT_SELENIUM_HEADSTART"]
            time.sleep(selenium_headstart)
