...
def configure_webdriver_pool(app_config: Dict[str, Any]) -> None:
    """
    Configure WebDriver pool settings in Zobi app config.

    This function adds WebDriver pool configuration to the Zobi app config
    if it doesn't already exist, using sensible defaults.

    Args:
        app_config: The Zobi application configuration dictionary
    """
...
def get_pool_stats_endpoint() -> Any:
    ...
    def pool_stats() -> Any:
        try:
            ...
            from zobi.mcp_service.screenshot.webdriver_pool import (
                get_webdriver_pool,
            )
