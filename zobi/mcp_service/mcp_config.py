...
# Note: MCP_DEV_USERNAME MUST be configured in zobi_config.py
...
# deployment should override this in zobi_config.py to point users at the
...
# user's Zobi administrator and the Zobi issue tracker.
...
# clients never see them. Use this when a Zobi-provided tool conflicts with
...
#
#   from zobi.mcp_service.storage import get_mcp_store
...
def create_default_mcp_auth_factory(app: Flask) -> Optional[Any]:
    ...
    try:
        ...
        if debug_errors:
            ...
            from zobi.mcp_service.jwt_verifier import DetailedJWTVerifier
...
def default_user_resolver(app: Any, access_token: Any) -> str | None:
    ...
    if isinstance(claims, dict) and claims:
        ...
        # (OIDC `sub` is often a stable opaque ID, not a Zobi username)
        username = (
...
def get_mcp_config(app_config: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    defaults = {
        "ZOBI_WEBSERVER_ADDRESS": ZOBI_WEBSERVER_ADDRESS,
...
def get_mcp_config_with_overrides(
    ...
    This version lets users add custom MCP config keys in zobi_config.py
