...
The Zobi MCP (Model Context Protocol) service provides programmatic access to Zobi functionality through a standardized protocol that enables AI assistants and automation tools to interact with dashboards, charts, datasets, and SQL Lab.

The MCP service runs as a **separate process** from the Zobi web server, using its own Flask application instance and HTTP server while sharing the same database and configuration with the main Zobi application.
...
- The MCP service runs as an independent process from the Zobi web server
...
from zobi.mcp_service.mcp_config import get_mcp_config
...
**Zobi's Existing RLS (Row-Level Security)**:
...
- The MCP service respects all RLS rules automatically through Zobi's query execution layer

**No Schema-Based Isolation**:
- The current implementation does NOT use separate database schemas per tenant
- All Zobi metadata (dashboards, charts, datasets) exists in the same database schema
- Database-level isolation is achieved through Zobi's permission system rather than physical schema separation
...
- Zobi's security manager then enforces permissions based on this user's roles
...
- All list operations (dashboards, charts, datasets) use Zobi's access filters:
...
```json
{
  ...
  "scopes": ["zobi:read", "zobi:chart:create"],
  "exp": 1672531200
}
```
...
The MCP service uses SQLAlchemy's connection pooling with configuration inherited from Zobi:
...
- Write operations (create chart, etc.) use Zobi's command pattern which handles commits internally
...
**Shared Database with Zobi**:
- MCP service and Zobi web server share the same database
...
**Example Calculation**:
- Zobi web: 8 workers × 10 connections = 80
...
sequenceDiagram
    ...
    participant DAO as Zobi DAO Layer<br/>(ChartDAO, DashboardDAO)
...
graph TD
    ...
    DB[(Zobi Database<br/>shared connection pool)]
...
graph TD
    ...
    DB[(Zobi Database<br/>single schema, filtered by permissions)]
...
| Security | Depends on Zobi RBAC | Stronger isolation |
...
- **Zobi Security Model**: https://zobi.dev/docs/security
