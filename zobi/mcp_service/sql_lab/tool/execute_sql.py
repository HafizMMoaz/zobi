...
async def execute_sql(  # noqa: C901: sequential guard clauses, complexity is inherent
    ...
    try:
        ...
        with event_logger.log_context(action="mcp.execute_sql.db_validation"):
            ...
            if not database:
                ...
                return ExecuteSqlResponse(
                    ...
                    error_type=ZobiErrorType.DATABASE_NOT_FOUND_ERROR.value,
                )
            ...
            if not security_manager.can_access_database(database):
                ...
                return ExecuteSqlResponse(
                    ...
                    error_type=ZobiErrorType.DATABASE_SECURITY_ACCESS_ERROR.value,
                )
        ...
        with event_logger.log_context(action="mcp.execute_sql.ddl_check"):
            try:
                ...
                if script.has_destructive():
                    ...
                    return ExecuteSqlResponse(
                        ...
                        error=(
                            ...
                            "are not allowed through MCP. Use the Zobi SQL "
                            "Lab UI for administrative database operations."
                        ),
                        error_type=ZobiErrorType.DML_NOT_ALLOWED_ERROR.value,
                    )
                ...
                return ExecuteSqlResponse(
                    ...
                    error_type=ZobiErrorType.INVALID_SQL_ERROR.value,
                )
        ...
        if request.template_params and not is_feature_enabled(
            ...
            response.template_warning = (
                ...
                "disabled on this Zobi instance "
        ...
        return ExecuteSqlResponse(
            ...
            error_type=ZobiErrorType.OAUTH2_REDIRECT.value,
        )
    except OAuth2Error:
        await ctx.error(
        ...
        return ExecuteSqlResponse(
            ...
            error_type=ZobiErrorType.OAUTH2_REDIRECT_ERROR.value,
        )
...
def _data_to_statement_data(data: Any) -> StatementData:
    ...
    """
    from zobi.utils import json as json_utils
