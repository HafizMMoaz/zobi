...
def pivot_df(  # pylint: disable=too-many-locals, too-many-arguments, too-many-statements, too-many-branches  # noqa: C901
        ...
        if not apply_metrics_on_rows:
            for col in df.columns:
                ...
                if pd.api.types.is_numeric_dtype(df[col]):
                    df[col].replace("ZOBI_PANDAS_NAN", np.nan, inplace=True)
                else:
                    df[col].replace("ZOBI_PANDAS_NAN", "nan", inplace=True)
        else:
            ...
            df.replace("ZOBI_PANDAS_NAN", np.nan, inplace=True)
    ...
    df.replace("ZOBI_PANDAS_NAN", np.nan, inplace=True)
    df.rename(
        index={"ZOBI_PANDAS_NAN": np.nan},
        columns={"ZOBI_PANDAS_NAN": np.nan},
        inplace=True,
    )
