...
from zobi.exceptions import (
    OAuth2RedirectError,
    ZobiDisallowedSQLFunctionException,
    ZobiDisallowedSQLTableException,
    ZobiDMLNotAllowedException,
    ZobiErrorException,
    ZobiErrorsException,
    ZobiInvalidCTASException,
    ZobiInvalidCVASException,
    ZobiResultsBackendNotConfigureException,
)
from zobi.extensions import celery_app, event_logger
from zobi.models.sql_lab import Query
from zobi.result_set import ZobiResultSet
from zobi.sql.parse import BaseSQLStatement, CTASMethod, SQLScript, Table
from zobi.sqllab.limiting_factor import LimitingFactor
from zobi.sqllab.utils import write_ipc_buffer
...
def execute_query(  # pylint: disable=too-many-statements, too-many-locals  # noqa: C901
    ...
    try:
        ...
        raise ZobiErrorException(
            ZobiError(
                ...
                error_type=ZobiErrorType.SQLLAB_TIMEOUT_ERROR,
    ...
    return ZobiResultSet(data, cursor_description, db_engine_spec)
...
def execute_sql_statements(  # noqa: C901
    ...
    if database.allow_run_async and not results_backend:
        raise ZobiResultsBackendNotConfigureException()
    ...
    if disallowed_functions and parsed_script.check_functions_present(
        ...
        raise ZobiDisallowedSQLFunctionException(disallowed_functions)
    ...
    if disallowed_tables and parsed_script.check_tables_present(disallowed_tables):
        ...
        raise ZobiDisallowedSQLTableException(found_tables or disallowed_tables)

    if parsed_script.has_mutation() and not database.allow_dml:
        raise ZobiDMLNotAllowedException()
                    ...
                    if serialized_payload_size > max_bytes:
                        ...
                        raise ZobiErrorException(
                            ZobiError(
                                ...
                                error_type=ZobiErrorType.RESULT_TOO_LARGE_ERROR,
                                level=ErrorLevel.ERROR,
                            )
                        )
