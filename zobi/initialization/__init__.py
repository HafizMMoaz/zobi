...
from zobi.extensions.context import extension_context
...
class ZobiAppInitializer:  # pylint: disable=too-many-public-methods
    def __init__(self, app: ZobiApp) -> None:
        super().__init__()
    ...
    @deprecated(details="use self.zobi_app instead of self.flask_app")  # type: ignore
    @property
    def flask_app(self) -> ZobiApp:
        return self.zobi_app
    ...
    def init_views(self) -> None:
        ...
        from zobi.available_domains.api import AvailableDomainsRestApi
        from zobi.cachekeys.api import CacheRestApi
        from zobi.charts.api import ChartRestApi
        from zobi.charts.data.api import ChartDataRestApi
        from zobi.css_templates.api import CssTemplateRestApi
        from zobi.dashboards.api import DashboardRestApi
        from zobi.dashboards.filter_state.api import DashboardFilterStateRestApi
        from zobi.dashboards.permalink.api import DashboardPermalinkRestApi
        from zobi.databases.api import DatabaseRestApi
        from zobi.datasets.api import DatasetRestApi
        from zobi.datasets.columns.api import DatasetColumnsRestApi
        from zobi.datasets.metrics.api import DatasetMetricRestApi
        from zobi.datasource.api import DatasourceRestApi
        from zobi.embedded.api import EmbeddedDashboardRestApi
        from zobi.embedded.view import EmbeddedView
        from zobi.explore.api import ExploreRestApi
        ...
        from zobi.reports.logs.api import ReportExecutionLogRestApi
        from zobi.row_level_security.api import RLSRestApi
        from zobi.security.api import (
            RoleRestAPI,
            SecurityRestApi,
            UserRegistrationsRestAPI,
        )
        from zobi.sqllab.api import SqlLabRestApi
        from zobi.sqllab.permalink.api import SqlLabPermalinkRestApi
        from zobi.tags.api import TagRestApi
        from zobi.themes.api import ThemeRestApi
        from zobi.views.alerts import AlertView, ReportView
        from zobi.views.all_entities import TaggedObjectsModelView
        from zobi.views.annotations import AnnotationLayerView
        from zobi.views.api import Api
        from zobi.views.chart.views import SliceModelView
        from zobi.views.core import Zobi
        from zobi.views.css_templates import CssTemplateModelView
        from zobi.views.dashboard.views import (
            Dashboard,
            DashboardModelView,
        )
        from zobi.views.database.views import DatabaseView
        from zobi.views.datasource.views import DatasetEditor, Datasource
        from zobi.views.dynamic_plugins import DynamicPluginsView
        ...
        from zobi.views.roles import RolesListView
        from zobi.views.sql_lab.views import (
            SavedQueryView,
            TableSchemaView,
            TabStateView,
        )
        from zobi.views.sqla import (
        ...
        from zobi.views.themes import ThemeModelView
        from zobi.views.user_info import UserInfoView
        ...
        from zobi.views.health import health_blueprint

        self.zobi_app.register_blueprint(health_blueprint)
        ...
        if feature_flag_manager.is_feature_enabled("SEMANTIC_LAYERS"):
            from zobi.semantic_layers.api import (
        ...
        appbuilder.add_view(
            ...
            menu_cond=lambda: bool(
                appbuilder.app.config.get("ZOBI_SECURITY_VIEW_MENU", True)
            ),
        )
        ...
        appbuilder.add_view_no_menu(Zobi)
    ...
    def init_core_dependencies(self) -> None:
        ...
        from zobi.core.mcp.core_mcp_injection import (
    ...
    def init_extensions(self) -> None:
        from zobi.extensions.utils import (
            eager_import,
            get_extensions,
    ...
    def check_secret_key(self) -> None:
        def log_default_secret_key_warning() -> None:
            ...
            logger.warning(
                "A Default SECRET_KEY was detected, please use zobi_config.py "
                "to override it.\n"
                "Use a strong complex alphanumeric string and use a tool to help"
                " you generate \n"
                "a sufficiently random sequence, ex: openssl rand -base64 42 \n"
                "For more info, see: https://zobi.dev/docs/"
                "configuration/configuring-zobi#specifying-a-secret_key"
            )
    ...
    def configure_session(self) -> None:
        if self.config["SESSION_SERVER_SIDE"]:
            Session(self.zobi_app)

    def register_request_handlers(self) -> None:
        """Register app-level request handlers"""
        from flask import request, Response

        @self.zobi_app.after_request
        def apply_http_headers(response: Response) -> Response:
            """Applies the configuration's http headers to all responses"""
            # HTTP_HEADERS is deprecated, this provides backwards compatibility
            response.headers.extend(
                {
                    **self.zobi_app.config["OVERRIDE_HTTP_HEADERS"],
                    **self.zobi_app.config["HTTP_HEADERS"],
                }
            )

            for k, v in self.zobi_app.config["DEFAULT_HTTP_HEADERS"].items():
        ...
        @self.zobi_app.context_processor
        def get_common_bootstrap_data() -> dict[str, Any]:
            # Import here to avoid circular imports
            from zobi.utils import json
            from zobi.views.base import common_bootstrap_payload
    ...
    def check_and_warn_database_connection(self) -> None:
        ...
        try:
            with self.zobi_app.app_context():
    ...
    def configure_auth_provider(self) -> None:
        machine_auth_provider_factory.init_app(self.zobi_app)

    def configure_ssh_manager(self) -> None:
        ssh_manager_factory.init_app(self.zobi_app)

    def configure_stats_manager(self) -> None:
        stats_logger_manager.init_app(self.zobi_app)

    def setup_event_logger(self) -> None:
        _event_logger["event_logger"] = get_event_logger_from_cfg_value(
            self.zobi_app.config.get("EVENT_LOGGER", DBEventLogger())
        )
    ...
    def configure_cache(self) -> None:
        cache_manager.init_app(self.zobi_app)
        results_backend_manager.init_app(self.zobi_app)

    def configure_feature_flags(self) -> None:
        feature_flag_manager.init_app(self.zobi_app)
    ...
    def configure_fab(self) -> None:
        ...
        custom_sm = self.config["CUSTOM_SECURITY_MANAGER"] or ZobiSecurityManager
        if not issubclass(custom_sm, ZobiSecurityManager):
            raise Exception(  # pylint: disable=broad-exception-raised
                """Your CUSTOM_SECURITY_MANAGER must now extend ZobiSecurityManager,
                 not FAB's security manager.
                 See [4565] in UPDATING.md"""
            )

        appbuilder.indexview = ZobiIndexView
        appbuilder.security_manager_class = custom_sm
        appbuilder.init_app(self.zobi_app, db.session)
    ...
    def configure_url_map_converters(self) -> None:
        ...
        from zobi.utils.url_map_converters import (
            ObjectTypeConverter,
            RegexConverter,
        )

        self.zobi_app.url_map.converters["regex"] = RegexConverter
        self.zobi_app.url_map.converters["object_type"] = ObjectTypeConverter
    ...
    def configure_middlewares(self) -> None:  # noqa: C901
        if self.config["ENABLE_CORS"]:
            ...
            CORS(self.zobi_app, **self.config["CORS_OPTIONS"])

        if self.config["ENABLE_PROXY_FIX"]:
            self.zobi_app.wsgi_app = ProxyFix(
                self.zobi_app.wsgi_app, **self.config["PROXY_FIX_CONFIG"]
            )
        ...
        if self.config["ENABLE_CHUNK_ENCODING"]:
            ...
            self.zobi_app.wsgi_app = ChunkedEncodingFix(self.zobi_app.wsgi_app)

        if self.config["UPLOAD_FOLDER"]:
            with contextlib.suppress(OSError):
                os.makedirs(self.config["UPLOAD_FOLDER"])
        for middleware in self.config["ADDITIONAL_MIDDLEWARE"]:
            self.zobi_app.wsgi_app = middleware(self.zobi_app.wsgi_app)

        # Flask-Compress
        Compress(self.zobi_app)

        # Talisman
        talisman_enabled = self.config["TALISMAN_ENABLED"]
        talisman_config = (
            self.config["TALISMAN_DEV_CONFIG"]
            if self.zobi_app.debug or self.config["DEBUG"]
            else self.config["TALISMAN_CONFIG"]
        )
        csp_warning = self.config["CONTENT_SECURITY_POLICY_WARNING"]

        if talisman_enabled:
            talisman.init_app(self.zobi_app, **talisman_config)

        show_csp_warning = False
        if (
            csp_warning
            and not self.zobi_app.debug
            and (
    ...
    def configure_logging(self) -> None:
        self.config["LOGGING_CONFIGURATOR"].configure_logging(
            self.config, self.zobi_app.debug
        )

    def configure_db_encrypt(self) -> None:
        encrypted_field_factory.init_app(self.zobi_app)

    def setup_db(self) -> None:
        db.init_app(self.zobi_app)

        with self.zobi_app.app_context():
            pessimistic_connection_handling(db.engine)

        migrate.init_app(self.zobi_app, db=db, directory=APP_DIR + "/migrations")

    def configure_wtf(self) -> None:
        if self.config["WTF_CSRF_ENABLED"]:
            csrf.init_app(self.zobi_app)
    ...
    def configure_async_queries(self) -> None:
        if feature_flag_manager.is_feature_enabled("GLOBAL_ASYNC_QUERIES"):
            async_query_manager_factory.init_app(self.zobi_app)

    def configure_task_manager(self) -> None:
        """Initialize the TaskManager for GTF realtime notifications."""
        if feature_flag_manager.is_feature_enabled("GLOBAL_TASK_FRAMEWORK"):
            from zobi.tasks.manager import TaskManager

            TaskManager.init_app(self.zobi_app)
    ...
    def register_blueprints(self) -> None:
        ...
        for bp in self.config["BLUEPRINTS"]:
            try:
                ...
                self.zobi_app.register_blueprint(bp)
            except Exception:  # pylint: disable=broad-except
                logger.exception("blueprint registration failed")

    def setup_bundle_manifest(self) -> None:
        manifest_processor.init_app(self.zobi_app)

    def enable_profiling(self) -> None:
        if self.config["PROFILING"]:
            profiling.init_app(self.zobi_app)


class ZobiIndexView(IndexView):
    @expose("/")
    def index(self) -> FlaskResponse:
        return redirect(url_for("Zobi.welcome"))
