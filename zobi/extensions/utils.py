...
from zobi.extensions.types import BundleFile, LoadedExtension
from zobi.utils import json
from zobi.utils.core import check_is_safe_zip
...
class InMemoryFinder(importlib.abc.MetaPathFinder):
    def __init__(self, file_dict: dict[str, bytes], source_base_path: str) -> None:
        ...
        # Detect if this is a virtual path (zobz://) or filesystem path
        is_virtual_path = source_base_path.startswith("zobz://")
        ...
        for path, content in file_dict.items():
            ...
            if is_virtual_path:
                ...
                # e.g., zobz://extension-id/backend/src/tasks.py
...
def install_in_memory_importer(
    ...
    :param source_base_path: Base path for traceback filenames. For LOCAL_EXTENSIONS,
        ...
        For EXTENSIONS_PATH (.zobz files), this should be a zobz:// URL
        (e.g., "zobz://extension-id").
    """
...
def get_extensions() -> dict[str, LoadedExtension]:
    ...
    # Load extensions from discovery path (.zobz files)
    if extensions_path := current_app.config.get("EXTENSIONS_PATH"):
        from zobi.extensions.discovery import discover_and_load_extensions
