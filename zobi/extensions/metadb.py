"""
A SQLAlchemy dialect for querying across Zobi databases.

The dialect ``zobi://`` allows users to query any table in any database that has been
configured in Zobi, eg:
...
from zobi import db, feature_flag_manager, security_manager
from zobi.sql.parse import Table


# pylint: disable=abstract-method
class ZobiAPSWDialect(APSWDialect):
    """
    A SQLAlchemy dialect for an internal Zobi engine.

    This dialect allows query to be executed across different Zobi
    databases. For example, to read data from the `birth_names` table in the
    `examples` databases:

        >>> engine = create_engine('zobi://')
        >>> conn = engine.connect()
        >>> results = conn.execute('SELECT * FROM "examples.birth_names"')

    Queries can also join data across different Zobi databases.

    The dialect is built in top of the Shillelagh library, leveraging SQLite to
    create virtual tables on-the-fly proxying Zobi tables. The
    `ZobiShillelaghAdapter` adapter is responsible for returning data when a
    Zobi table is accessed.
    """
...
def check_dml(method: F) -> F:
    ...
    @wraps(method)
    def wrapper(self: ZobiShillelaghAdapter, *args: Any, **kwargs: Any) -> Any:
...
class ZobiShillelaghAdapter(Adapter):
    """
    A Shillelagh adapter for Zobi tables.
    ...
    def supports(
        ...
        **kwargs: Any,
        ...
        eg, `zobi.examples.birth_names`.

        When using the Zobi SQLAlchemy and DB engine spec the prefix is dropped, so
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if not feature_flag_manager.is_feature_enabled("ENABLE_ZOBI_META_DB"):
            raise ProgrammingError("Zobi meta database is disabled")
    ...
    def get_data(
        ...
        **kwargs: Any,
        ...
        """
        app_limit: int | None = current_app.config["ZOBI_META_DB_LIMIT"]
        if limit is None:
            limit = app_limit
