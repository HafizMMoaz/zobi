...
from zobi import db
from zobi.constants import LRU_CACHE_MAX_SIZE
...
def get_columns_description(
    ...
    try:
        with database.get_raw_connection(catalog=catalog, schema=schema) as conn:
            ...
            result_set = ZobiResultSet(result, cursor.description, db_engine_spec)
            return result_set.columns
    except Exception as ex:
        raise ZobiGenericDBErrorException(message=str(ex)) from ex
