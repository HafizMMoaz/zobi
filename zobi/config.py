"""The main config file for Zobi

All configuration in this file can be overridden by providing a zobi_config
in your PYTHONPATH as there is a ``from zobi_config import *``
...
from zobi.advanced_data_type.plugins.internet_address import internet_address
from zobi.advanced_data_type.plugins.internet_port import internet_port
from zobi.advanced_data_type.types import AdvancedDataType
from zobi.constants import CHANGE_ME_SECRET_KEY
...
from zobi.utils.encrypt import SQLAlchemyUtilsAdapter
...
# https://github.com/HafizMMoaz/zobi/blob/master/zobi/utils/log.py
...
ZOBI_SECURITY_VIEW_MENU = True

BASE_DIR = str(files("zobi"))
if "ZOBI_HOME" in os.environ:
    DATA_DIR = os.environ["ZOBI_HOME"]
else:
    DATA_DIR = os.path.expanduser("~/.zobi")

# ---------------------------------------------------------
# Zobi specific config
# ---------------------------------------------------------
VERSION_INFO_FILE = str(files("zobi") / "static/version_info.json")
PACKAGE_JSON_FILE = str(files("zobi") / "static/assets/package.json")
...
# actually running Zobi, we will have already installed,
...
# ZobiClient HTTP retry configuration
# Controls retry behavior for all HTTP requests made through ZobiClient
# This helps handle transient server errors (like 502 Bad Gateway) automatically
ZOBI_CLIENT_RETRY_ATTEMPTS = 3  # Maximum number of retry attempts
ZOBI_CLIENT_RETRY_DELAY = 1000  # Initial retry delay in milliseconds
ZOBI_CLIENT_RETRY_BACKOFF_MULTIPLIER = 2  # Exponential backoff multiplier
ZOBI_CLIENT_RETRY_MAX_DELAY = 10000  # Maximum retry delay cap in milliseconds
ZOBI_CLIENT_RETRY_JITTER_MAX = 1000  # Maximum random jitter in milliseconds
# HTTP status codes that should trigger retries (502, 503, 504 gateway errors)
ZOBI_CLIENT_RETRY_STATUS_CODES = [502, 503, 504]
...
# (gunicorn, nginx, ...) timeout setting to be <= to this setting
ZOBI_WEBSERVER_TIMEOUT = int(timedelta(minutes=1).total_seconds())
...
ZOBI_DASHBOARD_PERIODICAL_REFRESH_LIMIT = 0
ZOBI_DASHBOARD_PERIODICAL_REFRESH_WARNING_MESSAGE = None

ZOBI_DASHBOARD_POSITION_DATA_LIMIT = 65535
...
# Your App secret key. Make sure you override it on zobi_config.py
# or use `ZOBI_SECRET_KEY` environment variable.
# Use a strong complex alphanumeric string and use a tool to help you generate
# a sufficiently random sequence, ex: openssl rand -base64 42"
SECRET_KEY = os.environ.get("ZOBI_SECRET_KEY") or CHANGE_ME_SECRET_KEY

# The SQLAlchemy connection string.
SQLALCHEMY_DATABASE_URI = (
    f"""sqlite:///{os.path.join(DATA_DIR, "zobi.db")}?check_same_thread=false"""
)
...
WTF_CSRF_EXEMPT_LIST = [
    "zobi.charts.data.api.data",
    "zobi.dashboards.api.cache_dashboard_screenshot",
    "zobi.views.core.explore_json",
    "zobi.views.core.log",
    "zobi.views.datasource.views.samples",
...
# Zobi allows server-side python stacktraces to be surfaced to the
...
# feature is on by default to make Zobi secure by default, but you should
...
RATELIMIT_ENABLED = os.environ.get("ZOBI_ENV") == "production"
...
# You can also specify a relative URL e.g. '/zobi/welcome' or '/dashboards/list'
...
# Enables SWAGGER UI for zobi openapi spec
...
BABEL_DEFAULT_FOLDER = "zobi/translations"
...
# Feature flags
# ---------------------------------------------------
# Feature flags control optional functionality in Zobi. They can be set in
# zobi_config.py to override the defaults below.
#
...
DEFAULT_FEATURE_FLAGS: dict[str, bool] = {
    ...
    # @lifecycle: testing
    # @docs: https://zobi.dev/docs/configuration/alerts-reports
    "ALERT_REPORTS": False,
    ...
    # Allows users to add a zobi:// DB that can query across databases.
    # Experimental with potential security/performance risks.
    # See ZOBI_META_DB_LIMIT.
    # @lifecycle: testing
    # @docs: https://zobi.dev/user-docs/databases/supported/zobi-meta-database
    "ENABLE_ZOBI_META_DB": False,
    ...
    # @lifecycle: testing
    # @docs: https://zobi.dev/docs/contributing/misc#async-chart-queries
    ...
    # @lifecycle: testing
    # @docs: https://zobi.dev/docs/configuration/setup-ssh-tunneling
    "SSH_TUNNELING": False,
    ...
    # Security note: When enabled, ensure Zobi's IAM role has restricted
    ...
    # Enable caching per user key for Zobi cache
    # @lifecycle: stable
    # @category: runtime_config
    ...
    # @lifecycle: stable
    # @category: runtime_config
    # @docs: https://zobi.dev/docs/using-zobi/creating-your-first-dashboard
    ...
    # @lifecycle: stable
    # @category: runtime_config
    # @docs: https://zobi.dev/docs/configuration/networking-settings#hiding-the-logout-button-in-embedded-contexts
    "DISABLE_EMBEDDED_ZOBI_LOGOUT": False,
    ...
    # Enable embedded Zobi functionality
    # @lifecycle: stable
    # @category: runtime_config
    "EMBEDDED_ZOBI": False,
    ...
    # @lifecycle: stable
    # @category: runtime_config
    # @docs: https://zobi.dev/docs/configuration/cache
    "THUMBNAILS": False,
    # =================================================================
...
# Feature flags may also be set via 'ZOBI_FEATURE_' prefixed environment vars.
DEFAULT_FEATURE_FLAGS.update(
    {
        k[len("ZOBI_FEATURE_") :]: utils.parse_boolean_string(v)
        for k, v in os.environ.items()
        if re.search(r"^ZOBI_FEATURE_\w+", k)
    }
)
...
# -----------------------------------------------------------------------------
# Zobi supports custom theming through Ant Design's theme structure.
#
# Theme Hierarchy:
...
THEME_DEFAULT: Theme = {
    "token": {
        ...
        # Default colors
        "colorPrimary": "#DA8F29",  # NOTE: previous lighter primary color was #20a7c9 # noqa: E501
        "colorLink": "#DA8F29",
        "colorError": "#e04355",
        "colorWarning": "#fcc700",
        "colorSuccess": "#5ac189",
...
# `zobi.tasks.types.ExecutorType` for a full list of executor options.
...
# Default cache for Zobi objects
CACHE_CONFIG: CacheConfig = {"CACHE_TYPE": "NullCache"}

# Cache for datasource metadata and query results
DATA_CACHE_CONFIG: CacheConfig = {"CACHE_TYPE": "NullCache"}

# Cache for dashboard filter state. `CACHE_TYPE` defaults to `ZobiMetastoreCache`
# that stores the values in the key-value table in the Zobi metastore, as it's
# required for Zobi to operate correctly, but can be replaced by any
# `Flask-Caching` backend.
FILTER_STATE_CACHE_CONFIG: CacheConfig = {
    "CACHE_TYPE": "ZobiMetastoreCache",
...
# Cache for explore form data state. `CACHE_TYPE` defaults to `ZobiMetastoreCache`
# that stores the values in the key-value table in the Zobi metastore, as it's
# required for Zobi to operate correctly, but can be replaced by any
# `Flask-Caching` backend.
...
# `pip install .[cors]` or `pip install zobi[cors]`, depending
ENABLE_CORS = True
...
# next available socket. PR #5039 added domain sharding for Zobi,
# and this feature can be enabled by configuration only (by default Zobi
# doesn't allow cross-domain request). This feature is deprecated, annd will be removed
# in the next major version of Zobi, as enabling HTTP2 will serve the same goals.
ZOBI_WEBSERVER_DOMAINS = None  # deprecated
...
# time grains in zobi/db_engine_specs/base.py).
# For example: to disable 1 second time grain:
# TIME_GRAIN_DENYLIST = ['PT1S']
TIME_GRAIN_DENYLIST: list[str] = []

# Additional time grains to be supported using similar definitions as in
# zobi/db_engine_specs/base.py.
...
# See zobi/common/query_context_processor.get_aggregated_join_column
#
...
DEFAULT_MODULE_DS_MAP = OrderedDict(
    [
        ("zobi.connectors.sqla.models", ["SqlaTable"]),
    ]
)
...
FILENAME = os.path.join(DATA_DIR, "zobi.log")
ROLLOVER = "midnight"
INTERVAL = 1
BACKUP_COUNT = 30
...
# The limit for the Zobi Meta DB when the feature flag ENABLE_ZOBI_META_DB is on
ZOBI_META_DB_LIMIT: int | None = 1000
...
# Additional static HTTP headers to be served by your Zobi server. Note
...
# from CSV files. For example, 'companyname-zobi'
...
# Roles that are controlled by the API / Zobi and should not be changed
# by humans.
ROBOT_PERMISSION_ROLES = ["Public", "Gamma", "Alpha", "Admin", "sql_lab"]

CONFIG_PATH_ENV_VAR = "ZOBI_CONFIG_PATH"
...
SMTP_USER = "zobi"
SMTP_PORT = 25
SMTP_PASSWORD = "zobi"  # noqa: S105
SMTP_MAIL_FROM = "zobi@zobi.com"
...
# See: zobi.db_engine_specs.base.BaseEngineSpec.validate_database_uri
# Example:
...
MACHINE_AUTH_PROVIDER_CLASS = "zobi.utils.machine_auth.MachineAuthProvider"
...
EMAIL_REPORTS_CTA = "Explore in Zobi"

# Slack API token for the zobi reports, either string or callable
...
# Send user to a link where they can read more about Zobi
...
# tokens. See https://github.com/HafizMMoaz/zobi/issues/20300 for more information. The
...
# specified. If you're running multiple Zobi instances you might want to have a
...
# https://zobi.dev/docs/configuration/networking-settings/#changing-flask-talisman-csp
...
# Some sqlalchemy connection strings can open Zobi to security risks.
...
# (zobi.connectors.sqla.models.SqlaTable), we call this hook
...
GLOBAL_ASYNC_QUERY_MANAGER_CLASS = (
    "zobi.async_events.async_query_manager.AsyncQueryManager"
)
...
# Audience for the Zobi guest token used in embedded mode.
...
# in zobi/custom_database_errors.py.
# Transform raw database errors into user-friendly messages with optional documentation
try:
    from zobi.custom_database_errors import CUSTOM_DATABASE_ERRORS
except ImportError:
...
if CONFIG_PATH_ENV_VAR in os.environ:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("zobi_config", cfg_path)
        ...
        raise
elif importlib.util.find_spec("zobi_config"):
    try:
        # pylint: disable=import-error,wildcard-import,unused-wildcard-import
        import zobi_config
        from zobi_config import *  # noqa: F403, F401

        click.secho(
            f"Loaded your LOCAL configuration at [{zobi_config.__file__}]",
            fg="cyan",
        )
    except Exception:
        logger.exception("Found but failed to import local zobi_config")
        raise

# Final environment variable processing - must be at the very end
# to override any config file assignments
ENV_VAR_KEYS = {
    "ZOBI__SQLALCHEMY_DATABASE_URI",
    "ZOBI__SQLALCHEMY_EXAMPLES_URI",
}

for env_var in ENV_VAR_KEYS:
    if env_var in os.environ:
        config_var = env_var.replace("ZOBI__", "")
