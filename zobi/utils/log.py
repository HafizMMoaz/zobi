...
from zobi.utils.core import get_user_id, LoggerLevel, to_int
...
class AbstractEventLogger(ABC):
    ...
    def log_with_context(  # pylint: disable=too-many-locals,too-many-arguments
        ...
        **payload_override: dict[str, Any] | None,
        ...
        from zobi import db
        from zobi.views.core import get_form_data
...
def get_event_logger_from_cfg_value(cfg_value: Any) -> AbstractEventLogger:
    ...
    if inspect.isclass(cfg_value):
        logging.warning(
            textwrap.dedent(
                """
                In zobi private config, EVENT_LOGGER has been assigned a class
                ...
                zobi.utils.log.AbstractEventLogger.
                """
            )
        )
    ...
    if not isinstance(result, AbstractEventLogger):
        raise TypeError(
            ...
            "of zobi.utils.log.AbstractEventLogger."
        )
...
class DBEventLogger(AbstractEventLogger):
    """Event logger that commits logs to Zobi DB"""
