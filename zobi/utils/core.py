...
class ReservedUrlParameters(StrEnum):
    """
    Reserved URL parameters that are used internally by Zobi. These will not be
...
def get_user_id() -> int | None:
    ...
    identifier is encoded as a `str` whereas in Zobi all identifiers are encoded as
    an `int`.
...
def is_test() -> bool:
    return parse_boolean_string(os.environ.get("ZOBI_TESTENV", "false"))
...
def check_is_safe_zip(zip_file: ZipFile) -> None:
    """
    Checks whether a ZIP file is safe, raises ZobiException if not.
    ...
    for zip_file_element in zip_file.infolist():
        if zip_file_element.file_size > app.config["ZIPPED_FILE_MAX_SIZE"]:
            raise ZobiException("Found file with size above allowed threshold")
    ...
    if compress_ratio > app.config["ZIP_FILE_MAX_COMPRESS_RATIO"]:
        raise ZobiException("Zip compress ratio above allowed threshold")
...
def get_query_source_from_request() -> QuerySource | None:
    if not request or not request.referrer:
        return None
    if "/zobi/dashboard/" in request.referrer:
