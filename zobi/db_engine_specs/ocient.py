...
with contextlib.suppress(ImportError, RuntimeError):  # pyocient may not be installed
    # Ensure pyocient inherits Zobi's logging level
    import geojson
    import pyocient
    from flask import current_app as app
    from shapely import wkt

    zobi_log_level = app.config["LOG_LEVEL"]
    pyocient.logger.setLevel(zobi_log_level)
...
def _linestring_to_geo_json(
    ...
    compatible with the Zobi visualization toolkit (powered
    by Deck.gl).
...
# that cannot be processed natively by Zobi.
#
# Zobi serializes temporal objects using a custom serializer
# defined in zobi/utils/core.py (#json_int_dttm_ser(...)). Other
...
def _find_columns_to_sanitize(cursor: Any) -> list[PlacedSanitizeFunc]:
    """
    Cleans the column value for consumption by Zobi.
...
class OcientEngineSpec(BaseEngineSpec):
    ...
    cte_alias = "cte__"
    # Store mapping of zobi Query id -> Ocient ID
    ...
    custom_errors: dict[Pattern[str], tuple[str, ZobiErrorType, dict[str, Any]]] = {
        ...
        CONNECTION_INVALID_PASSWORD_REGEX: (
            ...
            ZobiErrorType.CONNECTION_INVALID_PASSWORD_ERROR,
            {},
        ),
        ...
        CONNECTION_INVALID_PORT_ERROR: (
            ...
            ZobiErrorType.CONNECTION_INVALID_PORT_ERROR,
            {},
        ),
        ...
        TABLE_DOES_NOT_EXIST_REGEX: (
            ...
            ZobiErrorType.TABLE_DOES_NOT_EXIST_ERROR,
            {},
        ),
    ...
    def fetch_data(
        ...
        try:
            ...
            with OcientEngineSpec.query_id_mapping_lock:
                del OcientEngineSpec.query_id_mapping[cursor.zobi_query_id]
            raise
    ...
    def get_cancel_query_id(cls, cursor: Any, query: Query) -> Optional[str]:
        ...
        # If None is returned, Zobi will not call cancel_query
        return "DUMMY_VALUE"
