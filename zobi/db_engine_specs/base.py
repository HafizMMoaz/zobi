...
from zobi import db
from zobi.constants import QUERY_CANCEL_KEY, TimeGrain as TimeGrainConstants
from zobi.databases.utils import get_table_metadata, make_url_safe
...
from zobi.utils.json import redact_sensitive, reveal_sensitive
from zobi.utils.network import is_hostname_valid, is_port_open
...
if TYPE_CHECKING:
    ...
    from zobi.databases.schemas import TableMetadataResponse
...
class BaseEngineSpec:  # pylint: disable=too-many-public-methods
    ...
    def start_oauth2_dance(cls, database: Database) -> None:
        ...
        can authorize Zobi to access the database. Once the user has authorized, the
        ...
        # The state is passed to the OAuth2 provider, and sent back to Zobi after
        # the user authorizes the access. The redirect endpoint in Zobi can then
    ...
    def get_allows_alias_in_select(
        ...
        In Dremio this atribute is version-dependent, so Zobi needs to inspect the
    ...
    def get_dbapi_mapped_exception(cls, exception: Exception) -> Exception:
        """
        Get a zobi custom DBAPI exception from the driver specific exception.
        ...
        :return: Zobi custom DBAPI exception
        """
    ...
    ) -> dict[Any, tuple[str, ZobiErrorType, dict[str, Any]]]:
    ...
    def adjust_engine_params(  # pylint: disable=unused-argument
        ...
        Zobi knows in which schema a given query is running in order to enforce
    ...
    def get_column_types(
        ...
        :return: SQLAlchemy and generic Zobi column types
        """
        if not column_type:
            return None
        ...
        try:
            ...
            errors.append(
                ZobiError(
                    ...
                    error_type=ZobiErrorType.CONNECTION_INVALID_PORT_ERROR,
                    level=ErrorLevel.ERROR,
                    extra={"invalid": ["port"]},
                ),
            )
        if not (isinstance(port, int) and 0 <= port < 2**16):
            ...
            errors.append(
                ZobiError(
                    ...
                    error_type=ZobiErrorType.CONNECTION_PORT_CLOSED_ERROR,
                    level=ErrorLevel.ERROR,
                    extra={"invalid": ["port"]},
                ),
            )
