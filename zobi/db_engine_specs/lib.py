...
NICE_TO_HAVE_FEATURES = {
    ...
    "dbapi_exception_mapping": "Maps driver exceptions to Zobi exceptions",
    "custom_errors": "Parses error messages and returns Zobi errors",
...
def diagnose(spec: type[BaseEngineSpec]) -> dict[str, Any]:
    ...
    from zobi.sql_validators.presto_db import PrestoDBSQLValidator
...
def generate_feature_tables() -> str:
    ...
    info = {k: v for k, v in info.items() if v["module"].startswith("zobi")}
...
def generate_yaml_docs(output_dir: str | None = None) -> dict[str, dict[str, Any]]:
    ...
    for spec in sorted(load_engine_specs(), key=get_name):
        # Skip non-zobi modules (3rd party)
        if not spec.__module__.startswith("zobi"):
            continue
