...
from zobi.utils.oauth2 import get_oauth2_access_token

if TYPE_CHECKING:
    from zobi.models.core import Database
    from zobi.sql.parse import Table
    from zobi.zobi_typing import OAuth2ClientConfig, OAuth2State
...
class GSheetsEngineSpec(ShillelaghEngineSpec):
    ...
    metadata = {
        ...
        "install_instructions": 'pip install "zobi[gsheets]"',
    ...
    def get_oauth2_authorization_uri(
        ...
        from zobi.utils.oauth2 import encode_oauth2_state, generate_code_challenge
    ...
    def _do_post(
        ...
        **kwargs: Any,
        ...
        if "error" in payload:
            raise ZobiException(payload["error"]["message"])
    ...
    def df_to_sql(  # pylint: disable=too-many-locals
        ...
        if spreadsheet_url and "if_exists" in to_sql_kwargs:
            if to_sql_kwargs["if_exists"] == "append":
                ...
                # never going to work
                raise ZobiException("Append operation not currently supported")
            if to_sql_kwargs["if_exists"] == "fail":
                raise ZobiException("Table already exists")
