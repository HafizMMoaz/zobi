...
class AWSIAMAuthMixin:
...

    {
        "aws_iam": {
            "enabled": true,
            "role_arn": "arn:aws:iam::222222222222:role/ZobiDatabaseAccess",
            "external_id": "zobi-prod-12345",  # optional
            "region": "us-east-1",
    ...
    aws_iam_custom_errors: dict[str, tuple[ZobiErrorType, str]] = {
        "AccessDenied": (
            ZobiErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            "Unable to assume IAM role. Verify the role ARN and trust policy "
            "allow access from Zobi's IAM role.",
        ),
        "InvalidIdentityToken": (
            ZobiErrorType.CONNECTION_ACCESS_DENIED_ERROR,
            "Invalid IAM credentials. Ensure Zobi has a valid IAM role "
            "with permissions to assume the target role.",
        ),
        "MalformedPolicyDocument": (
            ZobiErrorType.CONNECTION_MISSING_PARAMETERS_ERROR,
    ...
    def get_iam_credentials(
        ...
        :raises ZobiSecurityException: If role assumption fails
        """
        ...
        try:
            ...
            assume_role_kwargs: dict[str, Any] = {
                "RoleArn": role_arn,
                "RoleSessionName": "zobi-iam-session",
    ...
    def generate_rds_auth_token(
        ...
        :raises ZobiSecurityException: If token generation fails
        """
        try:
            import boto3
            from botocore.exceptions import ClientError
    ...
    def generate_redshift_credentials(
        ...
        :raises ZobiSecurityException: If credential generation fails
        """
        try:
            import boto3
            from botocore.exceptions import ClientError
    ...
    def _apply_iam_authentication(
        ...
        :raises ZobiSecurityException: If any step fails
        """
        ...
        if not feature_flag_manager.is_feature_enabled("AWS_DATABASE_IAM_AUTH"):
            raise ZobiSecurityException(
                ZobiError(
                    ...
                    "Zobi configuration to enable this feature.",
                    error_type=ZobiErrorType.GENERIC_DB_ENGINE_ERROR,
                    level=ErrorLevel.ERROR,
                )
            )
