...
Usage:
    python zobi/db_engine_specs/lint_metadata.py [--json] [--strict]
...
import argparse
import json  # noqa: TID251 - standalone script, don't depend on zobi.utils
import sys
...
def check_pypi_package(package_name: str, timeout: float = 5.0) -> bool:
    ...
    try:
        req = urllib.request.Request(  # noqa: S310
            url, headers={"User-Agent": "zobi-lint/1.0"}
        )
...
def generate_markdown_report(reports: list[MetadataReport]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "```python",
            "from zobi.db_engine_specs.base import (",  # noqa: E501
            ...
            "```",
            "",
            "See `zobi/db_engine_specs/README.md` for full documentation.",
            "",
        ]
    )
