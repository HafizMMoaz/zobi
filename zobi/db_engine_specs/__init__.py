...
def load_engine_specs() -> list[type[BaseEngineSpec]]:
    ...
    for ep in entry_points(group="zobi.db_engine_specs"):
        try:
            engine_spec = ep.load()
        except Exception:  # pylint: disable=broad-except
            logger.warning("Unable to load Zobi DB engine spec: %s", ep.name)
            continue
...
def get_engine_spec(backend: str, driver: Optional[str] = None) -> type[BaseEngineSpec]:
    ...
    drivers to work with Zobi even if they are not listed in the DB engine spec
    drivers.
    """  # noqa: E501
