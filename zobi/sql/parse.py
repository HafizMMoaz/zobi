...
from zobi.sql.dialects import DB2, Dremio, Firebolt, OpenSearch, Pinot, Vertica
...
# engines supported by Zobi this is `sqlglot.exp.Expression`, but there is a special
...
class BaseSQLStatement(Generic[InternalRepresentation]):
    ...
    The `engine` parameters comes from the `engine` attribute in a Zobi DB engine
    spec.
    """
...
class SQLStatement(BaseSQLStatement[exp.Expression]):
    ...
    def _parse(cls, script: str, engine: str) -> list[exp.Expression]:
        ...
        try:
            ...
            raise ZobiParseError(script, engine, **kwargs) from ex
        except sqlglot.errors.SqlglotError as ex:
            raise ZobiParseError(
                script,
                engine,
...
class KustoKQLStatement(BaseSQLStatement[str]):
    ...
    def _extract_tables_from_statement(
        ...
        logger.warning(
            ...
            "roles will not be enforced by Zobi in the database."
        )
        return set()
...
class SQLScript:
    ...
    # adds a lot of complexity to Zobi, so we should avoid adding new engines to
    # this data structure.
    special_engines = {
...
def process_jinja_sql(
    ...
    :raises ZobiSecurityException: If SQLGlot is unable to parse the SQL statement
    :raises jinja2.exceptions.TemplateError: If the Jinjafied SQL could not be rendered
    """

    from zobi.jinja_context import (  # pylint: disable=import-outside-toplevel
