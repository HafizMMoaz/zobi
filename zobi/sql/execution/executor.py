...
methods defined in zobi_core.api.models.Database.
...
zobi_core.api.models for the public API contract.
"""
...
def execute_sql_with_cursor(
...
) -> list[tuple[str, ZobiResultSet | None, float, int]]:
    """
    ...
    results: list[tuple[str, ZobiResultSet | None, float, int]] = []
...
class SQLExecutor:
    ...
    See zobi_core.api.models.Database for the full public API documentation.
    """
    ...
    def execute(
        ...
        See zobi_core.api.models.Database.execute() for full documentation.
        """
        from zobi_core.queries.types import (
    ...
    def execute_async(
        ...
        See zobi_core.api.models.Database.execute_async() for full documentation.
        """
        from zobi_core.queries.types import (
            QueryOptions as QueryOptionsType,
            QueryResult as QueryResultType,
        ...
        if opts.dry_run:
            from zobi_core.queries.types import StatementResult
    ...
    def _check_security(self, script: SQLScript) -> None:
        ...
        :raises ZobiSecurityException: If security checks fail
        """
    ...
    def _store_in_cache(
        ...
        :param opts: Query options
        """
        from zobi_core.queries.types import QueryStatus
    ...
    def _submit_query_to_celery(
        ...
        from zobi.utils.core import get_username
    ...
    def _get_async_query_status(query_id: int) -> Any:
        ...
        from zobi_core.queries.types import QueryStatus as QueryStatusType
    ...
    def _get_async_query_result(query_id: int) -> Any:
        ...
        if query.results_key:
            ...
            from zobi import results_backend_manager
            ...
            if results_backend is not None:
                ...
                if blob:
                    try:
                        from zobi.utils.core import zlib_decompress
