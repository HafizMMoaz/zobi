...
from zobi.charts.schemas import ChartEntityResponseSchema
...
from zobi.commands.dashboard.delete import (
...
from zobi.commands.dashboard.export_example import ExportExampleCommand
from zobi.commands.dashboard.fave import AddFavoriteDashboardCommand
from zobi.commands.dashboard.importers.dispatcher import ImportDashboardsCommand
from zobi.commands.dashboard.permalink.create import CreateDashboardPermalinkCommand
from zobi.commands.dashboard.unfave import DelFavoriteDashboardCommand
from zobi.commands.dashboard.update import (
    UpdateDashboardChartCustomizationsCommand,
    UpdateDashboardColorsConfigCommand,
    UpdateDashboardCommand,
    UpdateDashboardNativeFiltersCommand,
)
from zobi.commands.database.exceptions import DatasetValidationError
from zobi.commands.exceptions import TagForbiddenError
from zobi.commands.importers.exceptions import NoValidFilesFoundError
from zobi.commands.importers.v1.utils import get_contents_from_bundle
...
from zobi.dashboards.filters import (
...
from zobi.dashboards.schemas import (
...
from zobi.exceptions import ScreenshotImageNotAvailableException
from zobi.extensions import event_logger, security_manager
from zobi.models.dashboard import Dashboard
from zobi.models.embedded_dashboard import EmbeddedDashboard
from zobi.security.guest_token import GuestUser
from zobi.tasks.thumbnails import (
    cache_dashboard_screenshot,
    cache_dashboard_thumbnail,
)
from zobi.tasks.utils import get_current_user
from zobi.utils import json
from zobi.utils.core import parse_boolean_string
from zobi.utils.file import get_filename
from zobi.utils.pdf import build_pdf_from_screenshots
from zobi.utils.screenshots import (
    DashboardScreenshot,
...
from zobi.views.custom_tags_api_mixin import CustomTagsOptimizationMixin
from zobi.views.error_handling import handle_api_exception
from zobi.views.filters import (
...
def with_dashboard(
    f: Callable[[BaseZobiModelRestApi, Dashboard], Response],
) -> Callable[[BaseZobiModelRestApi, str], Response]:
    """
    ...
    def wraps(self: BaseZobiModelRestApi, id_or_slug: str) -> Response:
        try:
...
class DashboardRestApi(CustomTagsOptimizationMixin, BaseZobiModelRestApi):
    ...
    @permission_name("get")
    @merge_response_func(
        BaseZobiModelRestApi.merge_order_columns, API_ORDER_COLUMNS_RIS_KEY
    )
    @merge_response_func(
        BaseZobiModelRestApi.merge_list_label_columns, API_LABEL_COLUMNS_RIS_KEY
    )
    @merge_response_func(
        BaseZobiModelRestApi.merge_description_columns,
        API_DESCRIPTION_COLUMNS_RIS_KEY,
    )
    @merge_response_func(
    ...
    def __repr__(self) -> str:
        ...
        return "Zobi.dashboards.api.DashboardRestApi@v{}{}".format(
    ...
    def export_as_example(self, pk: int) -> Response:
        ...
        get:
          ...
          description: >-
            ...
            format used by the Zobi example loading system. The export
    ...
    def cache_dashboard_screenshot(self, pk: int, **kwargs: Any) -> WerkzeugResponse:
        ...
        ) and not security_manager.can_access("can_export_image", "Zobi"):
            return self.response_403()
        try:
        ...
        dashboard_url = get_url_path("Zobi.dashboard_permalink", key=permalink_key)
    ...
    def thumbnail(self, pk: int, digest: str, **kwargs: Any) -> WerkzeugResponse:
        ...
        dashboard_url = get_url_path(
            "Zobi.dashboard", dashboard_id_or_slug=dashboard.id
        )
