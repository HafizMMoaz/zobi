...
from zobi_core.tasks.types import TaskOptions, TaskScope, TaskStatus

from zobi import is_feature_enabled
from zobi.commands.tasks.exceptions import GlobalTaskFrameworkDisabledError
from zobi.tasks.ambient_context import use_context
from zobi.tasks.constants import TERMINAL_STATES
...
from zobi.tasks.utils import generate_random_task_key
...
def task(
    ...
    *,
    ...
    Args:
        ...
        name: Optional unique task name (e.g., "zobi.generate_thumbnail").
...
class TaskWrapper(Generic[P]):
    ...
    def _execute_inline(
        ...
        :returns: Task in terminal state
        """
        from zobi.commands.tasks.internal_update import (
            InternalStatusTransitionCommand,
        )
        from zobi.daos.tasks import TaskDAO
        from zobi.tasks.constants import ABORT_STATES
