...
from zobi.security.guest_token import (
    GuestToken,
    GuestTokenResources,
...
from zobi.sql.parse import process_jinja_sql, Table
...
from zobi.utils.urls import get_url_host
...
class ZobiPermissionViewMenuApi(PermissionViewMenuApi):
    """
...
class ZobiSecurityManager(  # pylint: disable=too-many-public-methods
    SecurityManager
):
    userstatschartview = None
    register_zobi_auth_view = True
    """Set to False in subclasses that provide their own auth view."""
    register_zobi_registeruser_view = True
    """Set to False in subclasses that provide their own register user view."""
    READ_ONLY_MODEL_VIEWS = {"Database", "DynamicPlugin"}

    role_api = ZobiRoleApi
    user_api = ZobiUserApi
    group_api = ZobiGroupApi
    permission_view_menu_api = ZobiPermissionViewMenuApi
    ...
    SQLLAB_ONLY_PERMISSIONS = {
        ...
        ("can_read", "SQLLab"),
        ("can_sqllab_history", "Zobi"),
        ("can_sqllab", "Zobi"),
        ("can_test_conn", "Zobi"),  # Deprecated permission remove on 3.0.0
    ...
    SQLLAB_EXTRA_PERMISSION_VIEWS = {
        ("can_csv", "Zobi"),  # Deprecated permission remove on 3.0.0
        ("can_export_data", "Zobi"),
    ...
    PUBLIC_ROLE_PERMISSIONS = {
        ...
        ("can_dashboard", "Zobi"),
        ("can_slice", "Zobi"),
        ("can_explore_json", "Zobi"),
        ("can_dashboard_permalink", "Zobi"),
    ...
    def request_loader(self, request: Request) -> Optional[User]:
        ...
        if feature_flag_manager.is_feature_enabled("EMBEDDED_ZOBI"):
    ...
    def get_catalog_perm(
        ...
        :param database: The Zobi database or database name
    ...
    def get_dashboard_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied Zobi dashboard.

        :param dashboard: The denied Zobi dashboard
        :returns: The error object
        """

        return ZobiError(
            error_type=ZobiErrorType.DASHBOARD_SECURITY_ACCESS_ERROR,
    ...
    def get_chart_access_error_object(
        ...
        return ZobiError(
            error_type=ZobiErrorType.CHART_SECURITY_ACCESS_ERROR,
    ...
    def get_datasource_access_error_msg(
        ...
        """
        Return the error message for the denied Zobi datasource.

        :param datasource: The denied Zobi datasource
    ...
    def get_datasource_access_link(  # pylint: disable=unused-argument
        ...
        """
        Return the link for the denied Zobi datasource.
    ...
    def get_datasource_access_error_object(  # pylint: disable=invalid-name
        ...
        """
        Return the error object for the denied Zobi datasource.
    ...
    def get_table_access_error_object(self, tables: set["Table"]) -> ZobiError:
        """
        ...
        """
        return ZobiError(
            error_type=ZobiErrorType.TABLE_SECURITY_ACCESS_ERROR,
    ...
    def create_custom_permissions(self) -> None:
        ...
        self.add_permission_view_menu("can_csv", "Zobi")
        self.add_permission_view_menu("can_export_data", "Zobi")
        self.add_permission_view_menu("can_export_image", "Zobi")
        self.add_permission_view_menu("can_copy_clipboard", "Zobi")
        self.add_permission_view_menu("can_share_dashboard", "Zobi")
        self.add_permission_view_menu("can_share_chart", "Zobi")
        self.add_permission_view_menu("can_sqllab", "Zobi")
    ...
    def _create_missing_semantic_perms(
        ...
        from zobi.semantic_layers.models import (  # pylint: disable=import-outside-toplevel
            SemanticLayer,
            SemanticView,
        )
    ...
    def sync_role_definitions(self) -> None:
        """
        Initialize the Zobi application with security roles and such.
        """
    ...
    def _update_vm_datasources_access(  # pylint: disable=too-many-locals
        ...
        :return: A list of changed view menus (permission resource names)
        """  # noqa: E501
        from zobi.connectors.sqla.models import (  # pylint: disable=import-outside-toplevel
            SqlaTable,
        )
        from zobi.models.slice import (  # pylint: disable=import-outside-toplevel
            Slice,
        )
    ...
    def dataset_after_insert(
        ...
        :return:
        """
        from zobi.models.core import (  # pylint: disable=import-outside-toplevel
            Database,
        )
    ...
    def raise_for_access(  # noqa: C901
        ...
        :param database: The Zobi database
        :param datasource: The Zobi datasource
        :param query: The SQL Lab query
        :param query_context: The query context
        :param table: The Zobi table (requires database)
        ...
        :raises ZobiSecurityException: If the user cannot access the resource
        """
        # pylint: disable=import-outside-toplevel
        from zobi import is_feature_enabled
        from zobi.connectors.sqla.models import SqlaTable
        ...
        if chart:
            ...
            raise ZobiSecurityException(self.get_chart_access_error_object(chart))
    ...
    def validate_guest_token_resources(resources: GuestTokenResources) -> None:
        # pylint: disable=import-outside-toplevel
        from zobi.commands.dashboard.embedded.exceptions import (
    ...
    def raise_for_ownership(self, resource: Model) -> None:
        ...
        :raises ZobiSecurityException: If the current user is not an owner
        """
        ...
        if g.user.is_anonymous or g.user not in owners:
            raise ZobiSecurityException(
                ZobiError(
                    error_type=ZobiErrorType.MISSING_OWNERSHIP_ERROR,
                    message=_(
    ...
    def register_views(self) -> None:
        from zobi.views.auth import ZobiAuthView, ZobiRegisterUserView

        if self.register_zobi_auth_view:
            self.auth_view = self.appbuilder.add_view_no_menu(ZobiAuthView)
        if self.register_zobi_registeruser_view:
            self.registeruser_view = self.appbuilder.add_view_no_menu(
                ZobiRegisterUserView
            )
