...
from zobi.commands.exceptions import ForbiddenError
from zobi.exceptions import ZobiGenericErrorException
...
class SecurityRestApi(BaseZobiApi):
    ...
    def guest_token(self) -> Response:
        """Get a guest token that can be used for auth in embedded Zobi.
        ---
        post:
        ...
        try:
            ...
            if guest_token_validator_hook is not None:
                if callable(guest_token_validator_hook):
                    ...
                    raise ZobiGenericErrorException(
...
class UserRegistrationsRestAPI(BaseZobiModelRestApi):
    """
