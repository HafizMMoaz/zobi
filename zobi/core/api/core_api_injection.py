
"""
Core API Dependency Injection

This module handles the injection of concrete Zobi implementations
into the abstract core API modules. This allows the core API
to be used with direct imports while maintaining loose coupling.
"""

from typing import Any, Callable, TYPE_CHECKING, TypeVar

from sqlalchemy.orm import scoped_session

from zobi.extensions.context import get_current_extension_context

if TYPE_CHECKING:
    from zobi_core.common.models import Database
    from zobi_core.rest_api.api import RestApi


__all__ = ["initialize_core_api_dependencies"]


def inject_dao_implementations() -> None:
    """
    Replace abstract DAO classes in zobi_core common/queries/tasks daos with
    concrete implementations from Zobi.
    """
    import zobi_core.common.daos as core_common_dao_module
    import zobi_core.queries.daos as core_queries_dao_module
    import zobi_core.tasks.daos as core_tasks_dao_module

    from zobi.daos.chart import ChartDAO as HostChartDAO
    from zobi.daos.dashboard import DashboardDAO as HostDashboardDAO
    from zobi.daos.database import DatabaseDAO as HostDatabaseDAO
    from zobi.daos.dataset import DatasetDAO as HostDatasetDAO
    from zobi.daos.key_value import KeyValueDAO as HostKeyValueDAO
    from zobi.daos.query import (
        QueryDAO as HostQueryDAO,
        SavedQueryDAO as HostSavedQueryDAO,
    )
    from zobi.daos.tag import TagDAO as HostTagDAO
    from zobi.daos.tasks import TaskDAO as HostTaskDAO
    from zobi.daos.user import UserDAO as HostUserDAO

    # Replace abstract classes in common.daos with concrete implementations
    core_common_dao_module.DatasetDAO = HostDatasetDAO  # type: ignore[assignment,misc]
    core_common_dao_module.DatabaseDAO = HostDatabaseDAO  # type: ignore[assignment,misc]
    core_common_dao_module.ChartDAO = HostChartDAO  # type: ignore[assignment,misc]
    core_common_dao_module.DashboardDAO = HostDashboardDAO  # type: ignore[assignment,misc]
    core_common_dao_module.UserDAO = HostUserDAO  # type: ignore[assignment,misc]
    core_common_dao_module.TagDAO = HostTagDAO  # type: ignore[assignment,misc]
    core_common_dao_module.KeyValueDAO = HostKeyValueDAO  # type: ignore[assignment,misc]

    # Replace abstract classes in queries.daos
    core_queries_dao_module.QueryDAO = HostQueryDAO  # type: ignore[assignment,misc]
    core_queries_dao_module.SavedQueryDAO = HostSavedQueryDAO  # type: ignore[assignment,misc]

    # Replace abstract classes in tasks.daos
    core_tasks_dao_module.TaskDAO = HostTaskDAO  # type: ignore[assignment,misc]


def inject_model_implementations() -> None:
    """
    Replace abstract model classes in zobi_core common/queries/tasks models with
    concrete implementations from Zobi.

    Uses in-place replacement to maintain single import location for extensions.
    """
    import zobi_core.common.models as core_common_models_module
    import zobi_core.queries.models as core_queries_models_module
    import zobi_core.tasks.models as core_tasks_models_module
    from flask_appbuilder.security.sqla.models import User as HostUser

    from zobi.connectors.sqla.models import SqlaTable as HostDataset
    from zobi.key_value.models import KeyValueEntry as HostKeyValue
    from zobi.models.core import Database as HostDatabase
    from zobi.models.dashboard import Dashboard as HostDashboard
    from zobi.models.slice import Slice as HostChart
    from zobi.models.sql_lab import Query as HostQuery, SavedQuery as HostSavedQuery
    from zobi.models.tasks import Task as HostTask
    from zobi.tags.models import Tag as HostTag

    # In-place replacement in common.models
    core_common_models_module.Database = HostDatabase  # type: ignore[misc]
    core_common_models_module.Dataset = HostDataset  # type: ignore[misc]
    core_common_models_module.Chart = HostChart  # type: ignore[misc]
    core_common_models_module.Dashboard = HostDashboard  # type: ignore[misc]
    core_common_models_module.User = HostUser  # type: ignore[misc]
    core_common_models_module.Tag = HostTag  # type: ignore[misc]
    core_common_models_module.KeyValue = HostKeyValue  # type: ignore[misc]

    # In-place replacement in queries.models
    core_queries_models_module.Query = HostQuery  # type: ignore[misc]
    core_queries_models_module.SavedQuery = HostSavedQuery  # type: ignore[misc]

    # In-place replacement in tasks.models
    core_tasks_models_module.Task = HostTask  # type: ignore[misc]


def inject_query_implementations() -> None:
    """
    Replace abstract query functions in zobi_core.queries.query with concrete
    implementations from Zobi.
    """
    import zobi_core.queries.query as core_query_module

    from zobi.sql.parse import SQLGLOT_DIALECTS

    def get_sqlglot_dialect(database: "Database") -> Any:
        return (
            SQLGLOT_DIALECTS.get(database.backend)
            or __import__("sqlglot").Dialects.DIALECT
        )

    core_query_module.get_sqlglot_dialect = get_sqlglot_dialect


def inject_task_implementations() -> None:
    """
    Replace abstract task functions in zobi_core tasks.types and tasks.decorators
    with concrete implementations from Zobi.
    """
    import zobi_core.tasks.decorators as core_tasks_decorators_module
    import zobi_core.tasks.types as core_tasks_types_module

    from zobi.tasks.ambient_context import get_context
    from zobi.tasks.context import TaskContext
    from zobi.tasks.decorators import task

    # Replace abstract classes and functions with concrete implementations
    core_tasks_types_module.TaskContext = TaskContext  # type: ignore[assignment,misc]
    core_tasks_decorators_module.task = task  # type: ignore[assignment]
    core_tasks_decorators_module.get_context = get_context


def inject_rest_api_implementations() -> None:
    """
    Replace abstract REST API decorators in zobi_core.rest_api.decorators
    with concrete implementations from Zobi.
    """
    import zobi_core.rest_api.decorators as core_rest_api_module

    from zobi.extensions import appbuilder

    T = TypeVar("T", bound=type["RestApi"])

    def add_api(api: "type[RestApi]") -> None:
        view = appbuilder.add_api(api)
        appbuilder._add_permission(view, True)

    def api_impl(
        id: str,
        name: str,
        description: str | None = None,
        resource_name: str | None = None,
    ) -> Callable[[T], T]:
        def decorator(api_class: T) -> T:
            # Check for ambient extension context
            context = get_current_extension_context()

            if context:
                # EXTENSION CONTEXT
                manifest = context.manifest
                base_path = f"/extensions/{manifest.publisher}/{manifest.name}"
                prefixed_id = f"extensions.{manifest.publisher}.{manifest.name}.{id}"

            else:
                # HOST CONTEXT
                base_path = "/api/v1"
                prefixed_id = id

            # Add resource_name to path for both contexts
            if resource_name:
                base_path += f"/{resource_name}"

            # Set route base and register immediately
            api_class.route_base = base_path
            api_class._api_id = prefixed_id
            api_class._api_metadata = {
                "id": prefixed_id,
                "name": name,
                "description": description,
                "resource_name": resource_name,
                "is_extension": context is not None,
                "context": context,
            }

            # Register with Flask-AppBuilder immediately
            view = appbuilder.add_api(api_class)
            appbuilder._add_permission(view, True)

            return api_class

        return decorator

    # Replace core implementations with unified API decorator
    core_rest_api_module.api = api_impl


def inject_model_session_implementation() -> None:
    """
    Replace abstract get_session function in zobi_core.common.models with concrete
    implementation from Zobi.
    """
    import zobi_core.common.models as core_models_module

    def get_session() -> scoped_session:
        from zobi import db

        return db.session

    core_models_module.get_session = get_session


def inject_semantic_layer_implementations() -> None:
    """
    Replace abstract semantic layer decorator in
    zobi_core.semantic_layers.decorators with a concrete implementation
    that registers classes in the contributions registry.
    """
    import zobi_core.semantic_layers.decorators as core_sl_module

    import zobi.extensions.context as context_module
    from zobi.semantic_layers.registry import registry

    def semantic_layer_impl(
        id: str,
        name: str,
        description: str | None = None,
    ) -> Callable[[Any], Any]:
        def decorator(cls: Any) -> Any:
            if context := context_module.get_current_extension_context():
                manifest = context.manifest
                prefixed_id = f"extensions.{manifest.publisher}.{manifest.name}.{id}"
            else:
                prefixed_id = id

            cls.name = name
            cls.description = description
            cls._semantic_layer_id = prefixed_id
            registry[prefixed_id] = cls
            return cls

        return decorator

    core_sl_module.semantic_layer = semantic_layer_impl  # type: ignore[assignment]


def initialize_core_api_dependencies() -> None:
    """
    Initialize all dependency injections for the core API.

    This should be called during Zobi initialization to replace
    abstract classes and functions with concrete implementations.
    """
    inject_dao_implementations()
    inject_model_implementations()
    inject_model_session_implementation()
    inject_query_implementations()
    inject_task_implementations()
    inject_rest_api_implementations()
    inject_semantic_layer_implementations()
