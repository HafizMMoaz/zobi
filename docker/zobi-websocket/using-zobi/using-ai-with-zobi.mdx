...
Zobi supports AI assistants through the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/). Connect Claude, ChatGPT, or other MCP-compatible clients to explore your data, build charts, create dashboards, and run SQL -- all through natural language.

:::info
Requires Zobi 5.0+. Your admin must enable and deploy the MCP server before you can connect.
See the **[MCP Server admin guide](/admin-docs/configuration/mcp-server)** for setup instructions.
:::

---

## What Can AI Do with Zobi?

### Explore Your Data

Ask your AI assistant to browse what's available in your Zobi instance:
...
- **Get Explore links** -- open any chart in Zobi's Explore view for further refinement
...
1. **Explore** — AI generates an Explore link so you can see the chart before it exists in Zobi
...
Discover what database connections are configured in your Zobi instance:
...
**Example prompts:**
> "What databases are connected to Zobi?"
...
- **Run queries** -- execute SQL with full Zobi RBAC enforcement (you can only query data your roles allow)
...
- **Health check** -- verify your Zobi instance is up and the MCP connection is working
- **Instance info** -- get high-level statistics about your Zobi instance (number of datasets, charts, dashboards)
...
- **Use Explore for refinement** -- ask AI for an Explore link, then fine-tune interactively in the Zobi UI
- **Check permissions if you get errors** -- AI respects Zobi's RBAC, so you can only access data your roles allow
...
- Zobi's RBAC controls what you can access through AI, just like in the Zobi UI
- Ask your admin to verify your roles and permissions
- Try accessing the same data through the Zobi web UI to confirm your access
...
### AI doesn't see Zobi tools

- Verify the connection in your AI client (e.g., the hammer icon in Claude Desktop)
- Ask the AI "What Zobi tools are available?" to confirm the connection
