...
Zobi and provides additional context.
...
issue within Zobi, but instead a problem with the underlying
...
```
Zobi encountered an error while running a command.
```

Something unexpected happened, and Zobi encountered an error while
running a command. Please reach out to your administrator.

## Issue 1011

```
Zobi encountered an unexpected error.
```

Something unexpected happened in the Zobi backend. Please reach out
...
Your instance of Zobi doesn't have a results backend configured, which is needed for asynchronous queries. Please contact an administrator for further assistance.
...
The submitted query might be too complex to run under the time limit defined by your Zobi administrator. Please double check your query and verify if it can be optimized. Alternatively, contact your administrator to increase the timeout period.
