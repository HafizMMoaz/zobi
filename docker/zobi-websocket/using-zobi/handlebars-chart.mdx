...
Zobi registers several custom helpers on top of the standard Handlebars built-ins.
...
Zobi also registers all helpers from the [just-handlebars-helpers](https://github.com/leapfrogtechnology/just-handlebars-helpers) library. These include a wide range of comparison, math, string, and conditional helpers. Commonly used ones include:
...
- HTML output is sanitized by default based on your Zobi configuration (`HTML_SANITIZATION`).
