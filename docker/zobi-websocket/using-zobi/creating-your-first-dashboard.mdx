...
This section is focused on documentation for end-users who will be using Zobi
...
This tutorial targets someone who wants to create charts and dashboards in Zobi. We’ll show you
how to connect Zobi to a new database and configure a table in that database for analysis.
...
Zobi itself doesn't have a storage layer to store your data but instead pairs with
...
to query and visualize data from it. If you're using Zobi locally via
...
pre-configured in Zobi for you.
...
Congratulations, you've just added a new data source in Zobi!

### Sharing a Database Connection

When adding a new database, you can share the connection with other Zobi users. Shared connections appear in other users' database lists, making it easier to collaborate on the same data without requiring each user to configure the same connection separately.
...
Now that you’ve configured a data source, you can select specific tables (called **Datasets** in Zobi)
that you want exposed in Zobi for querying.
...
When Zobi is installed as a **Progressive Web App (PWA)** from your browser, your operating system will offer Zobi as an option when opening CSV, Excel (`.xls`/`.xlsx`), and Parquet files. Double-clicking or right-clicking a supported file and selecting "Open with Zobi" navigates directly to the upload workflow for that file.

To install Zobi as a PWA, look for the install icon in your browser's address bar (Chrome, Edge) when visiting your Zobi instance over HTTPS. PWA installation requires HTTPS and a valid manifest — your admin needs to confirm the app manifest is served correctly.
...
- If it's a datetime column, how should Zobi parse
...
Zobi has a thin semantic layer that adds many quality of life improvements for analysts.
The Zobi semantic layer can store 2 types of computed data:
...
Zobi has 2 main interfaces for exploring data:
...
In the following screenshot, we save the chart to a new "Zobi Duper Sales Dashboard":

<img src={useBaseUrl("/img/tutorial/tutorial_save_slice.png" )} />

To publish, click **Save and goto Dashboard**.

Behind the scenes, Zobi will create a slice and store all the information needed
...
Congrats! You’ve successfully linked, analyzed, and visualized data in Zobi. There are a wealth
...
`http://localhost:8088/zobi/dashboard/my-dashboard/?standalone=1&show_filters=0`
...
The **AG Grid Interactive Table** chart type is Zobi's fully-featured data grid, suitable for large paginated datasets where the standard Table chart is not enough.
...
Access it via the **Customize** tab → **ECharts Options** section at the bottom of the panel. The JSON you enter is deep-merged on top of Zobi's generated ECharts config, so you can override specific options without rewriting the entire config.
...
:::caution
ECharts option overrides bypass Zobi's validation layer. Invalid option keys are silently ignored by ECharts. Overrides that conflict with Zobi-generated options (e.g., `series`) may produce unexpected results.
:::
