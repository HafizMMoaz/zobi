...
export type EmbedDashboardParams = {
  /** The id provided by the embed configuration UI in Zobi */
  id: string;
  /** The domain where Zobi can be located, with protocol, such as: https://zobi.example.com */
  zobiDomain: string;
   ...
   * to allow the host app to customize the URL. If not provided, Zobi's default URL is used. */
...
/**
 * Embeds a Zobi dashboard into the page using an iframe.
 */
export async function embedDashboard({
  ...
  function log(...info: unknown[]) {
    if (debug) {
      console.debug(`[embedded-sdk][dashboard ${id}]`, ...info);
    }
  }

  log('embedding');

  if (zobiDomain.endsWith('/')) {
    zobiDomain = zobiDomain.slice(0, -1);
  }
  ...
  async function mountIframe(): Promise<Switchboard> {
    return new Promise(resolve => {
      ...
      iframe.src = `${zobiDomain}/embedded/${id}${urlParamsString}`;
