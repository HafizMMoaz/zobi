...
from zobi.exceptions import OAuth2Error, ZobiErrorException
from zobi.models.core import Database
from zobi.sql.parse import SQLStatement, Table
from zobi.sql_lab import (
    execute_query,
...
def test_execute_query(mocker: MockerFixture, app: None) -> None:
    ...
    ZobiResultSet = mocker.patch("zobi.sql_lab.ZobiResultSet")  # noqa: N806
    ...
    ZobiResultSet.assert_called_with([(42,)], cursor.description, db_engine_spec)
...
def test_execute_sql_statement_exceeds_payload_limit(
    ...
    mocker.patch("zobi.sql_lab.get_query", return_value=query)
    ...
    mocker.patch("zobi.sql_lab.results_backend", return_value=True)
...
def test_execute_sql_statement_within_payload_limit(mocker: MockerFixture, app) -> None:
    ...
    except ZobiErrorException:
        pytest.fail(
            "ZobiErrorException should not have been raised for payload within the limit"  # noqa: E501
        )
...
def test_get_sql_results_oauth2(mocker: MockerFixture, app) -> None:
    ...
    mocker.patch(
        "zobi.db_engine_specs.base.uuid4",
        return_value=UUID("fb11f528-6eba-4a8a-837e-6b0d39ee9187"),
    )
    mocker.patch(
        "zobi.db_engine_specs.base.generate_code_verifier",
        return_value="xkBPVZoFChVcy3VZ2l5u7d0FZPTU-olO7HtsAOok2IUGigyoZ62tG_oldy2xg9_HdqPKrWUmKZLmU-CUqz_SQ",
    )
    mocker.patch("zobi.daos.key_value.KeyValueDAO.delete_expired_entries")
    mocker.patch("zobi.daos.key_value.KeyValueDAO.create_entry")
    mocker.patch("zobi.db_engine_specs.base.db.session.commit")

    g = mocker.patch("zobi.db_engine_specs.base.g")
    ...
    assert error["error_type"] == ZobiErrorType.OAUTH2_REDIRECT
    ...
    from zobi.utils.oauth2 import generate_code_challenge
...
def test_apply_rls(mocker: MockerFixture) -> None:
    ...
    get_predicates_for_table = mocker.patch(
        "zobi.utils.rls.get_predicates_for_table",
...
def test_get_predicates_for_table(mocker: MockerFixture) -> None:
    ...
    db = mocker.patch("zobi.utils.rls.db")
