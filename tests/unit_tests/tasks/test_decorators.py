...
import pytest
from zobi_core.tasks.types import TaskOptions, TaskScope

from zobi.commands.tasks.exceptions import GlobalTaskFrameworkDisabledError
from zobi.tasks.decorators import task, TaskWrapper
...
class TestTaskDecoratorFeatureFlag:
    ...
    @patch("zobi.tasks.decorators.is_feature_enabled", return_value=False)
...
class TestTaskWrapperSchedule:
    ...
    @patch("zobi.tasks.decorators.TaskManager.submit_task")
...
class TestTaskWrapperCall:
    ...
    @patch("zobi.commands.tasks.update.UpdateTaskCommand.run")
    @patch("zobi.daos.tasks.TaskDAO.find_one_or_none")
    @patch("zobi.commands.tasks.submit.SubmitTaskCommand.run_with_info")
    ...
    @patch("zobi.utils.core.get_user_id")
    @patch("zobi.commands.tasks.update.UpdateTaskCommand.run")
    @patch("zobi.daos.tasks.TaskDAO.find_one_or_none")
    @patch("zobi.commands.tasks.submit.SubmitTaskCommand.run_with_info")
