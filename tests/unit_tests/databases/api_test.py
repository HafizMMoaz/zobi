...
from zobi.commands.database.uploaders.excel_reader import ExcelReader
...
def test_filter_by_uuid(
    ...
    Note: this functionality is not used by the Zobi UI, but is needed by 3rd
    party tools that use the Zobi API. If this tests breaks, please make sure
...
def test_password_mask(
    ...
    mocker.patch("zobi.utils.log.DBEventLogger.log")
...
def test_import(
    ...
    mocker.patch("zobi.databases.api.is_zipfile", return_value=True)
    mocker.patch("zobi.databases.api.ZipFile")
    mocker.patch(
        "zobi.databases.api.get_contents_from_bundle",
        return_value=contents,
    )
    command = mocker.patch("zobi.databases.api.ImportDatabasesCommand")
...
def test_oauth2_happy_path(
    ...
    mocker.patch(
        "zobi.commands.database.oauth2.KeyValueDAO.get_value",
        return_value=None,
    )
    ...
    mocker.patch("zobi.databases.api.render_template", return_value="OK")
...
def test_table_metadata_happy_path(
    ...
    mocker.patch("zobi.databases.api.DatabaseDAO.find_by_id", return_value=database)
    mocker.patch("zobi.databases.api.security_manager.raise_for_access")
...
def test_table_metadata_invalid_database(
    ...
    """
    mocker.patch("zobi.databases.api.DatabaseDAO.find_by_id", return_value=None)
        ...
        "errors": [
            {
                ...
                "extra": {
                    "issue_codes": [
                        {
                            "code": 1011,
                            "message": "Issue 1011 - Zobi encountered an unexpected error.",  # noqa: E501
                        },
                        {
                            "code": 1036,
...
def test_table_metadata_unauthorized(
    ...
    mocker.patch("zobi.databases.api.DatabaseDAO.find_by_id", return_value=database)
    mocker.patch(
        "zobi.databases.api.security_manager.raise_for_access",
...
def test_catalogs(
    ...
    DatabaseDAO = mocker.patch("zobi.databases.api.DatabaseDAO")  # noqa: N806
    DatabaseDAO.find_by_id.return_value = database

    security_manager = mocker.patch(
        "zobi.databases.api.security_manager",
...
def test_import_includes_configuration_method(
    ...
    with (
        patch("zobi.databases.api.is_zipfile", return_value=True),
        patch("zobi.databases.api.ZipFile"),
        patch("zobi.databases.api.get_contents_from_bundle", return_value=contents),
    ):
        form_data = {"formData": (BytesIO(b"test"), "test.zip")}
        response = client.post(
            "/api/v1/database/import/",
