...
if TYPE_CHECKING:
    from zobi.databases.schemas import DatabaseParametersSchemaMixin
...
def dummy_engine(mocker: MockerFixture) -> None:
    ...
    """
    from zobi.db_engine_specs.base import BasicParametersMixin

    class DummyEngine(BasicParametersMixin):
        engine = "dummy"
        default_driver = "dummy"

    mocker.patch("zobi.databases.schemas.get_engine_spec", return_value=DummyEngine)
...
def mock_bq_engine(mocker: MockerFixture) -> None:
    ...
    mocker.patch("zobi.databases.schemas.make_url_safe", return_value=mock_url)
    mocker.patch(
        "zobi.databases.schemas.get_engine_spec",
...
def test_database_parameters_schema_mixin(
    ...
    from zobi.models.core import ConfigurationMethod
...
def test_rename_encrypted_extra() -> None:
    ...
    """
    from zobi.databases.schemas import ConfigurationMethod, DatabasePostSchema
...
def test_import_schema_rejects_masked_fields_for_new_db(
    ...
    mock_session = mocker.patch("zobi.databases.schemas.db.session")
