...
from zobi.mcp_service.caching import _build_caching_settings
    ...
    with patch(
        ...
        with patch("flask.has_app_context", return_value=True):
            with patch(
                ...
                from zobi.mcp_service.caching import (
...
def test_create_response_caching_middleware_creates_middleware():
    ...
    with patch(
        ...
        with patch("flask.has_app_context", return_value=True):
            with patch(
                "zobi.mcp_service.caching.get_mcp_store", return_value=mock_store
            ):
                with patch(
                    "fastmcp.server.middleware.caching.ResponseCachingMiddleware",
                    return_value=mock_middleware,
