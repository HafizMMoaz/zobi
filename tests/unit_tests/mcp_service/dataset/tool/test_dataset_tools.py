...
from zobi.mcp_service.privacy import (
...
list_datasets_module = importlib.import_module(
    "zobi.mcp_service.dataset.tool.list_datasets"
)
get_dataset_info_module = importlib.import_module(
    "zobi.mcp_service.dataset.tool.get_dataset_info"
)
...
def test_dataset_discovery_tools_require_drill_permission() -> None:
    ...
    from zobi.mcp_service.dataset.tool.get_dataset_info import get_dataset_info
    from zobi.mcp_service.dataset.tool.list_datasets import list_datasets
...
async def test_get_dataset_info_returns_structured_privacy_error(mcp_server) -> None:
    ...
    from zobi.mcp_service.dataset.schemas import GetDatasetInfoRequest
...
def mock_auth():
    ...
    with patch("zobi.mcp_service.auth.get_user_from_request") as mock_get_user:
        mock_user = Mock()
        mock_user.id = 1
...
@patch("zobi.daos.dataset.DatasetDAO.list")
@pytest.mark.asyncio
...
async def test_get_dataset_info_success(mock_info, mock_base_url, mcp_server):
    ...
    async with Client(mcp_server) as client:
        ...
        assert (
            data["url"]
            == "http://test-zobi/explore/?datasource_type=table&datasource_id=1"
        )
...
@patch("zobi.mcp_service.mcp_core.ModelGetInfoCore._find_object")
@pytest.mark.asyncio
...
class TestDatasetCertificationSerialization:
    ...
    def test_serialize_dataset_with_certification_fields(self):
        ...
        from zobi.mcp_service.dataset.schemas import serialize_dataset_object
...
class TestDatasetDefaultColumnFiltering:
    ...
    def test_minimal_default_columns_constant(self):
        ...
        from zobi.mcp_service.common.schema_discovery import DATASET_DEFAULT_COLUMNS
    ...
    async def test_default_columns_filters_actual_response_data(
        ...
        See: https://github.com/HafizMMoaz/zobi/pull/37213
        """
        dataset = create_mock_dataset()
        mock_list.return_value = ([dataset], 1)
...
class TestDatasetSortableColumns:
    ...
    def test_dataset_sortable_columns_definition(self):
        ...
        from zobi.mcp_service.dataset.tool.list_datasets import (
...
async def test_create_virtual_dataset_success(mcp_server: object) -> None:
    ...
    with (
        patch(
            "zobi.commands.dataset.create.CreateDatasetCommand",
