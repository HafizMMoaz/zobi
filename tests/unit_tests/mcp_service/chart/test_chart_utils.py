...
class TestMapXYConfig:
    ...
    def test_map_xy_config_without_color_scheme(self) -> None:
        """color_scheme key omitted when not set, leaving Zobi default."""
        config = XYChartConfig(
            chart_type="xy",
            x=ColumnRef(name="date"),
            y=[ColumnRef(name="revenue")],
    ...
    @patch("zobi.mcp_service.chart.chart_utils.is_column_truly_temporal")
...
class TestMapConfigToFormData:
    ...
    @patch(
        "zobi.mcp_service.chart.chart_utils.is_column_truly_temporal",
        return_value=True,
    )
...
class TestGenerateExploreLink:
    ...
    @patch("zobi.mcp_service.chart.chart_utils.get_zobi_base_url")
    ...
    @patch("zobi.mcp_service.chart.chart_utils.get_zobi_base_url")
    @patch("zobi.mcp_service.commands.create_form_data.MCPCreateFormDataCommand")
    def test_generate_explore_link_with_form_data_key(
        ...
        with patch(
            "zobi.daos.dataset.DatasetDAO.find_by_id", return_value=mock_dataset
        ):
    ...
    def test_generate_explore_link_exception_handling(self, mock_get_base_url) -> None:
        ...
        with patch(
            "zobi.daos.dataset.DatasetDAO.find_by_id",
...
class TestCriticalBugFixes:
    ...
    def test_chart_type_mapping_comprehensive(self) -> None:
        """Test that chart types are mapped correctly to Zobi viz types."""
        test_cases = [
...
class TestIsColumnTrulyTemporal:
    ...
    def _create_mock_dataset(
        ...
        from zobi.utils.core import ColumnSpec
    ...
    @patch("zobi.daos.dataset.DatasetDAO")
...
class TestValidateChartDataset:
    ...
    @patch("zobi.mcp_service.auth.has_dataset_access")
    @patch("zobi.daos.dataset.DatasetDAO.find_by_id")
    ...
    @patch("zobi.mcp_service.auth.has_dataset_access")
    @patch("zobi.daos.dataset.DatasetDAO.find_by_id", return_value=None)
    ...
    @patch("zobi.mcp_service.auth.has_dataset_access", return_value=True)
    @patch("zobi.daos.dataset.DatasetDAO.find_by_id")
    ...
    @patch(
        "zobi.mcp_service.chart.chart_utils.get_zobi_base_url",
