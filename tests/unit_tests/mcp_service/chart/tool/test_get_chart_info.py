...
get_chart_info_module = importlib.import_module(
    "zobi.mcp_service.chart.tool.get_chart_info"
)
...
class TestGetChartInfoPrivacy:
    ...
    async def test_restricted_user_redacts_saved_chart_data_model_fields(
        ...
        with (
            ...
            patch("zobi.daos.chart.ChartDAO.find_by_id", return_value=Mock()),
            patch("zobi.mcp_service.auth.check_tool_permission", return_value=True),
        ):
