...
from zobi.mcp_service.chart.tool.get_chart_data import (
    _GENERIC_TYPE_MAP,
...
from zobi.mcp_service.utils.sanitization import LLM_CONTEXT_ESCAPED_CLOSE_DELIMITER
...
class TestUnsavedChartDataQueryConstruction:
    ...
    async def test_form_data_key_adhoc_filters_become_query_filters(
        ...
        query_context_factory_module = importlib.import_module(
            "zobi.common.query_context_factory"
        )
        get_data_command_module = importlib.import_module(
            "zobi.commands.chart.data.get_data_command"
        )
    ...
    async def test_form_data_key_mixed_timeseries_builds_secondary_query(
        ...
        monkeypatch.setattr(
            "zobi.mcp_service.chart.chart_helpers.resolve_datasource_engine",
...
class TestChartDataCommandValidation:
    ...
    def test_preview_utils_calls_validate_before_run(self):
        ...
        with (
            patch("zobi.extensions.db") as mock_db,
            patch(
                "zobi.commands.chart.data.get_data_command.ChartDataCommand",
                return_value=mock_command,
            ),
            patch(
                "zobi.common.query_context_factory.QueryContextFactory"
            ) as mock_factory,
        ):
    ...
    def test_preview_utils_security_exception_from_validate(self):
        """Test that ZobiSecurityException from validate() is propagated."""
        from unittest.mock import MagicMock, patch
        ...
        security_error = ZobiSecurityException(
            ZobiError(
        ...
        with (
            ...
            # ZobiSecurityException is caught by the broad except and
    ...
    def test_compile_chart_calls_validate_before_run(self):
        ...
        with (
            ...
            from zobi.mcp_service.chart.tool.generate_chart import _compile_chart
    ...
    def test_compile_chart_security_exception_from_validate(self):
        ...
        with (
            ...
            # ZobiSecurityException is not caught by _compile_chart's
...
def mock_auth():
    ...
    with (
        patch("zobi.mcp_service.auth.get_user_from_request") as mock_get_user,
...
class TestChartLookupEagerLoading:
    ...
    async def test_numeric_id_lookup_passes_metrics_eager_load(
        ...
        with patch(
            "zobi.daos.chart.ChartDAO.find_by_id", return_value=None
        ) as mock_find:
