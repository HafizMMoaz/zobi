...
from zobi.mcp_service.chart.chart_helpers import find_chart_by_identifier
...
from zobi.mcp_service.chart.tool.update_chart import (
...
update_chart_module = importlib.import_module(
    "zobi.mcp_service.chart.tool.update_chart"
)
...
class TestUpdateChartDatasetAccess:
    ...
    @patch("zobi.daos.chart.ChartDAO.find_by_id", new_callable=Mock)
...
class TestUpdateChartNameOnly:
    ...
    @patch(
        "zobi.commands.chart.update.UpdateChartCommand",
        new_callable=Mock,
    )
    @patch("zobi.daos.chart.ChartDAO.find_by_id", new_callable=Mock)
    @patch("zobi.db.session")
...
class TestUpdateChartValidationGate:
    ...
    async def test_preview_path_validation_failure_skips_cache(
        ...
        must NOT be called."""
        from zobi.mcp_service.chart.compile import CompileResult
        from zobi.mcp_service.common.error_schemas import ChartGenerationError
