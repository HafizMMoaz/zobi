...
def test_jwt_context_resolves_correct_user(app) -> None:
    ...
    with app.app_context():
        with (
            ...
            patch(
                "zobi.mcp_service.auth.load_user_with_relationships",
...
def test_jwt_context_raises_for_unknown_user(app) -> None:
    ...
    with app.app_context():
        with (
            ...
            with pytest.raises(ValueError, match="not found in Zobi database"):
...
def test_mcp_auth_hook_removes_stale_db_session_in_sync_wrapper(app) -> None:
    ...
    with app.test_request_context():
        g.user = fresh_user
        with patch("zobi.extensions.db") as mock_db:
...
def test_setup_user_context_propagates_valueerror(app) -> None:
    ...
    """
    from zobi.mcp_service.auth import _setup_user_context
