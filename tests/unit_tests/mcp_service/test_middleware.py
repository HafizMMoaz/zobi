...
from zobi.mcp_service.mcp_config import MCP_RESPONSE_SIZE_CONFIG
from zobi.mcp_service.middleware import (
    _is_user_error,
...
class TestResponseSizeGuardMiddleware:
    ...
    async def test_allows_small_response(self) -> None:
        ...
        with (
            patch("zobi.mcp_service.middleware.get_user_id", return_value=1),
            patch("zobi.mcp_service.middleware.event_logger"),
        ):
    ...
    async def test_logs_warning_at_threshold(self) -> None:
        ...
        with (
            ...
            patch("zobi.mcp_service.middleware.event_logger"),
            patch(
                "zobi.mcp_service.middleware.estimate_response_tokens",
                return_value=850,
            ),
            patch("zobi.mcp_service.middleware.logger") as mock_logger,
        ):
            result = await middleware.on_call_tool(context, call_next)
    ...
    async def test_logs_size_exceeded_event(self) -> None:
        ...
        with (
            patch("zobi.mcp_service.middleware.get_user_id", return_value=1),
            patch("zobi.mcp_service.middleware.event_logger") as mock_event_logger,
...
class TestCreateResponseSizeGuardMiddleware:
    ...
    def test_default_config_checks_chart_preview(self) -> None:
        ...
        with patch(
            "zobi.mcp_service.flask_singleton.get_flask_app",
...
def _make_security_exception(msg: str = "access denied") -> ZobiSecurityException:
    """Helper to construct ZobiSecurityException with a proper ZobiError."""
...
class TestIsUserError:
    ...
    def test_zobi_exception_status_based(self) -> None:
        """Test ZobiException classification is based on .status attribute."""
        # 4xx status → user error
        error_400 = ZobiException("bad request")
        error_400.status = 400
        assert _is_user_error(error_400) is True

        error_408 = ZobiException("timeout")
        error_408.status = 408
        assert _is_user_error(error_408) is True

        error_422 = ZobiException("unprocessable")
        error_422.status = 422
        assert _is_user_error(error_422) is True

        # 5xx status → system error
        error_500 = ZobiException("internal error")
        error_500.status = 500
        assert _is_user_error(error_500) is False

        error_503 = ZobiException("unavailable")
...
class TestGlobalErrorHandlerLogLevels:
    ...
    async def test_event_logger_includes_severity(self) -> None:
        ...
        with (
            patch("zobi.mcp_service.middleware.get_user_id", return_value=1),
            patch("zobi.mcp_service.middleware.event_logger") as mock_event_logger,
            patch("zobi.mcp_service.middleware.logger"),
            pytest.raises(ToolError),
        ):
            await middleware.on_message(context, call_next)
    ...
    @pytest.mark.asyncio
    async def test_zobi_exception_4xx_logs_warning(self) -> None:
        """ZobiException with 4xx status should log at WARNING."""
        middleware = GlobalErrorHandlerMiddleware()
        ...
        error = ZobiException("bad request")
        error.status = 400
    ...
    @pytest.mark.asyncio
    async def test_zobi_exception_5xx_logs_error(self) -> None:
        """ZobiException with 5xx status should log at ERROR."""
        middleware = GlobalErrorHandlerMiddleware()
        ...
        error = ZobiException("internal failure")
        error.status = 500
        call_next = AsyncMock(side_effect=error)
