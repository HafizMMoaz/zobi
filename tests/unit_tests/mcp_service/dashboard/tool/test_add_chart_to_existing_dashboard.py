...
- Patches applied at source locations (zobi.daos.dashboard.*, zobi.db.*, etc.)
...
@patch("zobi.security_manager.raise_for_ownership")
@patch("zobi.daos.dashboard.DashboardDAO.find_by_id")
@pytest.mark.asyncio
async def test_permission_denied(
    ...
    mock_raise_for_ownership.side_effect = ZobiSecurityException(
        ZobiError(
...
async def test_successful_add(
    ...
    assert "/zobi/dashboard/1/" in content["dashboard_url"]
