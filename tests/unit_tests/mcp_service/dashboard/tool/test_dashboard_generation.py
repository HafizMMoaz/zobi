...
from zobi.mcp_service.chart.chart_utils import DatasetValidationResult
from zobi.mcp_service.dashboard.constants import generate_id
from zobi.mcp_service.dashboard.tool.add_chart_to_existing_dashboard import (
    _add_chart_to_layout,
    _ensure_layout_structure,
    _find_next_row_position,
    _find_tab_insert_target,
)
from zobi.mcp_service.dashboard.tool.generate_dashboard import (
...
add_chart_to_existing_dashboard_module = import_module(
    "zobi.mcp_service.dashboard.tool.add_chart_to_existing_dashboard"
)
generate_dashboard_module = import_module(
    "zobi.mcp_service.dashboard.tool.generate_dashboard"
)
...
def mock_auth():
    ...
    with patch("zobi.mcp_service.auth.get_user_from_request") as mock_get_user:
        with patch("zobi.security_manager.raise_for_ownership"):
            mock_user = Mock()
            mock_user.id = 1
...
def mock_chart_access():
    ...
    with patch(
        "zobi.mcp_service.auth.check_chart_data_access",
...
class TestAddChartToExistingDashboard:
    ...
    @patch("zobi.commands.dashboard.update.UpdateDashboardCommand")
    @patch("zobi.daos.dashboard.DashboardDAO.find_by_id")
    @patch("zobi.db.session")
    @pytest.mark.asyncio
    ...
    async def test_add_chart_to_tabbed_dashboard_tabs_under_root(
        ...
        Real Zobi dashboards place TABS directly under ROOT_ID with an
...
class TestLayoutHelpers:
    ...
    def test_ensure_layout_structure_tabs_under_root_no_grid_added(self):
        ...
        Real Zobi tabbed dashboards place TABS under ROOT_ID, not
