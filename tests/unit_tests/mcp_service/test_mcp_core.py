...
from zobi.mcp_service.mcp_core import _slugify, ModelGetInfoCore
...
def _patch_id_or_slug_filter():
    ...
    with patch("zobi.models.dashboard.id_or_slug_filter", return_value=MagicMock()):
        yield
...
def test_title_fallback_ilike_pattern_preserves_word_order() -> None:
    ...
    with patch(
        "zobi.mcp_service.mcp_core.func.replace", side_effect=fake_replace
    ) as replace_spy:
        result = core.run_tool("world-banks-data")
...
def test_title_fallback_respects_base_filter_rbac() -> None:
    ...
    with (
        patch("zobi.mcp_service.mcp_core.db") as mock_db,
        patch("zobi.mcp_service.mcp_core.SQLAInterface") as mock_interface,
    ):
