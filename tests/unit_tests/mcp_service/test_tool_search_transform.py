...
from zobi.mcp_service.mcp_config import MCP_TOOL_SEARCH_CONFIG
from zobi.mcp_service.privacy import requires_data_model_metadata_access
from zobi.mcp_service.server import (
...
def test_tool_search_filter_hides_metadata_tools_without_access() -> None:
    ...
    with app.app_context():
        g.user = SimpleNamespace(username="viewer")
        with patch(
            "zobi.mcp_service.server.user_can_view_data_model_metadata",
            return_value=False,
        ):
...
def test_tool_search_permission_filter_still_applies_rbac_to_metadata_tools() -> None:
    ...
    with app.app_context():
        ...
        with (
            ...
            patch("zobi.security_manager", new_callable=Mock) as security_manager,
        ):
