...
from zobi.mcp_service.app import get_default_instructions, init_fastmcp_server


def test_get_default_instructions_with_default_branding():
    """Test that default branding produces Zobi in instructions."""
    instructions = get_default_instructions()

    assert "Zobi" in instructions
    assert "Zobi MCP" in instructions
...
def test_get_default_instructions_with_custom_branding():
    ...
    # Should not contain default Zobi branding
    assert "Zobi" not in instructions
...
def test_init_fastmcp_server_with_default_app_name():
    """Test that default APP_NAME produces Zobi branding."""
    mock_flask_app = _mock_flask_config("Zobi")

    # Patch at the import location to avoid actual Flask app creation
    with patch.dict(
        "sys.modules",
        {"zobi.mcp_service.flask_singleton": MagicMock(app=mock_flask_app)},
    ):
        with patch("zobi.mcp_service.app.mcp") as mock_mcp:
            init_fastmcp_server()

            # Verify the global mcp instance was configured with Zobi branding
            assert "Zobi MCP" in mock_mcp._mcp_server.instructions
            assert "Zobi dashboards" in mock_mcp._mcp_server.instructions
...
def test_init_fastmcp_server_with_custom_app_name():
    ...
    with patch.dict(
        ...
        with patch("zobi.mcp_service.app.mcp") as mock_mcp:
            ...
            # Should not contain default Zobi branding
            assert "Zobi" not in mock_mcp._mcp_server.instructions
