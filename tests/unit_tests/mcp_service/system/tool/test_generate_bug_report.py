...
from zobi.mcp_service.system.tool.generate_bug_report import _sanitize_text
...
class TestGenerateBugReportViaMCP:
    ...
    async def test_generate_bug_report_builds_markdown_and_redacts(self, mcp_server):
        ...
        assert "Zobi" in data["support_contact"]
        report = data["report"]

        # Structure
        assert "# Zobi MCP Bug Report" in report
...
def test_request_rejects_oversized_error_message():
    ...
    from zobi.mcp_service.system.schemas import GenerateBugReportRequest
...
def test_collect_environment_falls_back_when_version_unavailable():
    ...
    assert env["zobi_version"] == "unknown"
...
def test_collect_user_context_handles_role_typeerror():
    ...
    from zobi.mcp_service.system.tool.generate_bug_report import (
