...
from zobi.mcp_service.sql_lab.schemas import OpenSqlLabRequest
from zobi.mcp_service.utils.sanitization import sanitize_for_llm_context
...
def _restore_modules(saved_modules: dict[str, types.ModuleType]) -> None:
    ...
    for key in list(sys.modules.keys()):
        if key.startswith(("zobi_core.api", "zobi_core.mcp")) or key.startswith(
...
def _get_tool_module() -> tuple[types.ModuleType, dict[str, types.ModuleType]]:
    ...
    mod_name = "zobi.mcp_service.sql_lab.tool.open_sql_lab_with_context"
...
class TestOpenSqlLabWithContext:
    ...
    def test_sanitizes_direct_sql_and_title_in_url_and_response(self) -> None:
        ...
        try:
            ...
            with (
                patch(
                    "zobi.daos.database.DatabaseDAO.find_by_id",
                ...
                patch.object(
                    mod,
                    "get_zobi_base_url",
                    return_value="https://zobi.example.com",
                ),
            ):
            ...
            assert parsed.netloc == "zobi.example.com"
