...
from zobi_core.queries.types import QueryResult, QueryStatus, StatementResult

from zobi.mcp_service.app import mcp
from zobi.mcp_service.sql_lab.schemas import ColumnInfo
...
class TestExecuteSql:
    ...
    @patch("zobi.security_manager", new_callable=MagicMock)
    @patch("zobi.db")
    @pytest.mark.asyncio
...
class TestSanitizeRowValues:
    ...
    def test_sanitize_utf8_bytes(self):
        from zobi.mcp_service.sql_lab.tool.execute_sql import _sanitize_row_values
...
class TestDestructiveDDLBlocking:
    ...
    def ddl_mocks(self):
        ...
        with (
            patch("zobi.db") as mock_db,
            patch("zobi.security_manager") as mock_sm,
        ):
            mock_database = _mock_database()
    ...
    async def test_parse_failure_blocks_query(self, ddl_mocks, mcp_server):
        ...
        execute_sql_mod = sys.modules["zobi.mcp_service.sql_lab.tool.execute_sql"]
        with patch.object(
            execute_sql_mod,
            "SQLScript",
