...
def _force_passthrough_decorators():
    """Force zobi_core MCP tool decorator to be a passthrough.

    In CI, zobi_core is fully installed and the real @tool decorator
    ...
    # zobi_core package. Replacing sys.modules["zobi_core"] with
    # a MagicMock causes 'zobi_core' is not a package errors for
    ...
    mock_keys = [
        ...
        "zobi_core.api.types",
        "zobi_core.mcp",
        "zobi_core.mcp.decorators",
    ]
    for key in mock_keys:
        if key in sys.modules:
            saved_modules[key] = sys.modules[key]

    sys.modules["zobi_core.api"] = mock_api
    sys.modules["zobi_core.api.mcp"] = mock_mcp
    sys.modules["zobi_core.mcp"] = mock_mcp
    sys.modules["zobi_core.mcp.decorators"] = mock_decorators
    sys.modules.setdefault("zobi_core.api.types", MagicMock())
...
def _restore_modules(saved_modules: dict[str, types.ModuleType]) -> None:
    ...
    # under patched decorators. Do NOT remove the top-level zobi_core
    # package or unrelated submodules (queries, common, etc.).
    mock_prefixes = (
        "zobi_core.api",
        "zobi_core.mcp",
        "zobi.mcp_service.sql_lab.tool",
    )
...
def _get_tool_module():
    ...
    mod_name = "zobi.mcp_service.sql_lab.tool.save_sql_query"
    saved_tool_modules: dict[str, object] = {}
    for key in list(sys.modules.keys()):
        if key.startswith("zobi.mcp_service.sql_lab.tool"):
...
class TestSaveSqlQueryToolLogic:
    ...
    from zobi import db, etc.). We patch at the import source so that
    ...
    async def test_save_query_creates_saved_query(self) -> None:
        ...
        try:
            ...
            with (
                patch("zobi.db", mock_db_session),
                patch("zobi.security_manager", mock_sm),
                patch("zobi.daos.query.SavedQueryDAO", mock_dao),
                patch(
                    "zobi.mcp_service.utils.url_utils.get_zobi_base_url",
    ...
    async def test_save_query_database_not_found(self) -> None:
        ...
        try:
            ...
            with (
                ...
                from zobi.exceptions import ZobiErrorException

                with pytest.raises(ZobiErrorException, match="not found"):
    ...
    async def test_save_query_access_denied(self) -> None:
        ...
        try:
            ...
            with (
                ...
                with pytest.raises(ZobiSecurityException, match="Access denied"):
                    await mod.save_sql_query(request, mock_ctx)
        finally:
            _restore_modules(saved)
