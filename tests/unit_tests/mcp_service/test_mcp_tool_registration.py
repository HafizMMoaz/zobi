...
from zobi.mcp_service.app import get_default_instructions, init_fastmcp_server, mcp
...
def test_mcp_resources_registered():
    ...
    assert "chart://configs" in resource_uris, (
        ...
        "check zobi/mcp_service/chart/__init__.py exists"
    )
    assert "instance://metadata" in resource_uris, (
        "instance://metadata resource not registered - "
        "check zobi/mcp_service/system/resources/ imports"
    )
...
def test_mcp_packages_discoverable_by_setuptools():
    ...
    mcp_root = Path(__file__).parents[3] / "zobi" / "mcp_service"
...
def test_disabled_tools_are_removed_from_mcp_server() -> None:
    ...
    with (
        patch(
            "zobi.mcp_service.flask_singleton.app",
            flask_app,
        ),
...
def test_unknown_disabled_tool_logs_warning_not_raises(caplog) -> None:
    ...
    with (
        ...
        caplog.at_level(logging.WARNING, logger="zobi.mcp_service.app"),
    ):
        # Must not raise
        init_fastmcp_server()
...
def test_disabled_tools_read_from_flask_app_config() -> None:
    ...
    Zobi pattern where users set overrides in zobi_config.py, which
    create_app() loads into Flask config before any command runs."""
    from zobi.mcp_service.app import init_fastmcp_server, mcp
...
def test_instructions_generated_after_disabled_tools_removed() -> None:
    ...
    with (
        patch("zobi.mcp_service.flask_singleton.app", flask_app),
        patch.object(mcp.local_provider, "remove_tool"),
        patch(
            "zobi.mcp_service.app.get_default_instructions",
