...
def test_extract_tables_illdefined() -> None:
    ...
    """
    with pytest.raises(ZobiParseError) as excinfo:
...
def test_kustokqlstatement() -> None:
    ...
    with pytest.raises(ZobiParseError, match="Invalid engine: invalid-engine"):
...
def test_kusto_is_destructive(kql: str, expected: bool) -> None:
    ...
    """
    from zobi.sql.parse import KustoKQLStatement
...
def test_firebolt_old() -> None:
    ...
    """
    from zobi.sql.dialects import FireboltOld
...
def test_backtick_invalid_sql_still_fails() -> None:
    ...
    with pytest.raises(ZobiParseError):
        SQLScript(sql, "base")
