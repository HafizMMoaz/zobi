...
from zobi_core.queries.types import (
    CacheOptions,
    QueryOptions,
    QueryStatus,
)
...
def test_execute_creates_query_record(
    ...
    from zobi.sql.execution.executor import SQLExecutor
...
def test_execute_with_template_params(
    ...
    mocker.patch(
        "zobi.jinja_context.get_template_processor",
...
def test_execute_without_template_params_no_rendering(
    ...
    mock_get_tp = mocker.patch("zobi.jinja_context.get_template_processor")
...
def test_execute_timeout(
    ...
    from zobi.exceptions import ZobiTimeoutException
...
def test_execute_custom_timeout(
    ...
    mock_timeout = mocker.patch("zobi.sql.execution.executor.utils.timeout")
...
def test_execute_oauth2_redirect_error_propagates(
    ...
    mocker.patch.object(
        ...
        side_effect=OAuth2RedirectError(
            ...
            tab_id="test-tab",
            redirect_uri="https://zobi.example.com/callback",
        ),
    )
    mocker.patch.dict(
        current_app.config,
...
def test_execute_async_dml_without_permission_raises(
    ...
    with pytest.raises(ZobiSecurityException, match="DML queries are not allowed"):
...
def test_async_handle_get_status(
    ...
    mocker.patch("zobi.sql.execution.celery_task.execute_sql_task")
...
def test_execute_multi_statement_updates_query_progress(
    ...
    from zobi.models.sql_lab import Query as QueryModel
    ...
    mock_result_set = MagicMock(spec=ZobiResultSet)
    mock_result_set.to_pandas_df.return_value = pd.DataFrame(
        {"id": ["1"], "name": ["Alice"]}
    )
    mocker.patch("zobi.result_set.ZobiResultSet", return_value=mock_result_set)
    ...
    mock_query.id = 123
    mocker.patch("zobi.models.sql_lab.Query", return_value=mock_query)
...
def test_execute_sql_with_cursor_empty_statements(app_context: None) -> None:
    ...
    from zobi.sql.execution.executor import execute_sql_with_cursor
...
def test_execute_sql_with_cursor_stopped_mid_execution(
    ...
    mocker.patch("zobi.sql.execution.executor.db.session")
...
def test_async_handle_get_result_with_results_backend(
    ...
    mocker.patch(
        "zobi.results_backend_manager",
        mock_results_backend_manager,
    )
    mocker.patch("zobi.utils.core.zlib_decompress", return_value=payload)
    mocker.patch.dict(
        current_app.config, {"SQL_QUERY_MUTATOR": None, "SQLLAB_TIMEOUT": 30}
    )
    mocker.patch("zobi.sql.execution.celery_task.execute_sql_task")
...
def test_async_handle_get_result_backend_load_error(
    ...
    mocker.patch(
        "zobi.utils.core.zlib_decompress",
...
def test_cancel_query_early_cancel_flag(
    ...
    from zobi.constants import QUERY_EARLY_CANCEL_KEY
    from zobi.sql.execution.executor import SQLExecutor
...
def test_cancel_query_with_cancel_id(
    ...
    from zobi.constants import QUERY_CANCEL_KEY
    from zobi.sql.execution.executor import SQLExecutor
    from zobi.utils.core import QuerySource
...
def test_execute_uses_database_cache_timeout(
    ...
    mocker.patch.object(SQLExecutor, "_get_from_cache", return_value=None)
    mock_cache_set = mocker.patch("zobi.extensions.cache_manager.data_cache.set")
...
def test_execute_with_exception_on_execute(
    ...
    mocker.patch(
        "zobi.sql.execution.executor.execute_sql_with_cursor",
...
def test_store_in_cache_with_failed_status(
    ...
    from zobi_core.queries.types import QueryResult as QueryResultType
...
def test_create_query_record_with_user(
    ...
    mocker.patch("zobi.sql.execution.executor.has_app_context", return_value=True)
