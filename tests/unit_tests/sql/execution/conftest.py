...
def mock_db_session(mocker: MockerFixture) -> MagicMock:
    ...
    mocker.patch("zobi.sql.execution.executor.db.session", mock_session)
    mocker.patch("zobi.sql.execution.celery_task.db.session", mock_session)
    return mock_session
...
def mock_query_execution(
    ...
    # Create a real ZobiResultSet that converts to DataFrame properly
...
def mock_celery_task(mocker: MockerFixture) -> MagicMock:
    ...
    return mocker.patch("zobi.sql.execution.celery_task.execute_sql_task")
