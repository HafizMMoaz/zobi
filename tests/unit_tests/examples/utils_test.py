...
def _create_example_tree(base_dir: Path) -> Path:
    """Create a minimal example directory tree under base_dir/zobi/examples/.

    Returns the 'zobi' directory (what files("zobi") would return).
    """
    zobi_dir = base_dir / "zobi"
    examples_dir = zobi_dir / "examples"
...
def test_load_contents_builds_correct_import_structure():
    ...
    metadata.yaml).
    """
    from zobi.examples.utils import load_contents

    with TemporaryDirectory() as tmpdir:
        zobi_dir = _create_example_tree(Path(tmpdir))

        test_examples_uri = "sqlite:///path/to/examples.db"
        mock_app = MagicMock()
        mock_app.config = {"SQLALCHEMY_EXAMPLES_URI": test_examples_uri}

        with patch("zobi.examples.utils.files", return_value=zobi_dir):
...
def test_load_contents_replaces_sqlalchemy_examples_uri_placeholder():
    ...
    """
    from zobi.examples.utils import _load_shared_configs
...
@patch("zobi.examples.utils.ImportExamplesCommand")
@patch("zobi.examples.utils.load_contents")
def test_load_examples_from_configs_wires_command_correctly(
    ...
    """
    from zobi.examples.utils import load_examples_from_configs
