...
def test_df_to_records() -> None:
    ...
    results = ZobiResultSet(data, cursor_descr, BaseEngineSpec)
...
def test_max_pandas_timestamp(input_, expected) -> None:
    ...
    results = ZobiResultSet(input_, cursor_descr, BaseEngineSpec)
    df = results.to_pandas_df()
...
def test_df_to_records_nan_json_serialization() -> None:
    ...
    json_output = zobi_json.dumps(records)
    parsed = zobi_json.loads(json_output)
    ...
    with pytest.raises(
        ...
        zobi_json.dumps(records_without_fix, ignore_nan=False)
    ...
    json_with_ignore = zobi_json.dumps(records_without_fix, ignore_nan=True)
    parsed_with_ignore = zobi_json.loads(json_with_ignore)
...
def test_df_to_records_with_json_serialization_like_sql_lab() -> None:
    ...
    # Mimic sql_lab.py:332 - JSON serialization with Zobi's custom json.dumps
    # This should work without errors
    json_str = zobi_json.dumps(
        records, default=zobi_json.json_iso_dttm_ser, ignore_nan=True
    )

    # Verify it's valid JSON and NaN values are properly handled as null
    parsed = zobi_json.loads(json_str)
    ...
    json_str_no_flag = zobi_json.dumps(
        records, default=zobi_json.json_iso_dttm_ser, ignore_nan=False
    )
    parsed_no_flag = zobi_json.loads(json_str_no_flag)
