...
from zobi.sql_validators.sqlite import SQLiteSQLValidator
...
def test_valid_syntax() -> None:
    ...
    with (
        patch(
            "zobi.sql_validators.sqlite.get_binary_path",
...
def test_missing_syntaqlite_returns_annotation() -> None:
    mock_database = MagicMock()
    with patch("zobi.sql_validators.sqlite.get_binary_path", None):
        annotations = SQLiteSQLValidator.validate(
            sql="SELECT 1",
            catalog=None,
            schema="",
