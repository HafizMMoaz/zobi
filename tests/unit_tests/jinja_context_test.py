...
from zobi.exceptions import ZobiTemplateException
from zobi.jinja_context import (
    dataset_macro,
    ExtraCache,
...
def test_safe_proxy_lambda() -> None:
    ...
    Should raise ``ZobiTemplateException``.
    """

    def func(input_: Any) -> Any:
        return input_

    with pytest.raises(ZobiTemplateException):
...
def test_safe_proxy_nested_lambda() -> None:
    """
    Test the ``safe_proxy`` helper with a function returning a ``dict``
    containing ``lambda`` value. Should raise ``ZobiTemplateException``.
    """
...
def test_user_macros(
    ...
    """
    mock_g = mocker.patch("zobi.utils.core.g")
    mock_get_user_roles = mocker.patch("zobi.security_manager.get_user_roles")
    mock_get_user_rls = mocker.patch("zobi.security_manager.get_rls_filters")
    mock_cache_key_wrapper = mocker.patch(
        "zobi.jinja_context.ExtraCache.cache_key_wrapper"
    )
    mock_g.user.id = 1
...
def test_current_user_rls_rules_with_no_table(mocker: MockerFixture):
    ...
    mock_g = mocker.patch("zobi.utils.core.g")
    mock_get_user_rls = mocker.patch("zobi.security_manager.get_rls_filters")
    mock_is_guest_user = mocker.patch("zobi.security_manager.is_guest_user")
    mock_cache_key_wrapper = mocker.patch(
        "zobi.jinja_context.ExtraCache.cache_key_wrapper"
    )
    mock_g.user.id = 1
...
def test_current_user_rls_rules_guest_user(mocker: MockerFixture):
    ...
    """
    mock_g = mocker.patch("zobi.utils.core.g")
    mock_gg = mocker.patch("zobi.tasks.utils.g")
    mock_ggg = mocker.patch("zobi.security.manager.g")
    mock_get_user_rls = mocker.patch("zobi.security_manager.get_guest_rls_filters")
...
def test_dataset_macro(mocker: MockerFixture) -> None:
    ...
    """
    mocker.patch(
        "zobi.connectors.sqla.models.security_manager.get_guest_rls_filters",
        return_value=[],
    )
    ...
    DatasetDAO = mocker.patch("zobi.daos.dataset.DatasetDAO")  # noqa: N806
...
def test_metric_macro_with_dataset_id(mocker: MockerFixture) -> None:
    ...
    """
    mock_get_form_data = mocker.patch("zobi.views.utils.get_form_data")
    DatasetDAO = mocker.patch("zobi.daos.dataset.DatasetDAO")  # noqa: N806
...
def test_metric_macro_recursive(mocker: MockerFixture) -> None:
    ...
    mock_g = mocker.patch("zobi.jinja_context.g")
...
def test_metric_macro_expansion(mocker: MockerFixture) -> None:
    ...
    mocker.patch("zobi.jinja_context.get_user_id", return_value=42)
    mock_g = mocker.patch("zobi.jinja_context.g")
    mock_g.form_data = {"datasource": {"id": 1}}
    DatasetDAO = mocker.patch("zobi.daos.dataset.DatasetDAO")  # noqa: N806
    DatasetDAO.find_by_id.return_value = dataset
...
def test_metric_macro_recursive_cyclic(mocker: MockerFixture) -> None:
    ...
    processor = get_template_processor(database=database)
    with pytest.raises(ZobiTemplateException) as excinfo:
...
def test_metric_macro_no_dataset_id_with_context_chart_id(
    ...
    """
    ChartDAO = mocker.patch("zobi.daos.chart.ChartDAO")  # noqa: N806
...
def test_jinja2_template_syntax_error_handling(mocker: MockerFixture) -> None:
    ...
    from zobi.exceptions import ZobiSyntaxErrorException
    ...
    with pytest.raises(ZobiSyntaxErrorException) as exc_info:
    ...
    assert error.error_type == ZobiErrorType.GENERIC_COMMAND_ERROR
...
def test_jinja2_server_error_handling(mocker: MockerFixture) -> None:
    ...
    with patch.object(
        ...
        with pytest.raises(ZobiTemplateException) as exc_info:
            processor.process_template(template)
