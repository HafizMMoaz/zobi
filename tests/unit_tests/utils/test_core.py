...
def test_is_test():
    orig_value = os.getenv("ZOBI_TESTENV")

    os.environ["ZOBI_TESTENV"] = "true"
    assert is_test()
    os.environ["ZOBI_TESTENV"] = "false"
    assert not is_test()
    os.environ["ZOBI_TESTENV"] = ""
    assert not is_test()

    if orig_value is not None:
        os.environ["ZOBI_TESTENV"] = orig_value
...
def test_check_if_safe_zip_high_rate(app_context: None) -> None:
    ...
    with pytest.raises(ZobiException):
        check_is_safe_zip(ZipFile)
...
def test_generic_constraint_name_exists():
    ...
    with patch("zobi.utils.core.sa.Table") as table_creation_mock:
...
@pytest.mark.parametrize(
    ...
    [
        (None, None),
        ("https://myzobi.com/abc", None),
        ("https://myzobi.com/zobi/dashboard/", QuerySource.DASHBOARD),
        ("https://myzobi.com/explore/", QuerySource.CHART),
        ("https://myzobi.com/sqllab/", QuerySource.SQL_LAB),
    ],
)
def test_get_query_source_from_request(
    ...
    if referrer:
        ...
        with mocker.patch("flask.has_request_context", return_value=True):
            ...
            mocker.patch("zobi.utils.core.request", request_mock)
...
def test_get_user_agent(mocker: MockerFixture, app_context: None) -> None:
    ...
    assert get_user_agent(database_mock, QuerySource.DASHBOARD) == "Zobi", (
