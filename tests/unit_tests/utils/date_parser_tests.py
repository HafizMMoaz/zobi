...
from zobi.utils.date_parser import (
    DateRangeMigration,
    datetime_eval,
    get_past_or_future,
    get_since_until,
...
@patch("zobi.utils.date_parser.parse_human_datetime", mock_parse_human_datetime)
...
@patch("zobi.utils.date_parser.datetime")
...
def test_datetime_eval_does_not_emit_parsedatetime_debug_logs(
    ...
    in ``zobi/utils/date_parser.py`` (e.g.
    ``logging.getLogger("parsedatetime").setLevel(logging.WARNING)``).
    Their own DEBUG output is internal library chatter that Zobi
