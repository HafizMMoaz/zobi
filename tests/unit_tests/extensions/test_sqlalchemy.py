...
@with_feature_flags(ENABLE_ZOBI_META_DB=True)
def test_zobi(mocker: MockerFixture, app_context: None, table1: None) -> None:
    """
    Simple test querying a table.
    """
    # Mock the security_manager.raise_for_access to allow access
    mocker.patch(
        "zobi.extensions.metadb.security_manager.raise_for_access",
        return_value=None,
    )
    ...
    try:
        ...
        # Skip test if zobi:// dialect can't be loaded (common in Docker)
        pytest.skip(f"Zobi dialect not available: {e}")
...
@with_config(
    {
        ...
        "ZOBI_META_DB_LIMIT": 1,
        "DATABASE_OAUTH2_CLIENTS": {},
        "SQLALCHEMY_CUSTOM_PASSWORD_STORE": None,
    }
)
@with_feature_flags(ENABLE_ZOBI_META_DB=True)
def test_zobi_limit(mocker: MockerFixture, app_context: None, table1: None) -> None:
    """
...
def test_security_manager(
    ...
    try:
        import zobi.extensions.metadb  # noqa: F401
    except ImportError:
    ...
    mocker.patch("zobi.security.manager.g", mocker.MagicMock(user=mock_user))
    ...
    mocker.patch(
        "zobi.extensions.metadb.security_manager",
        new=security_manager,
    )
    security_manager.raise_for_access.side_effect = ZobiSecurityException(
        ZobiError(
    ...
    conn = engine.connect()
    with pytest.raises(ZobiSecurityException) as excinfo:
...
def test_allowed_dbs(mocker: MockerFixture, app_context: None, table1: None) -> None:
    ...
    try:
        engine = create_engine("zobi://", allowed_dbs=["database1"])
    except Exception as e:
        # Skip test if zobi:// dialect can't be loaded (common in Docker)
        pytest.skip(f"Zobi dialect not available: {e}")
