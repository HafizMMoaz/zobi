...
def test_query_datasources_by_name(mocker: MockerFixture) -> None:
    ...
    """
    db = mocker.patch("zobi.connectors.sqla.models.db")
...
def test_fetch_metadata_with_comment_field_new_columns(mocker: MockerFixture) -> None:
    ...
    mocker.patch("zobi.connectors.sqla.models.db.session")
    mocker.patch(
        "zobi.connectors.sqla.models.config", {"SQLA_TABLE_MUTATOR": lambda x: None}
    )
...
def test_fetch_metadata_with_comment_field_existing_columns(
    ...
    # Mock dependencies
    mock_session = mocker.patch("zobi.connectors.sqla.models.db.session")
