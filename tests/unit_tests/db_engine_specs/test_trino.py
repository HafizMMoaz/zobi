...
from zobi.db_engine_specs.exceptions import (
    ...
    ZobiDBAPIDatabaseError,
    ZobiDBAPIOperationalError,
...
@pytest.mark.parametrize(
    "extra,expected",
    [
        ({}, {"engine_params": {"connect_args": {"source": "Zobi"}}}),
        (
            {
                "first": 1,
                "engine_params": {
...
def test_get_extra_params(extra: dict[str, Any], expected: dict[str, Any]) -> None:
    from zobi.db_engine_specs.trino import TrinoEngineSpec
...
@patch("zobi.db_engine_specs.trino.create_ssl_cert_file")
...
def test_auth_custom_auth() -> None:
    ...
    with patch.dict(
        "zobi.config.ALLOWED_EXTRA_AUTHENTICATIONS",
...
def test_get_column_spec(
    ...
    from zobi.db_engine_specs.trino import TrinoEngineSpec as spec  # noqa: N813
...
@patch("zobi.db_engine_specs.presto.PrestoBaseEngineSpec.handle_cursor")
@patch("zobi.db_engine_specs.trino.TrinoEngineSpec.cancel_query")
@patch("zobi.db_engine_specs.trino.db")
@patch("zobi.db_engine_specs.trino.app")
...
def test_execute_with_cursor_in_parallel(app, mocker: MockerFixture):
    ...
    with app.test_request_context("/some/place/"):
        ...
        with patch.dict(
            "zobi.config.DISALLOWED_SQL_FUNCTIONS",
            {},
            clear=True,
        ):
...
def test_get_dbapi_exception_mapping():
    ...
    assert mapping.get(TrinoUserError) == ZobiDBAPIProgrammingError
    assert mapping.get(TrinoInternalError) == ZobiDBAPIDatabaseError
    assert mapping.get(TrinoExternalError) == ZobiDBAPIOperationalError
    assert mapping.get(RequestsConnectionError) == ZobiDBAPIConnectionError
...
@patch("zobi.db_engine_specs.trino.TrinoEngineSpec.latest_partition")
@pytest.mark.parametrize(
...
def test_get_oauth2_token(
    ...
    requests = mocker.patch("zobi.db_engine_specs.base.requests")
...
def test_needs_oauth2_with_401_error(mocker: MockerFixture) -> None:
    ...
    g = mocker.patch("zobi.db_engine_specs.trino.g")
...
def test_handle_cursor_progress_updates(
    ...
    with patch("zobi.db_engine_specs.trino.time.sleep", side_effect=update_stats):
        query = Query()
...
def test_handle_cursor_cancels_on_stopped_status(
    ...
    with patch("zobi.db_engine_specs.trino.time.sleep"):
        TrinoEngineSpec.handle_cursor(cursor=cursor_mock, query=query)
