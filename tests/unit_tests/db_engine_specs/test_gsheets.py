...
def test_validate_parameters_simple(mocker: MockerFixture) -> None:
    from zobi.db_engine_specs.gsheets import (
        GSheetsEngineSpec,
        GSheetsPropertiesType,
    )

    g = mocker.patch("zobi.db_engine_specs.gsheets.g")
...
def test_validate_parameters_catalog(
    ...
    create_engine = mocker.patch("zobi.db_engine_specs.gsheets.create_engine")
...
def test_upload_new(mocker: MockerFixture) -> None:
    ...
    mocker.patch("zobi.db_engine_specs.gsheets.db")
...
def test_upload_existing(mocker: MockerFixture) -> None:
    ...
    with pytest.raises(ZobiException) as excinfo:
...
def test_impersonate_user_username(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "zobi.db_engine_specs.gsheets.security_manager.find_user",
        return_value=user,
    )
    database = mocker.MagicMock()
...
def test_get_table_names_triggers_oauth2_dance(mocker: MockerFixture) -> None:
    ...
    get_oauth2_access_token = mocker.patch(
        "zobi.db_engine_specs.gsheets.get_oauth2_access_token",
        return_value=None,
    )
...
def test_get_table_names_does_not_trigger_oauth2_when_token_exists(
    ...
    mocker.patch(
        "zobi.db_engine_specs.shillelagh.ShillelaghEngineSpec.get_table_names",
