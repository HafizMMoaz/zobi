...
def test_convert_dttm(
    ...
    from zobi.db_engine_specs.snowflake import (
...
def test_database_connection_test_mutator() -> None:
    from zobi.db_engine_specs.snowflake import SnowflakeEngineSpec
...
def test_extract_errors() -> None:
    ...
    assert result == [
        ZobiError(
            ...
            error_type=ZobiErrorType.OBJECT_DOES_NOT_EXIST_ERROR,
...
def test_get_extra_params(mocker: MockerFixture) -> None:
    ...
    assert SnowflakeEngineSpec.get_extra_params(database) == {
        "engine_params": {"connect_args": {"application": "Zobi"}}
    }
