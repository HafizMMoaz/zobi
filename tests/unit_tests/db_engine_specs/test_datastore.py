...
def test_execute_with_cursor_no_warnings(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "zobi.db_engine_specs.base.BaseEngineSpec.execute_with_cursor",
    )
...
def test_get_client_dependencies_not_installed(mocker: MockerFixture) -> None:
    """
    Test that ``_get_client`` raises ``ZobiException`` when the
    google-cloud-datastore package is not installed.
    """
    from zobi.exceptions import ZobiException

    mocker.patch(
        "zobi.db_engine_specs.datastore.dependencies_installed",
        False,
    )
    ...
    with pytest.raises(ZobiException, match="Could not import libraries"):
        DatastoreEngineSpec._get_client(engine, database)


def test_get_client_default_credentials_error(mocker: MockerFixture) -> None:
    """
    Test that ``_get_client`` raises ``ZobiDBAPIConnectionError`` when
    google.auth.default() fails.
    """
    from google.auth.exceptions import DefaultCredentialsError

    from zobi.db_engine_specs.exceptions import ZobiDBAPIConnectionError

    mocker.patch(
        "zobi.db_engine_specs.datastore.google.auth.default",
...
def test_get_dbapi_exception_mapping() -> None:
    ...
    assert mapping[exc_cls] is ZobiDBAPIConnectionError
...
def test_get_catalog_names(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "zobi.db_engine_specs.base.BaseEngineSpec.get_catalog_names",
