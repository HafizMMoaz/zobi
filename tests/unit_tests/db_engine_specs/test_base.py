...
def create_expected_zobi_error(
    message: str,
    error_type: ZobiErrorType = ZobiErrorType.GENERIC_DB_ENGINE_ERROR,
    engine_name: str | None = None,
) -> ZobiError:
    """
    Helper function to create expected ZobiError objects for testing.
    """
    extra = {
...
def test_extra_table_metadata(mocker: MockerFixture) -> None:
    ...
    database = mocker.MagicMock()
    warnings = mocker.patch("zobi.db_engine_specs.base.warnings")
...
def test_impersonate_user_backwards_compatible(mocker: MockerFixture) -> None:
    ...
    signature = mocker.patch("zobi.db_engine_specs.base.signature")
...
def test_extract_errors(mocker: MockerFixture) -> None:
    ...
    expected = create_expected_zobi_error(
...
def test_get_oauth2_authorization_uri_with_pkce(mocker: MockerFixture) -> None:
    ...
    from zobi.utils.oauth2 import generate_code_challenge, generate_code_verifier
...
def test_get_oauth2_token_without_pkce(mocker: MockerFixture) -> None:
    ...
    mock_post = mocker.patch("zobi.db_engine_specs.base.requests.post")
...
def test_get_oauth2_token_with_pkce(mocker: MockerFixture) -> None:
    ...
    """
    from zobi.db_engine_specs.base import BaseEngineSpec
    from zobi.utils.oauth2 import generate_code_verifier
...
def test_get_oauth2_fresh_token_raises_on_auth_error(
    ...
    """
    from zobi.db_engine_specs.base import BaseEngineSpec
    from zobi.exceptions import OAuth2TokenRefreshError
...
def test_start_oauth2_dance_uses_config_redirect_uri(mocker: MockerFixture) -> None:
    ...
    mocker.patch("zobi.daos.key_value.KeyValueDAO")
    mocker.patch("zobi.db_engine_specs.base.db")
