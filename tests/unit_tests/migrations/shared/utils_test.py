...
def test_create_index_skips_if_index_exists(monkeypatch):
    ...
    monkeypatch.setattr("zobi.migrations.shared.utils.logger", dummy_logger)
    monkeypatch.setattr("zobi.migrations.shared.utils.op", dummy_op)
    monkeypatch.setattr(
        "zobi.migrations.shared.utils.table_has_index", fake_table_has_index_true
    )
...
def test_create_index_creates_index(monkeypatch):
    ...
    monkeypatch.setattr("zobi.migrations.shared.utils.op", dummy_op)
    monkeypatch.setattr(
        "zobi.migrations.shared.utils.table_has_index", fake_table_has_index_false
    )
