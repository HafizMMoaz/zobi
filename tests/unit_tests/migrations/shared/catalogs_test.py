...
def test_upgrade_catalog_perms(mocker: MockerFixture, session: Session) -> None:
    ...
    from zobi.models.sql_lab import Query, SavedQuery, TableSchema, TabState

    engine = session.get_bind()
    Database.metadata.create_all(engine)

    mocker.patch("zobi.migrations.shared.catalogs.op")
    db = mocker.patch("zobi.migrations.shared.catalogs.db")
...
def test_upgrade_catalog_perms_graceful(
    ...
    mocker.patch("zobi.migrations.shared.catalogs.op", session)
...
def test_upgrade_catalog_perms_oauth_connection(
    ...
    add_non_default_catalogs = mocker.patch(
        "zobi.migrations.shared.catalogs.add_non_default_catalogs"
    )
    mocker.patch("zobi.migrations.shared.catalogs.op", session)
