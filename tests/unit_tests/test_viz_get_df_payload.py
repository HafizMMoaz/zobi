...
def test_get_df_payload_propagates_oauth2_redirect_error() -> None:
    """
    OAuth2RedirectError (a ZobiErrorException) must propagate out of
    ...
    assert exc_info.value.error.error_type == ZobiErrorType.OAUTH2_REDIRECT
...
def test_get_df_payload_captures_generic_exception_as_viz_get_df_error() -> None:
    """
    Non-Zobi exception raised by ``get_df`` are downgraded to a
    ...
    assert obj.errors[0]["error_type"] == ZobiErrorType.VIZ_GET_DF_ERROR
