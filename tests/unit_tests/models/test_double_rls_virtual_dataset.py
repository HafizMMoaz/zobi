...
def test_rls_filters_include_guest_when_enabled(
    ...
    When include_global_guest_rls=True and EMBEDDED_ZOBI is enabled,
    ...
    with (
        patch(
            "zobi.connectors.sqla.models.security_manager.get_rls_filters",
        ...
        patch(
            "zobi.connectors.sqla.models.is_feature_enabled",
            return_value=True,
        ),
    ):
...
def test_guest_rls_skipped_when_feature_disabled(
    ...
    """
    Test that guest RLS is skipped when EMBEDDED_ZOBI is disabled.
