...
def test_get_metrics(mocker: MockerFixture) -> None:
    ...
    """
    from zobi.db_engine_specs.base import MetricType
...
def test_get_db_engine_spec(mocker: MockerFixture) -> None:
    ...
    load_engine_specs = mocker.patch("zobi.db_engine_specs.load_engine_specs")
...
def test_get_all_schema_names_needs_oauth2(mocker: MockerFixture) -> None:
    ...
    user.id = 42
    mocker.patch("zobi.db_engine_specs.base.g", user=user)
    ...
    assert excinfo.value.error.error_type == ZobiErrorType.OAUTH2_REDIRECT
...
def test_get_sqla_engine(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "zobi.models.core.security_manager.find_user",
        return_value=user,
    )
    mocker.patch("zobi.models.core.get_username", return_value="alice")

    create_engine = mocker.patch("zobi.models.core.create_engine")
...
def test_get_sqla_engine_user_impersonation(mocker: MockerFixture) -> None:
    ...
    create_engine.assert_called_with(
        make_url("trino:///"),
        connect_args={"user": "alice", "source": "Zobi"},
    )
...
def test_get_sqla_engine_user_impersonation_email(mocker: MockerFixture) -> None:
    ...
    create_engine.assert_called_with(
        make_url("trino:///"),
        connect_args={"user": "alice.doe", "source": "Zobi"},
    )
...
def test_get_sqla_engine_registers_prequery_event_listener(
    ...
    event_listen = mocker.patch("zobi.models.core.sqla.event.listen")
...
def test_get_raw_connection_executes_prequeries_exactly_once(
    ...
    original_listen = mocker.patch("zobi.models.core.sqla.event.listen")
...
def test_get_all_table_names_in_schema(mocker: MockerFixture) -> None:
    ...
    get_table_names = mocker.patch(
        "zobi.db_engine_specs.postgres.PostgresEngineSpec.get_table_names"
    )
...
def test_get_all_view_names_in_schema(mocker: MockerFixture) -> None:
    ...
    get_view_names = mocker.patch(
        "zobi.db_engine_specs.base.BaseEngineSpec.get_view_names"
    )
...
def test_database_execute_delegates_to_sql_executor(mocker: MockerFixture) -> None:
    ...
    mock_executor_class = mocker.patch("zobi.sql.execution.SQLExecutor")
