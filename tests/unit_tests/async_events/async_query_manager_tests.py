...
from zobi.async_events.async_query_manager import (
    AsyncQueryManager,
...
JWT_TOKEN_COOKIE_NAME = "zobi_async_jwt"  # noqa: S105
...
def test_parse_channel_id_from_request(async_query_manager):
    ...
    request = Mock()
    request.cookies = {"zobi_async_jwt": encoded_token}
...
def test_parse_channel_id_from_request_bad_jwt(async_query_manager):
    request = Mock()
    request.cookies = {"zobi_async_jwt": "bad_jwt"}
...
@mock.patch("zobi.is_feature_enabled")
