...
from zobi.semantic_layers.api import SemanticLayerRestApi, SemanticViewRestApi
...
def test_put_semantic_view(
    ...
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.UpdateSemanticViewCommand",
    )
...
def test_get_types(
    ...
    mocker.patch.dict(
        "zobi.semantic_layers.api.registry",
        {"snowflake": mock_cls},
        clear=True,
    )
...
def test_runtime_schema(
    ...
    mock_dao = mocker.patch("zobi.semantic_layers.api.SemanticLayerDAO")
...
def test_post_semantic_layer(
    ...
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.CreateSemanticLayerCommand",
    )
...
def test_put_semantic_layer(
    ...
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.UpdateSemanticLayerCommand",
    )
    mock_command.return_value.run.return_value = changed_model
...
def test_delete_semantic_layer(
    ...
    test_uuid = str(uuid_lib.uuid4())
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.DeleteSemanticLayerCommand",
    )
...
def test_infer_discriminators_injects_discriminator() -> None:
    ...
    from zobi.semantic_layers.api import _infer_discriminators
...
def test_parse_partial_config_strict_success() -> None:
    ...
    from zobi.semantic_layers.api import _parse_partial_config
...
def test_connections_list(
    ...
    mock_db_session = mocker.patch("zobi.semantic_layers.api.db.session")
    db_query = MagicMock()
    ...
    mocker.patch(
        "zobi.semantic_layers.api.is_feature_enabled",
        return_value=True,
    )
...
def test_connections_semantic_layer_filters_by_perms(
    ...
    mocker.patch(
        "zobi.semantic_layers.api.security_manager.can_access_all_datasources",
        return_value=False,
    )
    mocker.patch(
        "zobi.semantic_layers.api.security_manager.user_view_menu_names",
...
def test_post_semantic_view_bulk_create(
    ...
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.CreateSemanticViewCommand",
    )
    mock_command.return_value.run.return_value = new_model
...
def test_delete_semantic_view(
    ...
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.DeleteSemanticViewCommand",
    )
    mock_command.return_value.run.return_value = None
...
def test_bulk_delete_semantic_view(
    ...
    mock_command = mocker.patch(
        "zobi.semantic_layers.api.BulkDeleteSemanticViewCommand",
    )
    mock_command.return_value.run.return_value = None
...
def test_get_semantic_view_structure_forbidden(
    ...
    mock_view.raise_for_access.side_effect = ZobiSecurityException(access_error)
