...
from zobi.connectors.sqla.models import Database, SqlaTable
...
def test_raise_for_access_query_default_schema(
    ...
    SqlaTable = mocker.patch("zobi.connectors.sqla.models.SqlaTable")  # noqa: N806
...
def test_get_rls_filters_uses_table_id_directly(
    ...
    mocker.patch("zobi.security.manager.g", user=mock_user)
...
def test_get_rls_filters_returns_cached_result(
    ...
    mocker.patch("zobi.security.manager.g", new=mock_g)
    mocker.patch("zobi.security.manager.get_username", return_value="admin")
    mocker.patch.object(sm, "get_user_roles", return_value=mock_user.roles)
...
def test_prefetch_rls_filters_no_user(
    ...
    """
    sm = ZobiSecurityManager(appbuilder)
    mocker.patch("zobi.security.manager.g", new=SimpleNamespace())
...
def test_get_rls_filters_cache_works_for_guest_user(
    ...
    mocker.patch("zobi.security.manager.g", new=mock_g)
    mocker.patch("zobi.security.manager.get_username", return_value="guest_user")
