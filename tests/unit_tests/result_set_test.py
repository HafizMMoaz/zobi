...
from zobi.result_set import stringify_values, ZobiResultSet
from zobi.zobi_typing import DbapiResult
...
def test_column_names_as_bytes() -> None:
    ...
    result_set = ZobiResultSet(data, description, RedshiftEngineSpec)  # type: ignore
...
def test_timezone_series(mocker: MockerFixture) -> None:
    ...
    """
    logger = mocker.patch("zobi.result_set.logger")
...
def test_empty_column_names_get_synthetic_names() -> None:
    ...
    ZobiResultSet must replace empty column names with synthetic names
    so queries like ``SELECT COUNT(*) FROM t`` succeed on MSSQL.

    Regression test for https://github.com/HafizMMoaz/zobi/issues/23848
    """
    data = [(42,)]
    description = [("", 3, None, None, None, None, None)]
    result_set = ZobiResultSet(data, description, BaseEngineSpec)  # type: ignore
...
def test_empty_column_names_do_not_rename_explicit_synthetic_names() -> None:
    ...
    user-selected names that already look like Zobi fallbacks.
    """
    data = [(10, 20)]
    description = [
        ("", 3, None, None, None, None, None),
