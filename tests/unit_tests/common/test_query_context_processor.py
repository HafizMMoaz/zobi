...
from zobi.common.query_context_processor import QueryContextProcessor
...
def mock_query_context():
    with patch(
        "zobi.common.query_context_processor.QueryContextProcessor"
...
def processor(mock_query_context):
    from zobi.models.helpers import ExploreMixin
...
@patch("zobi.common.query_context_processor.csv.df_to_escaped_csv")
...
@patch("zobi.common.query_context_processor.excel.df_to_excel")
@patch("zobi.common.query_context_processor.excel.apply_column_types")
...
@patch("zobi.common.query_context_processor.excel.df_to_excel")
@patch(
    "zobi.common.query_context_processor.excel.apply_column_types",
...
def test_join_offset_dfs_date_range_basic(processor):
    ...
    with patch("zobi.models.helpers.feature_flag_manager") as mock_ff:
        mock_ff.is_feature_enabled.return_value = True
        with patch("zobi.common.utils.dataframe_utils.left_join_df") as mock_join:
...
def test_processing_time_offsets_temporal_column_error(processor):
    ...
    from zobi.exceptions import QueryObjectValidationError
    ...
    with patch(
        "zobi.common.utils.time_range_utils.get_since_until_from_query_object"
    ) as mock_dates:
...
def test_processing_time_offsets_date_range_enabled(processor):
    ...
    with patch("zobi.models.helpers.feature_flag_manager") as mock_ff:
        ...
        with patch(
            "zobi.utils.core.get_base_axis_labels",
            ...
            with patch(
                ...
                with patch(
                    "zobi.common.utils.time_range_utils.get_since_until_from_time_range"
                ) as mock_time_range:
...
def test_processing_time_offsets_uses_chart_row_limit(processor):
    ...
    with (
        patch(
            "zobi.models.helpers.get_since_until_from_query_object",
            return_value=(pd.Timestamp("1990-01-01"), pd.Timestamp("1991-01-01")),
        ),
        patch(
            "zobi.common.utils.query_cache_manager.QueryCacheManager"
...
def test_processing_time_offsets_falls_back_to_config_row_limit(processor):
    ...
    with (
        ...
        patch("zobi.models.helpers.app") as mock_app,
    ):
...
def test_ensure_totals_available_updates_cache_values():
    ...
    with patch.object(
        ...
        with patch(
            "zobi.common.query_context_processor.get_query_results"
            ...
            with patch(
                "zobi.common.query_context_processor.QueryCacheManager"
            ) as mock_cache_manager:
                mock_cache = MagicMock()
...
def test_force_cached_normalizes_totals_query_row_limit():
    ...
    with patch(
        "zobi.common.query_context_processor.security_manager"
