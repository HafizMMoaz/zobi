...
def test_remove_user_favorite_tag(mocker):
    ...
    # Mock the behavior of TagDAO and g
    mock_TagDAO = mocker.patch("zobi.daos.tag.TagDAO")  # noqa: N806
...
def test_remove_user_favorite_tag_no_user(mocker):
    from zobi.daos.tag import TagDAO
    from zobi.exceptions import MissingUserContextException

    # Mock the behavior of TagDAO and g
    mocker.patch("zobi.daos.tag.db.session")  # noqa: F841
    mock_TagDAO = mocker.patch("zobi.daos.tag.TagDAO")  # noqa: N806
    mock_tag = mocker.MagicMock(users_favorited=[])
    mock_TagDAO.find_by_id.return_value = mock_tag
...
def test_remove_user_favorite_tag_exc_raise(mocker):
    ...
    # Mock the behavior of TagDAO and g
    mock_session = mocker.patch("zobi.daos.tag.db.session")
    mock_TagDAO = mocker.patch("zobi.daos.tag.TagDAO")  # noqa: N806
    mock_tag = mocker.MagicMock(users_favorited=[])
    mock_TagDAO.find_by_id.return_value = mock_tag
...
def test_create_tag_relationship(mocker):
    from zobi.daos.tag import TagDAO
    from zobi.tags.models import (  # Assuming these are defined in the same module
        ObjectType,
    )
