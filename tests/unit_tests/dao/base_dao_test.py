...
from zobi.daos.base import BaseDAO, ColumnOperatorEnum
from zobi.daos.exceptions import DAOFindFailedError
...
def test_find_by_ids_sqlalchemy_error_with_model_cls():
    ...
    with (
        patch("zobi.daos.base.db") as mock_db,
        patch("zobi.daos.base.getattr") as mock_getattr,
    ):
        mock_session = Mock()
...
def test_find_by_ids_empty_list():
    ...
    with patch("zobi.daos.base.getattr") as mock_getattr:
