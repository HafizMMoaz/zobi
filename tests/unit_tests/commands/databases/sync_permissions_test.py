...
def test_sync_permissions_command_sync_mode(
    ...
    mocker.patch("zobi.commands.database.sync_permissions.ping", return_value=True)
    find_pvm_mock = mocker.patch(
        "zobi.commands.database.sync_permissions.security_manager.find_permission_view_menu"
    )
    find_pvm_mock.side_effect = [mocker.MagicMock(), None]
    add_pvm_mock = mocker.patch("zobi.commands.database.sync_permissions.add_pvm")
...
def test_sync_permissions_command_async_mode(
    ...
    async_task_mock = mocker.patch(
        "zobi.commands.database.sync_permissions.sync_database_permissions_task"
    )
    mocker.patch("zobi.commands.database.sync_permissions.ping", return_value=True)
...
def test_sync_permissions_command_raise(
    ...
    mock_ping = mocker.patch(
        "zobi.commands.database.sync_permissions.ping", return_value=False
    )
...
def test_sync_permissions_command_rename_db_in_perms(
    ...
    mock_dataset_dao = mocker.patch(
        "zobi.commands.database.sync_permissions.DatasetDAO"
    )
