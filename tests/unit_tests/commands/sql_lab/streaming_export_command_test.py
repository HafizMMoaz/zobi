...
def _setup_sqllab_mocks(
    ...
    mock_db_base = mocker.patch("zobi.commands.streaming_export.base.db")
...
@patch("zobi.commands.sql_lab.streaming_export_command.db")
...
def test_validate_access_denied(mock_db, mock_query):
    ...
    mock_query_result = mock_db.session.query.return_value.filter_by.return_value
    mock_query_result.one_or_none.return_value = mock_query
    mock_query.raise_for_access.side_effect = ZobiSecurityException(
...
@patch("zobi.commands.sql_lab.streaming_export_command.SQLScript")
...
def test_limiting_factor_dropdown(mocker, mock_query):
    ...
    with patch(
        "zobi.commands.sql_lab.streaming_export_command.SQLScript"
    ) as mock_sqlscript:
        mock_statement = Mock()
...
@patch("zobi.commands.streaming_export.base.logger")
