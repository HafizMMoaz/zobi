...
from zobi.daos.report import REPORT_SCHEDULE_ERROR_NOTIFICATION_MARKER
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ["url1", "url2"],
            [
                "zobi/dashboard/p/url1/",
                "zobi/dashboard/p/url2/",
            ],
        ),
...
def test_get_dashboard_urls_with_exporting_dashboard_only(
    ...
    base_url = app.config.get("WEBDRIVER_BASEURL", "http://0.0.0.0:8080/")
    expected_url = urllib.parse.urljoin(base_url, "zobi/dashboard/p/url1/")
    assert expected_url == result[0]


@patch("zobi.commands.report.execute.CreateDashboardPermalinkCommand")
@with_feature_flags(ALERT_REPORT_TABS=True)
def test_get_dashboard_urls_with_filters_and_tabs(
    ...
    base_url = app.config.get("WEBDRIVER_BASEURL", "http://0.0.0.0:8080/")
    assert result == [
        urllib.parse.urljoin(base_url, "zobi/dashboard/p/key1/"),
        urllib.parse.urljoin(base_url, "zobi/dashboard/p/key2/"),
    ]
...
def test_get_tab_urls(
    ...
    base_url = app.config.get("WEBDRIVER_BASEURL", "http://0.0.0.0:8080/")
    assert result == [
        urllib.parse.urljoin(base_url, "zobi/dashboard/p/uri1/"),
        urllib.parse.urljoin(base_url, "zobi/dashboard/p/uri2/"),
    ]
...
def test_screenshot_width_calculation(
    ...
    """
    from zobi.commands.report.execute import BaseReportState
    ...
    with (
        ...
        with patch("zobi.commands.report.execute.get_executor") as mock_get_executor:
            ...
            with patch(
                "zobi.commands.report.execute.ChartScreenshot",
...
def test_update_recipient_to_slack_v2(mocker: MockerFixture):
    ...
    """
    mocker.patch(
        "zobi.commands.report.execute.get_channels_with_search",
        return_value=[
            {
                "id": "abc124f",
                "name": "channel-1",
...
def test_create_log_stale_data_raises_unexpected_error(mocker: MockerFixture) -> None:
    ...
    mock_db = mocker.patch("zobi.commands.report.execute.db")
    ...
    mocker.patch(
        "zobi.commands.report.execute.ReportExecutionLog",
...
@patch("zobi.commands.report.execute.feature_flag_manager")
...
def test_working_state_timeout_raises_timeout_error(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "zobi.commands.report.execute.ReportScheduleDAO.find_last_entered_working_log",
...
def test_get_dashboard_urls_no_state_fallback(
    mocker: MockerFixture, app: ZobiApp
) -> None:
    ...
    assert len(result) == 1
    assert "zobi/dashboard/" in result[0]
...
def test_success_state_alert_command_error_sends_error_and_reraises(
    ...
    mocker.patch(
        "zobi.commands.report.execute.AlertCommand"
