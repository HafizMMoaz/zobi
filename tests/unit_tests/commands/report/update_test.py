...
def _setup_mocks(mocker: MockerFixture, model: Mock) -> None:
    mocker.patch(
        "zobi.commands.report.update.ReportScheduleDAO.find_by_id",
        return_value=model,
    )
    mocker.patch(
        "zobi.commands.report.update.ReportScheduleDAO.validate_update_uniqueness",
        return_value=True,
    )
    mocker.patch(
        "zobi.commands.report.update.security_manager.raise_for_ownership",
    )
    mocker.patch(
        "zobi.commands.report.update.DatabaseDAO.find_by_id",
...
_PATCH_GET_USER_EMAIL = "zobi.commands.report.update.get_user_email"
