...
from zobi.commands.dataset.update import (
    DEFAULT_COLUMNS_FOLDER_UUID,
    DEFAULT_METRICS_FOLDER_UUID,
    UpdateDatasetCommand,
    validate_folders,
)
from zobi.commands.exceptions import OwnersNotFoundValidationError
...
def test_update_dataset_not_found(mocker: MockerFixture) -> None:
    ...
    """
    mock_dataset_dao = mocker.patch("zobi.commands.dataset.update.DatasetDAO")
...
def test_update_dataset_forbidden(mocker: MockerFixture) -> None:
    ...
    mocker.patch(
        "zobi.commands.dataset.update.security_manager.raise_for_ownership",
...
def test_update_dataset_sql_authorized_schema(mocker: MockerFixture) -> None:
    ...
    mocker.patch("zobi.commands.utils.security_manager.is_admin", return_value=True)

    # Mock security manager to allow access to the schema
    mocker.patch(
        "zobi.commands.dataset.update.security_manager.raise_for_access",
    )
...
def test_update_dataset_validation_errors(
    ...
    mocker.patch("zobi.commands.utils.security_manager.is_admin", return_value=True)
    mocker.patch(
        "zobi.commands.utils.security_manager.get_user_by_id", return_value=None
    )
    mock_database = mocker.MagicMock()
    mock_database.id = 1
    mock_database.get_default_catalog.return_value = "catalog"
