...
def test_update_semantic_view_success(mocker: MockerFixture) -> None:
    ...
    dao = mocker.patch(
        "zobi.commands.semantic_layer.update.SemanticViewDAO",
    )
    dao.find_by_id.return_value = mock_model
    dao.update.return_value = mock_model

    mocker.patch(
        "zobi.commands.semantic_layer.update.security_manager",
    )
...
def test_update_semantic_layer_success(mocker: MockerFixture) -> None:
    ...
    dao = mocker.patch(
        "zobi.commands.semantic_layer.update.SemanticLayerDAO",
    )
...
def test_update_semantic_layer_validates_configuration(
    ...
    mock_cls = MagicMock()
    mocker.patch.dict(
        "zobi.commands.semantic_layer.update.registry",
        {"snowflake": mock_cls},
    )
