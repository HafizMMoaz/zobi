...
from zobi.commands.importers.v1.examples import transpile_virtual_dataset_sql
from zobi.examples.utils import _normalize_dataset_schema
...
@patch("zobi.commands.importers.v1.examples.db")
...
@patch("zobi.commands.importers.v1.examples.db")
@patch("zobi.commands.importers.v1.examples.transpile_to_dialect")
...
def test_transpile_virtual_dataset_sql_error_fallback(mock_transpile, mock_db):
    ...
    from zobi.exceptions import QueryClauseValidationException
...
@patch("zobi.commands.importers.v1.examples.import_database")
def test_import_passes_ignore_permissions_to_all_importers(
    ...
    """
    from zobi.commands.importers.v1.examples import ImportExamplesCommand
    ...
    with patch(
        "zobi.commands.importers.v1.examples.get_example_default_schema",
        return_value=None,
    ):
        with patch(
            "zobi.commands.importers.v1.examples.find_chart_uuids",
            return_value=[],
        ):
            with patch(
                "zobi.commands.importers.v1.examples.update_id_refs",
