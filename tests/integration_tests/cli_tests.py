...
def test_export_dashboards_versioned_export(app_context, fs):
    ...
    import zobi.cli.importexport  # noqa: F811

    # reload to define export_dashboards correctly based on the
    # feature flags
    importlib.reload(zobi.cli.importexport)

    runner = current_app.test_cli_runner()
    with freeze_time("2021-01-01T00:00:00Z"):
        response = runner.invoke(zobi.cli.importexport.export_dashboards, ())
...
@mock.patch(
    "zobi.commands.dashboard.export.ExportDashboardsCommand.run",
...
def test_export_datasources_versioned_export(app_context, fs):
    ...
    runner = current_app.test_cli_runner()
    with freeze_time("2021-01-01T00:00:00Z"):
        response = runner.invoke(zobi.cli.importexport.export_datasources, ())
...
@mock.patch(
    "zobi.commands.dashboard.export.ExportDatasetsCommand.run",
    side_effect=Exception(),
)
...
@mock.patch("zobi.commands.dashboard.importers.dispatcher.ImportDashboardsCommand")
def test_import_dashboards_versioned_export(import_dashboards_command, app_context, fs):
    ...
    response = runner.invoke(
        zobi.cli.importexport.import_dashboards,
...
@mock.patch(
    "zobi.commands.dashboard.importers.dispatcher.ImportDashboardsCommand.run",
    side_effect=Exception(),
)
...
@mock.patch("zobi.commands.dataset.importers.dispatcher.ImportDatasetsCommand")
def test_import_datasets_versioned_export(import_datasets_command, app_context, fs):
    ...
    runner = current_app.test_cli_runner()
    response = runner.invoke(
        zobi.cli.importexport.import_datasources, ("-p", "datasets.yaml")
    )
    ...
    runner = current_app.test_cli_runner()
    response = runner.invoke(
        zobi.cli.importexport.import_datasources, ("-p", "datasets.zip")
    )
...
@mock.patch(
    "zobi.commands.dataset.importers.dispatcher.ImportDatasetsCommand.run",
    side_effect=Exception(),
)
...
@pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
@mock.patch("zobi.tasks.thumbnails.cache_dashboard_thumbnail")
def test_compute_thumbnails(thumbnail_mock, app_context, fs):
    ...
    response = runner.invoke(
        zobi.cli.thumbnails.compute_thumbnails,
        ["-d", "-i", dashboard.id],
    )
...
def test_re_encrypt_secrets_without_previous_key_is_noop(app_context):
    ...
    runner = current_app.test_cli_runner()
    with mock.patch.object(zobi.cli.update.SecretsMigrator, "run") as run_mock:
        response = runner.invoke(zobi.cli.update.re_encrypt_secrets, [])
...
def test_re_encrypt_secrets_failure_exits_nonzero(app_context):
    ...
    with mock.patch.object(
        zobi.cli.update.SecretsMigrator,
        "run",
        side_effect=Exception("Re-encryption failed for 2 value(s)"),
    ):
        response = runner.invoke(
            zobi.cli.update.re_encrypt_secrets,
