...
from zobi.commands.database.tables import TablesDatabaseCommand
...
from zobi.databases.schemas import DatabaseTestConnectionSchema  # noqa: F401
...
class TestCreateDatabaseCommand(ZobiTestCase):
    @patch("zobi.commands.database.test_connection.event_logger.log_with_context")
...
class TestExportDatabasesCommand(ZobiTestCase):
    @skip("Flaky")
...
class TestImportDatabasesCommand(ZobiTestCase):
    @patch("zobi.security.manager.g")
    @patch("zobi.commands.database.importers.v1.utils.add_permissions")
    ...
    def test_import_v1_database_broken_csv_fields(self, mock_add_permissions, mock_g):
        ...
        https://github.com/HafizMMoaz/zobi/pull/16756 renamed some fields, changing
    ...
    @patch("zobi.databases.schemas.is_feature_enabled")
    @patch("zobi.commands.database.importers.v1.utils.add_permissions")
    ...
    @patch("zobi.commands.database.importers.v1.import_dataset")
    @patch("zobi.commands.database.importers.v1.utils.add_permissions")
...
class TestTestConnectionDatabaseCommand(ZobiTestCase):
    ...
    def test_connection_do_ping_timeout(
        ...
        database = get_example_database()
        mock_g.user = security_manager.find_user("admin")
        mock_func_timeout.side_effect = ZobiTimeoutException(
            error_type=ZobiErrorType.BACKEND_TIMEOUT_ERROR,
            message="ERROR",
        ...
        with pytest.raises(ZobiTimeoutException) as excinfo:
            command_without_db_name.run()
    ...
    @patch("zobi.commands.database.test_connection.event_logger.log_with_context")
    @patch("zobi.utils.core.g")
    def test_connection_zobi_security_connection(
        self, mock_g, mock_event_logger, mock_get_sqla_engine
    ):
        ...
        database = get_example_database()
        mock_g.user = security_manager.find_user("admin")
        mock_get_sqla_engine.side_effect = ZobiSecurityException(
            ZobiError(error_type=500, message="test", level="info")
        )
        db_uri = database.sqlalchemy_uri_decrypted
        json_payload = {"sqlalchemy_uri": db_uri}
        command_without_db_name = TestConnectionDatabaseCommand(json_payload)
    ...
    def test_connection_db_api_exc(
        ...
        with pytest.raises(ZobiErrorsException) as excinfo:  # noqa: PT012
            command_without_db_name.run()
            assert str(excinfo.value) == (
...
@patch("zobi.db_engine_specs.base.is_hostname_valid")
@patch("zobi.db_engine_specs.base.is_port_open")
@patch("zobi.commands.database.validate.DatabaseDAO")
def test_validate(
...
class TestTablesDatabaseCommand(ZobiTestCase):
    @patch("zobi.daos.database.DatabaseDAO.find_by_id")
    ...
    @patch("zobi.daos.database.DatabaseDAO.find_by_id")
    @patch("zobi.security.manager.ZobiSecurityManager.can_access_database")
    @patch("zobi.utils.core.g")
    def test_database_tables_zobi_exception(
        ...
        mock_can_access_database.side_effect = ZobiException("Test Error")
        mock_g.user = security_manager.find_user("admin")

        command = TablesDatabaseCommand(database.id, None, "main", False)
        with pytest.raises(ZobiException) as excinfo:  # noqa: PT012
            command.run()
