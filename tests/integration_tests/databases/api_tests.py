...
from zobi.commands.database.exceptions import MissingOAuth2TokenError
from zobi.connectors.sqla.models import SqlaTable
from zobi.databases.ssh_tunnel.models import SSHTunnel
from zobi.databases.utils import make_url_safe  # noqa: F401
...
from zobi.db_engine_specs.hana import HanaEngineSpec
from zobi.errors import ZobiError
from zobi.models.core import Database, ConfigurationMethod
...
class TestDatabaseApi(ZobiTestCase):
    def insert_database(
        self,
        database_name: str,
        sqlalchemy_uri: str,
    ...
    @mock.patch(
        "zobi.commands.database.test_connection.TestConnectionDatabaseCommand.run",
    )
    @mock.patch("zobi.models.core.Database.get_all_catalog_names")
    @mock.patch("zobi.models.core.Database.get_all_schema_names")
    ...
    @mock.patch("zobi.models.core.Database.get_all_catalog_names")
    @mock.patch("zobi.models.core.Database.get_all_schema_names")
    @mock.patch("zobi.extensions.db.session.rollback")
    ...
    def test_create_database_conn_fail(self):
        ...
        response = self.client.post(uri, json=database_data)
        response_data = json.loads(response.data.decode("utf-8"))
        zobi_error_mysql = ZobiError(
            message='Either the username "zobi" or the password is incorrect.',
        ...
        zobi_error_postgres = ZobiError(
            message='The password provided for username "zobi" is incorrect.',
        ...
        expected_response_mysql = {"errors": [dataclasses.asdict(zobi_error_mysql)]}
        expected_response_postgres = {
            "errors": [dataclasses.asdict(zobi_error_postgres)]
        }
    ...
    def test_database_tables(self):
        ...
        if database.backend == "postgresql":
            ...
            assert response["count"] >= 40  # Core zobi tables
    ...
    @mock.patch("zobi.utils.log.logger")
    @mock.patch("zobi.security.manager.ZobiSecurityManager.can_access_database")
    @mock.patch("zobi.models.core.Database.get_all_table_names_in_schema")
            ...
            "errors": [
                {
                    ...
                    "extra": {
                        "issue_codes": [
                            {
                                "code": 1010,
                                "message": "Issue 1010 - Zobi encountered an error while running a command.",  # noqa: E501
                            }
                        ]
                    },
                }
    ...
    def test_test_connection_unsafe_uri(self):
        ...
        data = {
            "sqlalchemy_uri": "sqlite:///home/zobi/unsafe.db",
    ...
    @mock.patch(
        "zobi.commands.database.test_connection.DatabaseDAO.build_db_for_connection_test",
    )
    @mock.patch(
        "zobi.commands.database.test_connection.event_logger",
    )
    def test_test_connection_failed_invalid_hostname(
        ...
        assert rv.headers["Content-Type"] == "application/json; charset=utf-8"
        response = json.loads(rv.data.decode("utf-8"))
        expected_response = {"errors": [dataclasses.asdict(zobi_error)]}
        assert response == expected_response
    ...
    @mock.patch("zobi.commands.database.importers.v1.utils.add_permissions")
    ...
    @mock.patch("zobi.databases.schemas.is_feature_enabled")
    @mock.patch("zobi.commands.database.importers.v1.utils.add_permissions")
                        ...
                        "issue_codes": [
                            {
                                "code": 1010,
                                "message": (
                                    "Issue 1010 - Zobi encountered an "
                        ...
                        "issue_codes": [
                            {
                                "code": 1010,
                                "message": (
                                    "Issue 1010 - Zobi encountered an error while "
                                    "running a command."
                                ),
                            }
                        ],
    ...
    @mock.patch(
        "zobi.db_engine_specs.base.BaseEngineSpec.get_function_names",
    )
    ...
    @mock.patch("zobi.databases.api.get_available_engine_specs")
    ...
    @mock.patch("zobi.db_engine_specs.base.is_hostname_valid")
    @mock.patch("zobi.db_engine_specs.base.is_port_open")
    @mock.patch("zobi.databases.api.ValidateDatabaseParametersCommand")
    ...
    @mock.patch.dict(
        "zobi.config.SQL_VALIDATORS_BY_ENGINE",
        SQL_VALIDATORS_BY_ENGINE,
        clear=True,
    )
    ...
    @mock.patch("zobi.commands.database.validate_sql.get_validator_by_name")
    @mock.patch.dict(
        "zobi.config.SQL_VALIDATORS_BY_ENGINE",
        PRESTO_SQL_VALIDATORS_BY_ENGINE,
        clear=True,
    ...
    @with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
    @mock.patch("zobi.commands.database.sync_permissions.DatabaseDAO.find_by_id")
    ...
    @with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": False})
    @mock.patch("zobi.commands.database.sync_permissions.ping")
    ...
    @mock.patch(
        "zobi.commands.database.sync_permissions.sync_database_permissions_task.delay"
    )
    ...
    @with_config({"SYNC_DB_PERMISSIONS_IN_ASYNC_MODE": True})
    @mock.patch(
        "zobi.commands.database.sync_permissions.security_manager.get_user_by_username"
    )
