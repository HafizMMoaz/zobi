...
def post_assert_metric(
    ...
    :param client: test client for zobi api requests
    ...
    """
    with patch.object(
        BaseZobiModelRestApi, "incr_stats", return_value=None
    ) as mock_method:
...
class ZobiTestCase(TestCase):
    ...
    def get_birth_names_dataset() -> SqlaTable:
        return ZobiTestCase.get_table(name="birth_names")
    ...
    def get_table(
        ...
        return (
            ...
            .filter_by(
                ...
                or ZobiTestCase.get_database_by_name("examples").id,
                schema=schema,
                table_name=name,
            )
            .one()
    ...
    def run_sql(
        ...
        if username:
            self.logout()
            self.login(username)
        dbid = ZobiTestCase.get_database_by_name(database_name).id
        json_payload = {
            "database_id": dbid,
            "sql": sql,
            "client_id": client_id,
