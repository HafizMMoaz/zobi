...
class TestRowLevelSecurity(ZobiTestCase):
    """
...
class TestRowLevelSecurityCreateAPI(ZobiTestCase):
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
...
class TestRowLevelSecurityUpdateAPI(ZobiTestCase):
...
class TestRowLevelSecurityDeleteAPI(ZobiTestCase):
    def test_invalid_id_failure(self):
        self.login(ADMIN_USERNAME)
...
class TestRowLevelSecurityWithRelatedAPI(ZobiTestCase):
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    @mock.patch(
        "zobi.row_level_security.api.RLSRestApi.base_related_field_filters",
...
class GuestTokenRowLevelSecurityTests(ZobiTestCase):
    query_obj: dict[str, Any] = dict(  # noqa: C408
        groupby=[],
        metrics=None,
        filter=[],
