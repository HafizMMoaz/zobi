...
from zobi.exceptions import CertificateException, ZobiException  # noqa: F401
from zobi.models.core import Database, Log
from zobi.models.dashboard import Dashboard  # noqa: F401
...
from zobi.utils import schema
from zobi.utils.hashing import hash_from_str
from zobi.views.utils import build_extra_filters, get_form_data  # noqa: F401
...
class TestUtils(ZobiTestCase):
    ...
    def test_get_or_create_db(self):
        get_or_create_db("test_db", "sqlite:///zobi.db")
        database = db.session.query(Database).filter_by(database_name="test_db").one()
        assert database is not None
        assert database.sqlalchemy_uri == "sqlite:///zobi.db"
        assert (
    ...
    def test_get_or_create_db_invalid_uri(self):
        with self.assertRaises(DatabaseInvalidError):  # noqa: PT027
            get_or_create_db("test_db", "yoursql:zobi.db/()")

    def test_get_or_create_db_existing_invalid_uri(self):
        database = get_or_create_db("test_db", "sqlite:///zobi.db")
