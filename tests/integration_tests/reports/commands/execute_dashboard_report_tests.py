...
@patch(
    "zobi.commands.report.execute.DashboardScreenshot",
)
@patch.dict(
...
def test_report_for_dashboard_with_tabs(
    ...
    with create_dashboard_report(
        ...
        expected_url = get_url_path("Zobi.dashboard_permalink", key=permalink_key)
...
def test_report_with_header_data(
    ...
    with create_dashboard_report(
        ...
        assert url.endswith(f"/zobi/dashboard/p/{permalink_key}/")
        assert send_email_smtp_mock.call_count == 1
...
@patch("zobi.reports.notifications.email.send_email_smtp")
@patch("zobi.commands.report.execute.DashboardScreenshot")
@pytest.mark.usefixtures("login_as_admin")
def test_report_schedule_stale_data_error_preserves_cause(
    ...
    with create_dashboard_report(
        ...
        with patch("zobi.db.session.commit") as mock_commit:
