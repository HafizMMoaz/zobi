...
from zobi.commands.report.execute import (
    AsyncExecuteReportScheduleCommand,
    BaseReportState,
)
from zobi.commands.report.log_prune import AsyncPruneReportScheduleLogCommand
...
@patch("zobi.utils.screenshots.ChartScreenshot.get_screenshot")
def test_email_chart_report_schedule_with_cc_bcc(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            '%7D&force=false">Explore in Zobi</a>' in email_mock.call_args[0][2]
        )
...
def test_email_chart_report_schedule_force_screenshot(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            '%7D&force=true">Explore in Zobi</a>' in email_mock.call_args[0][2]
        )
        # Assert the email smtp address
        assert email_mock.call_args[0][0] == notification_targets[0]
        # Assert the email inline screenshot
...
@patch("zobi.utils.csv.urllib.request.urlopen")
@patch("zobi.utils.csv.urllib.request.OpenerDirector.open")
@patch("zobi.reports.notifications.email.send_email_smtp")
@patch("zobi.utils.csv.get_chart_csv_data")
def test_email_chart_report_schedule_with_csv(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        ...
        assert (
            ...
            'force=false">Explore in Zobi</a>' in email_mock.call_args[0][2]
        )
        # Assert the email smtp address
        assert email_mock.call_args[0][0] == notification_targets[0]
...
@patch("zobi.utils.csv.urllib.request.OpenerDirector.open")
@patch("zobi.reports.notifications.email.send_email_smtp")
@patch("zobi.utils.csv.get_chart_dataframe")
...
@patch("zobi.reports.notifications.email.send_email_smtp")
@patch("zobi.utils.screenshots.DashboardScreenshot.get_screenshot")
def test_email_dashboard_report_schedule(
    ...
    with freeze_time("2020-01-01T00:00:00Z"):
        with patch(
            "zobi.extensions.stats_logger_manager.instance.gauge"
        ) as statsd_mock:
            AsyncExecuteReportScheduleCommand(
...
@patch("zobi.reports.notifications.email.send_email_smtp")
@patch.dict(
    "zobi.extensions.feature_flag_manager._feature_flags", ALERT_REPORT_TABS=True
)
...
@patch("zobi.reports.notifications.email.send_email_smtp")
@patch.dict(
    "zobi.extensions.feature_flag_manager._feature_flags", ALERT_REPORT_TABS=False
)
...
@patch("zobi.commands.report.execute.get_channels_with_search")
@patch("zobi.reports.notifications.slack.should_use_v2_api", return_value=True)
@patch("zobi.reports.notifications.slackv2.get_slack_client")
@patch("zobi.utils.screenshots.ChartScreenshot.get_screenshot")
...
@patch("zobi.utils.slack.get_slack_client")
@patch("zobi.utils.screenshots.ChartScreenshot.get_screenshot")
...
@patch("zobi.reports.notifications.slack.should_use_v2_api", return_value=False)
@patch("zobi.reports.notifications.slack.get_slack_client")
@patch("zobi.utils.csv.urllib.request.urlopen")
@patch("zobi.utils.csv.urllib.request.OpenerDirector.open")
@patch("zobi.utils.csv.get_chart_csv_data")
...
|  1 | c21  | c22  | c23       |"""
        ...
        assert (
            f"<http://0.0.0.0:8080/explore/?form_data=%7B%22slice_id%22:+{create_report_slack_chart_with_text.chart.id}%7D&force=false|Explore in Zobi>"  # noqa: E501
            in slack_client_mock_class.return_value.chat_postMessage.call_args[1][
                "text"
            ]
        )
...
@patch("zobi.utils.slack.WebClient.files_upload")
@patch("zobi.utils.screenshots.ChartScreenshot.get_screenshot")
@patch("zobi.reports.notifications.slack.get_slack_client")
...
def test_email_dashboard_report_fails_uncaught_exception(
    ...
    assert (
        '<a href="http://0.0.0.0:8080/zobi/dashboard/'
...
@patch("zobi.utils.slack.WebClient")
@patch("zobi.utils.screenshots.ChartScreenshot.get_screenshot")
...
def test_soft_timeout_alert(email_mock, create_alert_email_chart):
    ...
    from zobi.commands.report.exceptions import AlertQueryTimeout
...
def test_fail_screenshot(screenshot_mock, email_mock, create_report_email_chart):
    ...
    from zobi.commands.report.exceptions import AlertQueryTimeout  # noqa: F401
...
@patch("zobi.daos.report.ReportScheduleDAO.bulk_delete_logs")
...
@patch("zobi.commands.report.execute.logger")
@patch("zobi.commands.report.execute.create_notification")
def test__send_with_client_errors(notification_mock, logger_mock):
    ...
    assert excinfo.errisinstance(ZobiException)
    logger_mock.warning.assert_called_with(
        "ZobiError(message='', error_type=<ZobiErrorType.REPORT_NOTIFICATION_ERROR: 'REPORT_NOTIFICATION_ERROR'>, level=<ErrorLevel.WARNING: 'warning'>, extra=None)"  # noqa: E501
    )
...
def test__send_with_multiple_errors(notification_mock, logger_mock):
    ...
    logger_mock.warning.assert_has_calls(
        [
            ...
            call(
                "ZobiError(message='', error_type=<ZobiErrorType.REPORT_NOTIFICATION_ERROR: 'REPORT_NOTIFICATION_ERROR'>, level=<ErrorLevel.ERROR: 'error'>, extra=None)"  # noqa: E501
            ),
        ]
    )
