...
from zobi.connectors.sqla.models import (  # noqa: F401
    SqlaTable,
    SqlMetric,
    TableColumn,
)
from zobi.constants import CacheRegion
from zobi.daos.exceptions import DatasourceNotFound, DatasourceTypeNotSupportedError
from zobi.exceptions import ZobiGenericDBErrorException
from zobi.models.core import Database
from zobi.utils import json
from zobi.utils.core import backend, get_example_default_schema  # noqa: F401
...
class TestDatasource(ZobiTestCase):
    def setUp(self):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("zobi.connectors.sqla.models.SqlaTable.external_metadata")
    def test_external_metadata_error_return_400(self, mock_get_datasource):
        self.login(ADMIN_USERNAME)
        tbl = self.get_table(name="birth_names")
        url = f"/datasource/external_metadata/table/{tbl.id}/"

        mock_get_datasource.side_effect = ZobiGenericDBErrorException("oops")
    ...
    def test_save_default_endpoint_validation_success(self):
        ...
        datasource_post["id"] = tbl_id
        datasource_post["owners"] = [1]
        datasource_post["default_endpoint"] = "http://localhost/zobi/1"
        data = dict(data=json.dumps(datasource_post))  # noqa: C408
