...
from .base_tests import DEFAULT_PASSWORD, ZobiTestCase


class TestDashboard(ZobiTestCase):
    @pytest.fixture
    ...
    def test_zobi_dashboard_url(self):
        url_for("Zobi.dashboard", dashboard_id_or_slug=1)
    ...
    def test_new_dashboard(self):
        ...
        group = re.match(
            r"\/zobi\/dashboard\/([0-9]*)\/\?edit=true",
    ...
    def test_public_user_dashboard_access(self):
        ...
        assert "/zobi/dashboard/births/" not in resp
        ...
        resp = self.get_resp("/api/v1/dashboard/")
        assert "/zobi/dashboard/births/" in resp
        ...
        resp = self.get_resp("/api/v1/dashboard/")
        assert "/zobi/dashboard/world_health/" not in resp
    ...
    def test_dashboard_with_created_by_can_be_accessed_by_public_users(self):
        ...
        res: Response = self.client.get("/zobi/dashboard/births/")
    ...
    def test_anonymous_user_redirects_to_login_with_next(self):
        self.logout()
        target_path = f"/zobi/dashboard/{pytest.hidden_dash_slug}/"
    ...
    def test_anonymous_user_redirects_to_login_for_missing_dashboard(self):
        self.logout()
        target_path = "/zobi/dashboard/nonexistent-dashboard/"
    ...
    def test_users_can_list_published_dashboard(self):
        ...
        resp = self.get_resp("/api/v1/dashboard/")
        assert f"/zobi/dashboard/{pytest.hidden_dash_slug}/" not in resp
        assert f"/zobi/dashboard/{pytest.published_dash_slug}/" in resp
    ...
    def test_users_can_view_own_dashboard(self):
        ...
        assert f"/zobi/dashboard/{my_dash_slug}/" in resp
        assert f"/zobi/dashboard/{not_my_dash_slug}/" not in resp
    ...
    def test_user_can_not_view_unpublished_dash(self):
        ...
        assert f"/zobi/dashboard/{slug}/" not in resp
