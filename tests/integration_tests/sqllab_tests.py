...
from zobi.exceptions import ZobiErrorException, ZobiInvalidCVASException
...
from zobi.sql.parse import CTASMethod
...
from zobi.utils.json import datetime_to_epoch  # noqa: F401
...
class TestSqlLab(ZobiTestCase):
    ...
    def test_sql_json_cta_dynamic_db(self, ctas_method: CTASMethod) -> None:
        ...
        with mock.patch(
            "zobi.sqllab.sqllab_execution_context.get_cta_schema_name",
    ...
    def test_ps_conversion_no_dict(self):
        ...
        data = [["a", 4, 4.0]]
        results = ZobiResultSet(data, cols, BaseEngineSpec)
    ...
    @mock.patch("zobi.sql_lab.db")
    @mock.patch("zobi.sql_lab.get_query")
    @mock.patch("zobi.sql_lab.execute_query")
    ...
    @mock.patch("zobi.sql_lab.results_backend", None)
    @mock.patch("zobi.sql_lab.get_query")
    @mock.patch("zobi.sql_lab.execute_query")
    def test_execute_sql_statements_no_results_backend(
        ...
        with pytest.raises(ZobiErrorException) as excinfo:
            execute_sql_statements(
                query_id=1,
                rendered_query=sql,
                return_results=True,
    ...
    def test_execute_sql_statements_ctas(
        ...
        with pytest.raises(ZobiInvalidCVASException) as excinfo:
            execute_sql_statements(
                query_id=1,
                rendered_query=sql,
                return_results=True,
