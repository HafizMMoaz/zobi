...
def setup_sample_data() -> Any:
    ...
    with app.app_context():
        ...
        from zobi.examples.css_templates import load_css_templates
...
def physical_dataset():
    from zobi.connectors.sqla.models import SqlaTable, SqlMetric, TableColumn
    from zobi.connectors.sqla.utils import get_identifier_quoter
...
only_postgresql = pytest.mark.skipif(
    "postgresql" not in os.environ.get("ZOBI__SQLALCHEMY_DATABASE_URI", ""),
    reason="Only run test case in Postgresql",
)

only_sqlite = pytest.mark.skipif(
    "sqlite" not in os.environ.get("ZOBI__SQLALCHEMY_DATABASE_URI", ""),
    reason="Only run test case in SQLite",
)

only_mysql = pytest.mark.skipif(
    "mysql" not in os.environ.get("ZOBI__SQLALCHEMY_DATABASE_URI", ""),
