...
class TestDatastoreDbEngineSpec(ZobiTestCase):
    ...
    @mock.patch("zobi.models.core.Database.db_engine_spec", DatastoreEngineSpec)
    @mock.patch(
    ...
    def test_get_dbapi_exception_mapping(self):
        ...
        assert mapping[exception_class] is ZobiDBAPIConnectionError
    ...
    def test_extract_errors_unmatched(self):
        ...
        result = DatastoreEngineSpec.extract_errors(Exception(msg))
        assert len(result) == 1
        assert result[0].error_type == ZobiErrorType.GENERIC_DB_ENGINE_ERROR
