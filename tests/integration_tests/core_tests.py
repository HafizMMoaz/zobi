...
from zobi.exceptions import OAuth2RedirectError, ZobiException
...
class TestCore(ZobiTestCase):
    ...
    def test_dashboard_endpoint(self):
        ...
        resp = self.client.get("/zobi/dashboard/-1/")
        assert resp.status_code == 404

    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    def test_slice_endpoint(self):
        self.login(ADMIN_USERNAME)
        resp = self.client.get("/zobi/slice/-1/")
        assert resp.status_code == 404
    ...
    def test_custom_password_store(self):
        database = zobi.utils.database.get_example_database()
    ...
    def test_warm_up_cache_error(self) -> None:
        ...
        with mock.patch.object(
            ...
            assert self.get_json_resp(f"/zobi/warm_up_cache?slice_id={slc.id}") == [
                {
                    "slice_id": slc.id,
    ...
    def test_templated_sql_json(self):
        if zobi.utils.database.get_example_database().backend == "presto":
    ...
    def test_fetch_datasource_metadata(self):
        self.login(ADMIN_USERNAME)
        url = "/zobi/fetch_datasource_metadata?datasourceKey=1__table"
    ...
    def test_dataframe_timezone(self):
        ...
        results = ZobiResultSet(list(data), [["data"]], BaseEngineSpec)
    ...
    def test_explore_json_data_invalid_cache_key(self):
        ...
        rv = self.client.get(f"/zobi/explore_json/data/{cache_key}")
        data = json.loads(rv.data.decode("utf-8"))
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @mock.patch("zobi.viz.BaseViz.get_df")
    def test_explore_json_propagates_oauth2_redirect_error(
        self, mock_get_df: mock.Mock
    ) -> None:
        """
        ZobiErrorException exceptions bubble up properly.
        """
        ...
        slc = self.get_slice("Life Expectancy VS Rural %")
        rv = self.client.post(
            f"/zobi/explore_json/{slc.datasource_type}/{slc.datasource_id}/",
    ...
    def test_explore_json_generic_exception_still_returns_viz_get_df_error(
        ...
        """
        Non-Zobi exceptions raised by ``get_df`` are reported as the
    ...
    def test_results_default_deserialization(self):
        ...
        results = ZobiResultSet(data, cursor_descr, db_engine_spec)
        query = {
            "database_id": 1,
        ...
        deserialized_payload = zobi.views.utils._deserialize_results_payload(
    ...
    def test_explore_database_id(self):
        database = zobi.utils.database.get_example_database()
        explore_database = zobi.utils.database.get_example_database()
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @mock.patch("zobi.models.core.DB_CONNECTION_MUTATOR")
    def test_explore_injected_exceptions(self, mock_db_connection_mutator):
        ...
        exception = ZobiException("Error message")
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    @mock.patch("zobi.commands.explore.form_data.create.CreateFormDataCommand.run")
    ...
    def test_has_table(self):
        if backend() in ("sqlite", "mysql"):
            return
        example_db = zobi.utils.database.get_example_database()
        assert example_db.has_table(Table("birth_names", "public")) is True

    @mock.patch("zobi.views.core.request")
    @mock.patch(
        "zobi.commands.dashboard.permalink.get.GetDashboardPermalinkCommand.run"
    )
    def test_dashboard_permalink(self, get_dashboard_permalink_mock, request_mock):
        ...
        self.login(ADMIN_USERNAME)
        resp = self.client.get("zobi/dashboard/p/123/")

        expected_url = "/zobi/dashboard/1/?permalink_key=123&standalone=3"

        assert resp.headers["Location"] == expected_url
        assert resp.status_code == 302


class TestLocalePatch(ZobiTestCase):
    MOCK_LANGUAGES = (
