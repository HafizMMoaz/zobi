...
from zobi.models.core import FavStar, FavStarClassName
from zobi.reports.models import ReportSchedule, ReportScheduleType
from zobi.models.slice import Slice
from zobi.tags.models import Tag, TaggedObject, TagType, ObjectType
from zobi.utils.core import backend, override_user
from zobi.utils.screenshots import ScreenshotCachePayload
...
class TestDashboardApi(ApiOwnersTestCaseMixin, InsertChartMixin, ZobiTestCase):
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("zobi.dashboards.schemas.security_manager.has_guest_access")
    @patch("zobi.dashboards.schemas.security_manager.is_guest_user")
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("zobi.utils.log.logger")
    @patch("zobi.daos.dashboard.DashboardDAO.get_datasets_for_dashboard")
        ...
        expected_results = [
            {
                ...
                "url": "/zobi/dashboard/create_slug1/",
            },
            {
                "dashboard_title": "create_title0",
                "url": "/zobi/dashboard/create_slug0/",
            },
        ]
    ...
    def test_delete_dashboard_not_owned(self):
        ...
        user_alpha1 = self.create_user(
            "alpha1", "password", "Alpha", email="alpha1@zobi.org"
        )
        user_alpha2 = self.create_user(
            "alpha2", "password", "Alpha", email="alpha2@zobi.org"
        )
        existing_slice = (
    ...
    @with_feature_flags(THUMBNAILS=True, ENABLE_DASHBOARD_SCREENSHOT_ENDPOINTS=True)
    @pytest.mark.usefixtures("create_dashboard_with_tag")
    @patch("zobi.dashboards.api.cache_dashboard_screenshot")
    @patch("zobi.dashboards.api.DashboardScreenshot.get_from_cache_key")
    ...
    @pytest.mark.usefixtures("create_dashboard_with_tag")
    @patch("zobi.dashboards.api.cache_dashboard_screenshot")
    @patch("zobi.dashboards.api.build_pdf_from_screenshots")
    @patch("zobi.dashboards.api.DashboardScreenshot.get_from_cache_key")
...
class TestDashboardCustomTagsFiltering(ZobiTestCase):
    ...
    def test_dashboard_custom_tags_relationship_filters_correctly(self):
        ...
        With DASHBOARD_LIST_CUSTOM_TAGS_ONLY=True in zobi_test_config.py:
