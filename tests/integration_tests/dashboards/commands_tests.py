...
from zobi.commands.dashboard.importers import v0, v1
...
class TestExportDashboardsCommand(ZobiTestCase):
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("zobi.commands.dashboard.export.suffix")
...
class TestImportDashboardsCommand(ZobiTestCase):
...
class TestCopyDashboardCommand(ZobiTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    ...
    def test_copy_dashboard_command_no_access(self):
        ...
        with self.client.application.test_request_context():
            ...
            with override_user(security_manager.find_user("gamma")):
                with patch(
                    "zobi.commands.dashboard.copy.is_feature_enabled",
                    return_value=True,
                ):
                    command = CopyDashboardCommand(example_dashboard, copy_data)
...
class TestDeleteEmbeddedDashboardCommand(ZobiTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
...
class TestFavoriteDashboardCommand(ZobiTestCase):
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    def test_fave_unfave_dashboard_command(self):
        ...
        with self.client.application.test_request_context():
            ...
            with override_user(security_manager.find_user("admin")):
                with patch(
                    "zobi.daos.dashboard.DashboardDAO.get_by_id_or_slug",
    ...
    @pytest.mark.usefixtures("load_world_bank_dashboard_with_slices")
    @patch("zobi.models.dashboard.Dashboard.get")
