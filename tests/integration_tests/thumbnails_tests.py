...
class TestThumbnailsSeleniumLive(LiveServerTestCase):
    ...
    def test_get_async_dashboard_screenshot(self):
        ...
        """
        with patch("zobi.dashboards.api.DashboardRestApi.get") as mock_get:  # noqa: F841
...
class TestWebDriverScreenshotErrorDetector(ZobiTestCase):
    @patch("zobi.utils.webdriver.WebDriverWait")
    @patch("zobi.utils.webdriver.firefox")
    @patch("zobi.utils.webdriver.WebDriverSelenium.find_unexpected_errors")
    def test_not_call_find_unexpected_errors_if_feature_disabled(
        ...
        url = get_url_path("Zobi.dashboard", dashboard_id_or_slug=1)
...
class TestWebDriverSelenium(ZobiTestCase):
    @patch("zobi.utils.webdriver.WebDriverWait")
    @patch("zobi.utils.webdriver.firefox")
    @patch("zobi.utils.webdriver.sleep")
    def test_screenshot_selenium_headstart(
        ...
        user = security_manager.get_user_by_username(ADMIN_USERNAME)
        url = get_url_path("Zobi.slice", slice_id=1, standalone="true")
...
class TestThumbnails(ZobiTestCase):
    ...
    def test_get_async_dashboard_screenshot_as_fixed_user(self):
        ...
        with (
            ...
            patch(
                "zobi.thumbnails.digest._adjust_string_for_executor"
