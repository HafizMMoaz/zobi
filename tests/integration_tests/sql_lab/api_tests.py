...
from zobi.utils.database import (
...
class TestSqlLabApi(ZobiTestCase):
    ...
    def test_get_access_denied(self):
        ...
        unauth_user = self.create_user(
            ...
            "password",
            "Dummy Role",
            email="unauth_user1@zobi.org",  # noqa: F541
        )
    ...
    @mock.patch("zobi.commands.sql_lab.results.results_backend_use_msgpack", False)
    ...
    def test_execute_valid_request(self) -> None:
        from zobi import sql_lab as core
    ...
    def test_get_results_with_display_limit(self):
        from zobi.commands.sql_lab import results as command
        ...
        with mock.patch("zobi.commands.sql_lab.results.db") as mock_zobi_db:
            mock_zobi_db.session.query().filter_by().one_or_none.return_value = (
                query_mock
            )
            # get all results
            arguments = {"key": "key"}
    ...
    @mock.patch("zobi.models.sql_lab.Query.raise_for_access", lambda _: None)  # noqa: PT008
    @mock.patch("zobi.models.core.Database.get_df")
