...
from zobi.daos.datasource import DatasourceDAO  # noqa: F401
from zobi.models.dashboard import Dashboard
from zobi import appbuilder, db, security_manager, viz
...
class TestRolePermission(ZobiTestCase):
    ...
    def test_gamma_user_schema_access_to_dashboards(self):
        ...
        assert "/zobi/dashboard/world_health/" in data
        assert "/zobi/dashboard/births/" not in data
    ...
    def assert_can_gamma(self, perm_set):
        ...
        assert ("can_csv", "Zobi") in perm_set
        assert ("can_dashboard", "Zobi") in perm_set
        assert ("can_explore", "Zobi") in perm_set
        assert ("can_share_chart", "Zobi") in perm_set
        assert ("can_share_dashboard", "Zobi") in perm_set
        assert ("can_explore_json", "Zobi") in perm_set
        assert ("can_explore_json", "Zobi") in perm_set
    ...
    @unittest.skipUnless(
        ZobiTestCase.is_module_installed("pydruid"), "pydruid not installed"
    )
    ...
    def test_is_public_pvm(self):
        ...
        assert security_manager._is_public_pvm(
            security_manager.find_permission_view_menu("can_dashboard", "Zobi")
        )
        assert security_manager._is_public_pvm(
            security_manager.find_permission_view_menu("can_explore_json", "Zobi")
        )
    ...
    def test_public_role_permissions(self):
        ...
        assert ("can_dashboard", "Zobi") in public_perm_set
        assert ("can_slice", "Zobi") in public_perm_set
        assert ("can_explore_json", "Zobi") in public_perm_set
        assert ("can_dashboard_permalink", "Zobi") in public_perm_set
        ...
        assert ("can_share_dashboard", "Zobi") not in public_perm_set
        assert ("can_share_chart", "Zobi") not in public_perm_set

        # Should NOT have SQL Lab access
        assert ("can_sqllab", "Zobi") not in public_perm_set
    ...
    def test_gamma_permissions(self):
        ...
        assert ("can_csv", "Zobi") in gamma_perm_set
        assert ("can_dashboard", "Zobi") in gamma_perm_set
        assert ("can_explore", "Zobi") in gamma_perm_set
        assert ("can_share_chart", "Zobi") in gamma_perm_set
        assert ("can_share_dashboard", "Zobi") in gamma_perm_set
        assert ("can_explore_json", "Zobi") in gamma_perm_set
    ...
    def test_views_are_secured(self):
        ...
        views_allowlist = [
            ...
            ["SecurityApi", "refresh"],
            ["ZobiIndexView", "index"],
            ["ZobiIndexView", "patch_flask_locale"],
            ["DatabaseRestApi", "oauth2"],
            ["ZobiAuthView", "login"],
            ["ZobiAuthView", "logout"],
            ["ZobiRegisterUserView", "register"],
            ["ZobiRegisterUserView", "activation"],
...
class TestSecurityManager(ZobiTestCase):
    """
    Testing the Security Manager.
    """

    @patch("zobi.security.ZobiSecurityManager.raise_for_access")
    def test_can_access_datasource(self, mock_raise_for_access):
        ...
        mock_raise_for_access.side_effect = ZobiSecurityException(
            ZobiError(
                "dummy",
    ...
    def test_can_access_table(self, mock_raise_for_access):
        ...
        mock_raise_for_access.side_effect = ZobiSecurityException(
            ZobiError(
                "dummy", ZobiErrorType.TABLE_SECURITY_ACCESS_ERROR, ErrorLevel.ERROR
            )
        )

        assert not security_manager.can_access_table(database, table)

    @patch("zobi.security.ZobiSecurityManager.is_owner")
    @patch("zobi.security.ZobiSecurityManager.can_access")
    @patch("zobi.security.ZobiSecurityManager.can_access_schema")
    def test_raise_for_access_datasource(
        ...
        with self.assertRaises(ZobiSecurityException):  # noqa: PT027
            security_manager.raise_for_access(datasource=datasource)
...
class TestDatasources(ZobiTestCase):
    @patch("zobi.security.ZobiSecurityManager.can_access_database")
    @patch("zobi.security.ZobiSecurityManager.session")
...
class TestGuestTokens(ZobiTestCase):
    ...
    @patch("zobi.security.ZobiSecurityManager._get_current_epoch_time")
