...
from zobi import db as metadata_db
from zobi.db_engine_specs.postgres import PostgresEngineSpec  # noqa: F401
...
class TestDatabaseModel(ZobiTestCase):
    @unittest.skipUnless(
        ZobiTestCase.is_module_installed("requests"), "requests not installed"
    )
    @unittest.skipUnless(
        ZobiTestCase.is_module_installed("pyhive"), "pyhive not installed"
    )
    ...
    @unittest.skipUnless(
        ZobiTestCase.is_module_installed("mysqlclient"), "mysqlclient not installed"
    )
    def test_database_schema_mysql(self):
        sqlalchemy_uri = "mysql://root@localhost/zobi"
        model = Database(database_name="test_database", sqlalchemy_uri=sqlalchemy_uri)
    ...
    @mock.patch("zobi.models.core.create_engine")
    @unittest.skipUnless(
        ZobiTestCase.is_module_installed("pyhive"), "pyhive not installed"
    )
    ...
    def test_multi_statement(self):
        ...
        if main_db.backend == "mysql":
            df = main_db.get_df("USE zobi; SELECT 1", None, None)
            assert df.iat[0, 0] == 1

            df = main_db.get_df("USE zobi; SELECT ';';", None, None)
            assert df.iat[0, 0] == ";"
    ...
    def test_get_sqla_engine(self, mocked_create_engine):
        ...
        model.db_engine_spec.get_dbapi_exception_mapping = mock.Mock(
            return_value={Exception: ZobiException}
        )
        mocked_create_engine.side_effect = Exception()
        with self.assertRaises(ZobiException):  # noqa: PT027
            model._get_sqla_engine()


class TestSqlaTableModel(ZobiTestCase):
