...
from zobi.commands.chart.importers.v1 import ImportChartsCommand
...
class TestImportChartsCommand(ZobiTestCase):
    @patch("zobi.utils.core.g")
    @patch("zobi.security.manager.g")
...
class TestChartsCreateCommand(ZobiTestCase):
    @patch("zobi.utils.core.g")
    @patch("zobi.commands.chart.create.g")
    @patch("zobi.security.manager.g")
    @pytest.mark.usefixtures("load_energy_table_with_slice")
...
class TestChartsUpdateCommand(ZobiTestCase):
    @patch("zobi.commands.chart.update.g")
    @patch("zobi.utils.core.g")
    @patch("zobi.security.manager.g")
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    ...
    def test_update_chart_dashboard_security_new_unauthorized_relationship(
        ...
        from zobi.commands.chart.exceptions import ChartInvalidError
        from zobi.models.dashboard import Dashboard
...
class TestChartWarmUpCacheCommand(ZobiTestCase):
...
class TestFavoriteChartCommand(ZobiTestCase):
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    ...
    @pytest.mark.usefixtures("load_energy_table_with_slice")
    @patch("zobi.daos.base.BaseDAO.find_by_id")
