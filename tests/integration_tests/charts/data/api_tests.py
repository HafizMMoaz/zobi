...
class BaseTestChartDataApi(ZobiTestCase):
...
class TestPostChartDataApi(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("zobi.utils.decorators.g")
    ...
    @with_feature_flags(GLOBAL_ASYNC_QUERIES=True)
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("zobi.extensions.event_logger.log")
    ...
    @with_feature_flags(GLOBAL_ASYNC_QUERIES=True)
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("zobi.charts.data.api.ChartDataCommand.run")
...
class TestGetChartDataApi(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @with_feature_flags(GLOBAL_ASYNC_QUERIES=True)
    @mock.patch("zobi.charts.data.api.QueryContextCacheLoader")
...
class TestGetChartDataWithDashboardFilter(BaseTestChartDataApi):
    ...
    @pytest.mark.usefixtures("load_birth_names_dashboard_with_slices")
    @mock.patch("zobi.charts.data.api.get_dashboard_filter_context")
    ...
    def test_get_data_dashboard_access_denied_returns_403(self, mock_get_filter_ctx):
        ...
        chart = self._setup_chart_with_query_context()
        mock_get_filter_ctx.side_effect = ZobiSecurityException(
            ZobiError(
