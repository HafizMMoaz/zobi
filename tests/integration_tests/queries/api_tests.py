...
class TestQueryApi(ZobiTestCase):
    def insert_query(
        self,
        database_id: int,
        user_id: int,
    ...
    def test_get_query_no_data_access(self):
        ...
        gamma1 = self.create_user(
            "gamma_1", "password", "Gamma", email="gamma1@zobi.org"
        )
        gamma2 = self.create_user(
            "gamma_2", "password", "Gamma", email="gamma2@zobi.org"
        )
    ...
    # @mock.patch("zobi.sql_lab.cancel_query")
