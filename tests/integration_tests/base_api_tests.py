...
from zobi.views.base_api import BaseZobiModelRestApi, requires_json  # noqa: F401
...
class Model1Api(BaseZobiModelRestApi):
...
class TestOpenApiSpec(ZobiTestCase):
    def setUp(self) -> None:
...
class TestBaseModelRestApi(ZobiTestCase):
...
class ApiOwnersTestCaseMixin:
    ...
    @patch(
        "zobi.security.ZobiSecurityManager.get_exclude_users_from_lists",
        return_value=["gamma"],
    )
