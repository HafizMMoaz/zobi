...
import zobi.viz as viz
from flask import current_app
from zobi.exceptions import QueryObjectValidationError, SpatialException
...
class TestBaseViz(ZobiTestCase):
...
class TestPairedTTest(ZobiTestCase):
...
class TestPartitionViz(ZobiTestCase):
    @patch("zobi.viz.BaseViz.query_obj")
...
class TestRoseVis(ZobiTestCase):
...
class TestTimeSeriesTableViz(ZobiTestCase):
...
class TestBaseDeckGLViz(ZobiTestCase):
    ...
    def test_setup_metric_label(self):
        ...
        with patch("zobi.utils.core.get_metric_name") as mock_get_metric_name:
    ...
    def test_deckscatter_get_data_uses_setup_metric_label(self):
        ...
        with patch.object(test_viz, "_setup_metric_label") as mock_setup:
            with patch("zobi.viz.BaseDeckGLViz.get_data") as mock_super_get_data:
    ...
    def test_tooltip_integration_end_to_end(self):
        ...
        with patch("zobi.viz.BaseDeckGLViz.query_obj") as mock_super_query:
...
class TestDeckGLMultiLayer(ZobiTestCase):
    ...
    def test_setup_metric_label(self):
        ...
        with patch("zobi.viz.utils.get_metric_name") as mock_get_name:
...
class TestTimeSeriesViz(ZobiTestCase):
