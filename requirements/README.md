...
```
:::warning
The pinned dependencies are based on the `current` version of python supported in Zobi.
...
Note that `development.txt` is a zobi of what's in `base.txt`, and all version numbers for shared library should fully match at all times. `translations.txt` is meant as a supplemental file to be used in conjunction with the other requirements files, and is not meant to be used standalone.
