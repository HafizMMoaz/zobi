...
const { zobiPort = 8088, zobi: zobiUrl = null } = parsedEnvArg();
const backend = (zobiUrl || `http://localhost:${zobiPort}`).replace(
  '//+$/',
  '',
...
function toDevHTML(originalHtml) {
  ...
  if (manifest) {
    ...
    // in zobi/templates/zobi/partials/asset_bundle.html
    html = html.replace(
