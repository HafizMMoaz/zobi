...
jest.mock('@zobi.dev/core/components/InfoTooltip', () => ({
  InfoTooltip: () => <div data-test="mock-tooltip" />,
}));

jest.mock(
  '@zobi.dev/chart-controls/components/ColumnTypeLabel/ColumnTypeLabel',
  () => ({
    ColumnTypeLabel: () => <div data-test="mock-column-type-label" />,
  }),
);
jest.mock(
  '@zobi.dev/core/components/Tooltip',
  () =>
    ({ children }: { children: React.ReactNode }) => (
      <div data-test="mock-tooltip">{children}</div>
    ),
);
jest.mock('@zobi.dev/chart-controls/components/SQLPopover', () => ({
...
test('sets target="_blank" when openInNewWindow is true', () => {
  const { getByRole } = setup({
    url: 'https://github.com/HafizMMoaz/zobi',
