...
import { zobiTheme, ZobiTheme } from '@zobi/core-legacy/theme';
...
function SuperChart({
  ...
  // so we fall back to the default zobiTheme to avoid passing an invalid theme downstream.
  const themeFromContext = useTheme() as Partial<ZobiTheme>;
  const theme =
    themeProp ??
    (Object.keys(themeFromContext).length > 0
      ? (themeFromContext as ZobiTheme)
      : zobiTheme);
