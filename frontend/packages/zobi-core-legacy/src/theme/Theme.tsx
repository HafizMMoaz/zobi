...
export class Theme {
  ...
  private constructor({ config }: { config?: AnyThemeConfig }) {
    this.ZobiThemeProvider = this.ZobiThemeProvider.bind(this);
  ...
  setConfig(config: AnyThemeConfig): void {
    ...
    // Extract Zobi-specific properties from top-level config.
    // These are custom properties that aren't part of Ant Design's token system
    // but need to be passed through to the ZobiTheme for ECharts customization.
    ...
    this.theme = {
      ...
      // Include Zobi-specific properties from top-level config
  ...
  private updateProviders(
    ...
    // Overridden at runtime by ZobiThemeProvider using setThemeState
  }

  ZobiThemeProvider({ children }: { children: React.ReactNode }) {
    ...
    const [themeState, setThemeState] = React.useState({
      ...
      emotionCache: createCache({ key: 'zobi' }),
    });
