...
import { JsonValue, ZobiClientClass } from '@zobi.dev/core';
import { makeApi, ZobiApiError } from '../../../../src/query';
...
describe('makeApi()', () => {
  ...
  test('should allow custom path', async () => {
    ...
    const client = new ZobiClientClass({ appRoot: '/foo' });
  ...
  test('should handle errors', async () => {
    ...
    try {
      ...
      expect((error as ZobiApiError).message).toContain('Invalid payload');
    }
  });
  ...
  test('should handle error on 200 response', async () => {
    ...
    try {
      ...
      expect((error as ZobiApiError).message).toContain('not ok');
    }
  });
