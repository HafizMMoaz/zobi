...
import { CallApi, JsonObject } from '@zobi.dev/core';
...
describe('callApi()', () => {
  ...
  describe('caching', () => {
    ...
    test('caches requests with ETags', async () => {
      ...
      expect(calls).toHaveLength(1);
      const zobiCache = await caches.open(constants.CACHE_KEY);
      const cachedResponse = await zobiCache.match(mockCacheUrl);
