...
import { ZobiClientClass, ClientConfig, CallApi } from '@zobi.dev/core';
import { LOGIN_GLOB } from './fixtures/constants';

beforeAll(() => fetchMock.mockGlobal());
afterAll(() => fetchMock.hardReset());

describe('ZobiClientClass', () => {
  beforeEach(() => {
    fetchMock.clearHistory().removeRoutes();
    fetchMock.get(LOGIN_GLOB, { result: '' }, { name: LOGIN_GLOB });
  });

  describe('new ZobiClientClass()', () => {
    test('fallback protocol to https when setting only host', () => {
      const client = new ZobiClientClass({ host: 'TEST-HOST' });
  ...
  describe('.getUrl()', () => {
    let client = new ZobiClientClass();

    beforeEach(() => {
      client = new ZobiClientClass({
        protocol: 'https:',
        host: 'CONFIG_HOST',
      });
    });
    ...
    test('does not throw if url, endpoint, and host are all empty', () => {
      client = new ZobiClientClass({ protocol: 'https:', host: '' });
  ...
  describe('.init()', () => {
    ...
    test('calls api/v1/security/csrf_token/ when init() is called if no CSRF token is passed', async () => {
      // expect.assertions(1);
      await new ZobiClientClass().init();
    ...
    test('does NOT call api/v1/security/csrf_token/ when init() is called if a CSRF token is passed', async () => {
      expect.assertions(1);
      await new ZobiClientClass({ csrfToken: 'abc' }).init();
    ...
    test('getCSRFToken() returns existing csrfToken without fetching when already set', async () => {
      const client = new ZobiClientClass({ csrfToken: 'existing_token' });
    ...
    test('calls api/v1/security/csrf_token/ when init(force=true) is called even if a CSRF token is passed', async () => {
      ...
      const client = new ZobiClientClass({ csrfToken: initialToken });
  ...
  describe('.isAuthenticated()', () => {
    ...
    test('returns true if a token is passed at configuration', () => {
      expect.assertions(2);
      const clientWithoutToken = new ZobiClientClass({
        csrfToken: undefined,
      });
      const clientWithToken = new ZobiClientClass({ csrfToken: 'token' });
  ...
  describe('requests', () => {
    ...
    test('checks for authentication before every get and post request', async () => {
      ...
      const authSpy = jest.spyOn(ZobiClientClass.prototype, 'ensureAuth');
      const client = new ZobiClientClass({ protocol, host });
    ...
    test('sets protocol, host, headers, mode, and credentials from config', async () => {
      ...
      const client = new ZobiClientClass(clientConfig);
      await client.init();
      await client.get({ url: mockGetUrl });
  ...
  describe('when unauthorized', () => {
    ...
    beforeEach(() => {
      ...
      authSpy = jest
        .spyOn(ZobiClientClass.prototype, 'ensureAuth')
        .mockImplementation();
  ...
  describe('.postForm()', () => {
    ...
    beforeEach(async () => {
      ...
      client = new ZobiClientClass({ protocol, host });
      authSpy = jest.spyOn(ZobiClientClass.prototype, 'ensureAuth');
      await client.init();
      appendChild = jest.fn();
      removeChild = jest.fn();
      submit = jest.fn();
    ...
    test.each(['', '/prefix'])(
      ...
      async appRoot => {
        if (appRoot !== '') {
          client = new ZobiClientClass({ protocol, host, appRoot });
          authSpy = jest.spyOn(ZobiClientClass.prototype, 'ensureAuth');
          await client.init();
        }
    ...
    test('makes postForm request with guest token', async () => {
      client = new ZobiClientClass({ protocol, host, guestToken });
      await client.init();
