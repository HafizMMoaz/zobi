...
export default async function callApi({
  ...
  if (
    ...
    let zobiCache: Cache | null = null;
    try {
      zobiCache = await caches.open(CACHE_KEY);
      const cachedResponse = await zobiCache.match(url);
      if (cachedResponse) {
      ...
      // If zobi is in an iframe and third-party cookies are disabled, caches.open throws
    }

    const response = await fetchWithRetry(url, request);

    if (zobiCache && response.status === HTTP_STATUS_NOT_MODIFIED) {
      const cachedFullResponse = await zobiCache.match(url);
    ...
    if (
      ...
      zobiCache.delete(url);
      zobiCache.put(url, response.clone());
    }
