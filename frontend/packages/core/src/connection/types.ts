...
export interface ZobiClientInterface extends Pick<
  ZobiClientClass,
  | 'delete'
  | 'get'
  | 'post'
  ...
  | 'getUrl'
> {
  configure: (config?: ClientConfig) => ZobiClientInterface;
  reset: () => void;
  getCSRFToken: () => CsrfPromise;
}

export type ZobiClientResponse = Response | JsonResponse | TextResponse;
