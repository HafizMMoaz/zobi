...
test('disables overwrite option for externally managed slice', () => {
  ...
  expect(
    getByText(
      "This chart is managed externally and can't be overwritten in Zobi.",
    ),
  ).toBeInTheDocument();
});
...
test('dispatches removeChartState when saving and going to dashboard', async () => {
  ...
  const dashboardUrl = '/zobi/dashboard/test-dashboard/';
...
test('addChartToDashboardTab successfully adds chart to existing row with space', async () => {
  ...
  ZobiClient.get = jest.fn().mockResolvedValueOnce({
    json: { result: mockDashboard },
  });

  ZobiClient.put = jest.fn().mockResolvedValueOnce({
    json: { result: mockDashboard },
  });
  ...
  try {
    ...
    const putCall = ZobiClient.put.mock.calls[0][0];
    ...
    ZobiClient.get = originalGet;
    ZobiClient.put = originalPut;
...
test('addChartToDashboardTab creates new row when no existing row has space', async () => {
  ...
  ZobiClient.put = jest.fn().mockImplementationOnce((request: any) => {
  ...
  try {
    ...
    expect(ZobiClient.put).toHaveBeenCalled();
