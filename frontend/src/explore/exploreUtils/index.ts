...
export const exportChart = async ({
  ...
  if (onStartStreamingExport) {
    ...
    // does not go through ZobiClient (which would add it automatically).
    onStartStreamingExport({
    ...
    // ZobiClient.postForm calls getUrl({ endpoint }) internally, which prepends
    // appRoot — so the URL must NOT be pre-prefixed here.
    ZobiClient.postForm(url as string, {
