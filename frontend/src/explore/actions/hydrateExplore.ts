...
export const hydrateExplore =
  ...
  (dispatch: Dispatch, getState: () => ExplorePageState) => {
    ...
    const verifyColorScheme = (type: ColorSchemeType) => {
      ...
      const defaultScheme =
        type === 'CATEGORICAL' ? 'zobiColors' : 'zobi_seq_1';
    ...
    const exploreState = {
      ...
      can_download: granularExport
        ? findPermission('can_export_data', 'Zobi', user?.roles)
        : findPermission('can_csv', 'Zobi', user?.roles),
      can_export_image: granularExport
        ? findPermission('can_export_image', 'Zobi', user?.roles)
        : findPermission('can_csv', 'Zobi', user?.roles),
      can_copy_clipboard: granularExport
        ? findPermission('can_copy_clipboard', 'Zobi', user?.roles)
        : findPermission('can_csv', 'Zobi', user?.roles),
