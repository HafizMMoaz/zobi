...
test('displays loading state initially', () => {
  mockZobiClientGet.mockImplementation(() => new Promise(() => {}));
...
test('displays disabled select when no deck.gl multi layer charts are found', async () => {
  mockZobiClientGet.mockResolvedValue({ json: { result: [] } });
...
test('renders layer selection control with layers from API', async () => {
  mockZobiClientGet.mockResolvedValue(mockApiResponse);
...
test('collects unique layer IDs from multiple deck_multi charts', async () => {
  ...
  await waitFor(() => {
    expect(mockZobiClientGet).toHaveBeenCalled();
  });

  const callArgs = mockZobiClientGet.mock.calls[0][0];
