...
beforeEach(() => {
  ...
  // Mock ZobiClient.get
  (ZobiClient.get as jest.Mock).mockResolvedValue(mockResponse);
...
test('handles API errors and calls done callback', async () => {
  ...
  (ZobiClient.get as jest.Mock).mockRejectedValue(apiError);
...
test('handles network errors and logs them', async () => {
  ...
  (ZobiClient.get as jest.Mock).mockRejectedValue(networkError);
...
test('handles 404 errors when resource not found', async () => {
  ...
  (ZobiClient.get as jest.Mock).mockRejectedValue(notFoundError);
...
test.each(doublePrefixTestCases)(
  ...
  async ({ appRoot, resource, ids }) => {
    ...
    // The endpoint passed to ZobiClient.get should NOT have the appRoot prefix
    // because ZobiClient.getUrl() adds it when building the full URL.
