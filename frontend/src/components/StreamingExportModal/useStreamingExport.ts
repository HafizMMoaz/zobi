...
interface StreamingExportParams {
   ...
   * subdirectory deployments (e.g., /zobi/api/v1/...).
   *
...
const ensureUrlPrefix = (url: string): string => {
  ...
  // Also handle query strings and hashes (e.g., /zobi?foo=1 or /zobi#hash)
  if (
    url === appRoot ||
...
const createFetchRequest = async (
  ...
  // Get CSRF token using ZobiClient
  const csrfToken = await ZobiClient.getCSRFToken();
  if (csrfToken) {
