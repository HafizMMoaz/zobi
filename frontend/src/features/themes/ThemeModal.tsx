...
const ThemeModal: FunctionComponent<ThemeModalProps> = ({
  ...
  // ZobiText URL configurations
  const themeEditorUrl =
    ZobiText?.THEME_MODAL?.THEME_EDITOR_URL ||
    'https://ant.design/theme-editor';
  const documentationUrl =
    ZobiText?.THEME_MODAL?.DOCUMENTATION_URL ||
    'https://zobi.dev/docs/configuration/theming/';
  ...
  const modalIcon = useMemo(() => {
    ...
    return (
      <Icon
        iconSize="l"
        css={css`
          margin: auto ${zobiTheme.sizeUnit * 2}px auto 0;
        `}
      />
    );
  }, [isEditMode, zobiTheme.sizeUnit]);
