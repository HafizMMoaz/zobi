...
const RightMenu = ({
  ...
  const canSql = findPermission('can_sqllab', 'Zobi', roles);
  ...
  const menuItems = useMemo(() => {
    ...
    const buildSettingsMenuItems = (): MenuItem[] => {
      ...
      if (!navbarRight.user_is_anonymous) {
        ...
        const showLogout =
          !isEmbedded() ||
          !isFeatureEnabled(FeatureFlag.DisableEmbeddedZobiLogout);
        if (showLogout) {
          userItems.push({
            key: 'logout',
            label: (
...
// Zobi still has multiple entry points, and not all of them have
