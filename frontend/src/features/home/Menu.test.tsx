...
const mockedProps = {
  ...
  data: {
    ...
    brand: {
      path: '/zobi/welcome/',
      icon: '/static/assets/images/logo-horiz.svg',
      alt: 'Zobi',
      width: '126',
      tooltip: '',
      text: '',
...
beforeEach(() => {
  ...
  useThemeMock.mockReturnValue(CoreTheme.zobiTheme);
});
...
test('should render the brand text if available', async () => {
  ...
  const brandText = await screen.findByText('Welcome to Zobi');
  expect(brandText).toBeInTheDocument();
});

test('should not render the brand text if not available', async () => {
  useSelectorMock.mockReturnValue({ roles: [] });
  const text = 'Welcome to Zobi';
  render(<Menu {...mockedProps} />, {
    useRouter: true,
    useQueryParams: true,
    useRedux: true,
...
test('brand logo href should not be prefixed with app root when brandLogoHref is an absolute URL', async () => {
  applicationRootMock.mockReturnValue('/zobi');
...
test('brand path should be prefixed with app root in subdirectory deployment', async () => {
  ...
  expect(brandLink).toHaveAttribute('href', '/zobi/welcome/');
});

test('brand link falls back to brand.path when theme brandLogoUrl is absent', async () => {
  // useThemeMock default returns zobiTheme with brandLogoUrl undefined (falsy)
  applicationRootMock.mockReturnValue('/zobi');
  useSelectorMock.mockReturnValue({ roles: user.roles });
  ...
  // ensureAppRoot must have been applied: /welcome/ → /zobi/welcome/
  expect(brandLink).toHaveAttribute('href', '/zobi/welcome/');
});
