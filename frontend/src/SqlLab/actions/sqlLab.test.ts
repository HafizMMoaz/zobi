...
describe('async actions', () => {
  ...
  describe('popSavedQuery', () => {
    const zobiClientGetSpy = jest.spyOn(ZobiClient, 'get');
    const store = mockStore({});
    ...
    beforeEach(() => {
      zobiClientGetSpy.mockClear();
      store.clearActions();
    });

    afterAll(() => {
      zobiClientGetSpy.mockRestore();
    });

    test('calls API endpint with correct params', async () => {
      zobiClientGetSpy.mockResolvedValue({
        json: { result: mockSavedQueryApiResponse },
      } as any);

      await makeRequest(123);

      expect(zobiClientGetSpy).toHaveBeenCalledWith({
    ...
    test('should dispatch addDangerToast on API error', async () => {
      zobiClientGetSpy.mockResolvedValue(new Error() as any);
