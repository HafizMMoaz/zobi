  ...
  return ZobiClient.post({
    ...
    .catch(() =>
      dispatch(
        addWarningToast(
          t(
            'Unable to migrate table schema state to backend. Zobi will retry ' +
  ...
  return ZobiClient.post({
    ...
    .catch(() =>
      dispatch(
        addWarningToast(
          t(
            'Unable to migrate query state to backend. Zobi will retry later. ' +
    ...
    return ZobiClient.post({
      ...
      .catch(() =>
        dispatch(
          addWarningToast(
            t(
              'Unable to migrate query editor state to backend. Zobi will retry ' +
                'later. Please contact your administrator if this problem persists.',
            ),
          ),
        ),
...
export function popPermalink(key: string): SqlLabThunkAction<Promise<unknown>> {
  return function (dispatch: AppDispatch) {
    return ZobiClient.get({ endpoint: `/api/v1/sqllab/permalink/${key}` })
      .then(({ json }) =>
        dispatch(
          addQueryEditor({
