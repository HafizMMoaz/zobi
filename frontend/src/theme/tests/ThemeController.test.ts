...
test('ThemeController fallbacks to Zobi default theme when BootstrapData themes are empty', () => {
...
test('ThemeController constructor recovers from corrupted stored theme', () => {
  ...
  mockLocalStorage.getItem.mockImplementation((key: string) => {
    if (key === 'zobi-dev-theme-override') {
  ...
  expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(
    'zobi-dev-theme-override',
  );
  expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(
    'zobi-crud-theme-id',
  );
  expect(mockLocalStorage.removeItem).toHaveBeenCalledWith(
    'zobi-applied-theme-id',
  );
...
test('setThemeConfig applies appropriate theme after configuration', () => {
  ...
  controller.setThemeConfig(themeConfig as ZobiThemeConfig);
...
test('font loading: injects font URLs as CSS @import when theme has fontUrls', () => {
  ...
  const style = document.querySelector('style[data-zobi-fonts]');
...
test('font loading: does not duplicate font URLs when same theme applied twice', () => {
  ...
  const styles = document.querySelectorAll('style[data-zobi-fonts]');
