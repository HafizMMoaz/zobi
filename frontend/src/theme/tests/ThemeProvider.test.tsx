...
// Mock theme objects
const mockTheme = {
  ZobiThemeProvider: ({ children }: { children: ReactNode }) => (
    <div data-test="zobi-theme-provider">{children}</div>
  ),
  theme: {
    token: {
      colorPrimary: '#1890ff',
...
const mockDarkTheme = {
  ZobiThemeProvider: ({ children }: { children: ReactNode }) => (
    <div data-test="zobi-dark-theme-provider">{children}</div>
  ),
  theme: {
    token: {
      colorPrimary: '#1890ff',
...
const createWrapper =
  ...
  ({ children }: { children: ReactNode }) => (
    <ZobiThemeProvider themeController={controller}>
      {children}
    </ZobiThemeProvider>
  );

// eslint-disable-next-line no-restricted-globals -- TODO: Migrate from describe blocks
describe('ZobiThemeProvider', () => {
  ...
  describe('Provider Initialization', () => {
    test('should render children within theme provider wrapper', () => {
      render(
        <ZobiThemeProvider themeController={mockThemeController}>
          <div data-test="test-child">Hello Zobi</div>
        </ZobiThemeProvider>,
      );

      expect(screen.getByTestId('zobi-theme-provider')).toBeInTheDocument();
