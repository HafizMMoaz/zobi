...
const STORAGE_KEYS = {
  THEME_MODE: 'zobi-theme-mode',
  CRUD_THEME_ID: 'zobi-crud-theme-id',
  DEV_THEME_OVERRIDE: 'zobi-dev-theme-override',
  APPLIED_THEME_ID: 'zobi-applied-theme-id',
} as const;
...
export class ThemeController {
  ...
  constructor({
    ...
    themeObject = zobiThemeObject,
    defaultTheme = (zobiThemeObject.theme as AnyThemeConfig) ?? {},
    onChange = undefined,
   ...
   */
  public setThemeConfig(config: ZobiThemeConfig): void {
  ...
  private async fetchCrudTheme(
    ...
    try {
      // Use ZobiClient for proper authentication handling
   ...
   *
   * Note: Uses raw fetch() instead of ZobiClient because ThemeController
   * initializes early in the app lifecycle, before ZobiClient is fully
