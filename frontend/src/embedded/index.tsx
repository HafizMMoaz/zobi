...
function log(...info: unknown[]) {
  if (debugMode) logging.debug(`[zobi]`, ...info);
}
...
//   throw new Error(
//     `[zobi] iframe parent ${parentOrigin} is not in the list of allowed origins`,
//   );
// }
 ...
 * 401 errors from the api and ZobiClient will call this function.
 */
...
/**
 * Configures ZobiClient with the correct settings for the embedded dashboard page.
 */
...
window.addEventListener('message', function embeddedPageInitializer(event) {
  ...
  if (event.data.handshake === 'port transfer' && port) {
    ...
    Switchboard.defineMethod(
      'setThemeConfig',
      (payload: { themeConfig: ZobiThemeConfig }) => {
