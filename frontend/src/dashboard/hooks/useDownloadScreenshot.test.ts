...
jest.mock('@zobi.dev/core', () => ({
  ...
  ZobiApiError: class ZobiApiError extends Error {
    status: number;
...
beforeEach(() => {
  ...
  (ZobiClient.get as jest.Mock).mockReturnValue(new Promise(() => {}));
});
...
test('downloadScreenshot calls API with force=true to ensure fresh screenshots', async () => {
  ...
  expect(ZobiClient.post).toHaveBeenCalledTimes(1);
  const callArgs = (ZobiClient.post as jest.Mock).mock.calls[0][0];
...
test('does not issue overlapping GETs while a previous GET is in-flight', async () => {
  ...
  expect(ZobiClient.get).toHaveBeenCalledTimes(1);
