...
describe('dashboardState actions', () => {
  ...
  beforeEach(() => {
    ...
    getStub = jest.spyOn(ZobiClient, 'get').mockResolvedValue({
      json: {
        result: {
          ...mockDashboardData,
          css: updatedCss,
    ...
    putStub = jest.spyOn(ZobiClient, 'put').mockResolvedValue({
      json: {
  ...
  describe('saveDashboardRequest', () => {
    ...
    test('should navigate to the new dashboard after Save As', async () => {
      ...
      postStub.mockRestore();
      postStub = jest.spyOn(ZobiClient, 'post').mockResolvedValue({
        json: {
          result: {
            ...mockDashboardData,
            id: newDashboardId,
  ...
  test('fetchCharts returns a Promise that resolves after all refreshes', async () => {
    ...
    const { getState } = setup({
      dashboardInfo: {
        metadata: {},
        common: { conf: { ZOBI_WEBSERVER_TIMEOUT: 60 } },
      },
    });
