...
export function saveDashboardRequest(
  ...
  return (dispatch: AppDispatch, getState: GetState) => {
    ...
    const onCopySuccess = (response: JsonObject): JsonObject => {
      ...
      navigateTo(
        `/zobi/dashboard/${(response.json as JsonObject).result?.id}/`,
      );
    ...
    const onUpdateSuccess = (response: JsonObject): JsonObject => {
      ...
      navigateWithState(`/zobi/dashboard/${slug || id}/`, {
...
// Even though SET_ACTIVE_TABS is not being called from Zobi's codebase,
  ...
  async (dispatch: AppDispatch): Promise<void> => {
    try {
      ...
      if (hasInvalidColorScheme) {
        ...
        const fallbackScheme = defaultScheme?.toString() || 'zobiColors';
        hasChanged = true;
