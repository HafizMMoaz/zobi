...
const defaultState = {
  ...
  dashboardInfo: {
    ...
    zobi_can_explore: false,
    zobi_can_share: false,
    zobi_can_csv: false,
    common: { conf: { ZOBI_WEBSERVER_TIMEOUT: 0, SQL_MAX_ROW: 666 } },
  },
  dashboardLayout: {
    present: {},
    past: [],
...
test('should call exportChart when exportCSV is clicked', async () => {
  ...
  const { findByText, getByRole } = setup(
    {},
    {
      dashboardInfo: { ...defaultState.dashboardInfo, zobi_can_csv: true },
    },
  );
  fireEvent.click(getByRole('button', { name: 'More Options' }));
