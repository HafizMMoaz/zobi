...
jest.mock('src/dashboard/components/SliceHeaderControls', () => ({
  ...
  default: (props: any) => (
    <div
      ...
      data-zobi-can-explore={props.zobiCanExplore}
      data-zobi-can-csv={props.zobiCanCSV}
...
test('Should render - default props', () => {
  ...
  delete props.zobiCanExplore;
  delete props.zobiCanCSV;
...
test('Should render click to edit prompt and run onExploreChart on click', async () => {
  ...
  const history = createMemoryHistory({
    initialEntries: ['/zobi/dashboard/1/'],
  });
  render(
...
test('Should not render click to edit prompt and run onExploreChart on click if zobiCanExplore=false', () => {
  const props = createProps({ zobiCanExplore: false });
  const history = createMemoryHistory({
    initialEntries: ['/zobi/dashboard/1/'],
  });
  render(
  ...
  userEvent.click(screen.getByText('Vaccine Candidates per Phase'));
  expect(history.location.pathname).toMatch('/zobi/dashboard');
});
