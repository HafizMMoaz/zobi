...
type DashboardInfo = {
  ...
  common?: {
    conf?: {
      ZOBI_DASHBOARD_PERIODICAL_REFRESH_LIMIT?: number;
    };
  };
};
...
const PropertiesModal = ({
  ...
  useEffect(() => {
    if (show) {
      ...
      ZobiClient.get({ endpoint: `/api/v1/theme/?q=${themeQuery}` })
        .then(({ json }) => {
    ...
    <StandardModal
      ...
      errorTooltip={
        dashboardInfo?.isManagedExternally
          ? t(
              "This dashboard is managed externally, and can't be edited in Zobi",
            )
          : errorTooltip
      }
      saveText={saveLabel}
