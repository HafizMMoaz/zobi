...
/**
 * Helper to set up error test scenarios with ZobiClient spy
...
const setupErrorTestScenario = ({
  ...
  // Spy on ZobiClient method and throw error for specific endpoint
  const originalMethod =
    method === 'get'
      ? ZobiClient.get.bind(ZobiClient)
      : ZobiClient.post.bind(ZobiClient);

  jest.spyOn(ZobiClient, method).mockImplementation(async request => {
    if (request.endpoint?.includes(endpoint)) {
      throw buildZobiClientError({
        status: errorStatus,
...
test('delete action gracefully handles 403 forbidden error', async () => {
  ...
  // Wait for ZobiClient.get to be called (deterministic anchor - API was attempted)
  await waitFor(() => {
    expect(ZobiClient.get).toHaveBeenCalledWith(
...
test('edit action shows error toast when dataset fetch fails', async () => {
  ...
  // Mock ZobiClient.get to fail for the specific dataset endpoint
  jest.spyOn(ZobiClient, 'get').mockImplementation(async request => {
