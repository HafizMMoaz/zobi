...
/**
 * Format a date string to ISO format expected by Zobi, preserving local timezone
 */
export function formatDateForZobi(dateStr: string): string {
  // AG Grid typically provides dates in format: "YYYY-MM-DD HH:MM:SS"
  // Zobi expects: "YYYY-MM-DDTHH:MM:SS" in local timezone (not UTC)
...
export function getStartOfDay(dateStr: string): string {
  ...
  return formatDateForZobi(date.toISOString());
}
...
// Converts date filters to TEMPORAL_RANGE format for Zobi backend
