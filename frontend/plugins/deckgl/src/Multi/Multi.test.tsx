...
import { zobiTheme, ThemeProvider } from '@zobi.dev/extension-api/theme';
import { Provider } from 'react-redux';
import { configureStore } from '@reduxjs/toolkit';
import { DatasourceType, ZobiClient } from '@zobi.dev/core';
...
const renderWithProviders = (component: React.ReactElement) =>
  render(
    <Provider store={mockStore}>
      <ThemeProvider theme={zobiTheme}>{component}</ThemeProvider>
    </Provider>,
  );
...
describe('DeckMulti Autozoom Functionality', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    (ZobiClient.get as jest.Mock).mockResolvedValue({
      json: {
        data: {
          features: [],
        },
...
describe('DeckMulti Component Rendering', () => {
  ...
  test('should include dashboardId in child slice requests when present', async () => {
    ...
    await waitFor(() => {
      expect(ZobiClient.get).toHaveBeenCalled();
    });

    // Check that all requests include the dashboardId
    const { calls } = (ZobiClient.get as jest.Mock).mock;
    calls.forEach(call => {
...
test('includes parent_slice_id in child slice requests when parent has slice_id', async () => {
  ...
  (ZobiClient.get as jest.Mock) = mockGet;
