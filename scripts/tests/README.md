...
You can use a different DB backend by defining `ZOBI__SQLALCHEMY_DATABASE_URI` env var.
...
From the zobi repo root directory:
...
- Example run a single test, without any init procedures. Init procedures include:
  resetting test database, db upgrade, zobi init, loading example data. If your tests
