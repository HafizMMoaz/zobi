...
const ZOBI_ROOT = dirname(__dirname);
...
async function runFullTypeCheck(packageRootDir, excludeDeclarationDirArg) {
  const packageRootDirAbsolute = join(ZOBI_ROOT, packageRootDir);
 ...
 *
 * For example: `frontend/foo/bar.ts` -> `foo/bar.ts`
 *
 * @param {string[]} args
