...
# -----------------------------------------------------------------------
# Smart docker-compose wrapper for running multiple ZOBI instances
#
# Features:
...
#   ./scripts/docker-compose-up.sh --build   # Rebuild images, then start
...
find_and_claim_port $BASE_ZOBI ZOBI_PORT
...
if docker compose ps --status running 2>/dev/null | grep -q "$PROJECT_NAME"; then
    ...
    ZOBI_PORT=$(get_running_port zobi 8088 $ZOBI_PORT)
    NODE_PORT=$(get_running_port zobi-node 9000 $NODE_PORT)
    WEBSOCKET_PORT=$(get_running_port zobi-websocket 8080 $WEBSOCKET_PORT)
...
case "${1:-}" in
    ...
    --env)
        ...
        echo "export ZOBI_PORT=$ZOBI_PORT"
    ...
    open)
        ...
        exit 0
        ;;
    down|stop|logs|ps|exec|restart|build)
