...
from zobi_core.extensions.types import (
    ExtensionConfig,
    Manifest,
    ManifestBackend,
    ManifestFrontend,
...
from zobi_extensions_cli.constants import MIN_NPM_VERSION
from zobi_extensions_cli.exceptions import ExtensionNameError
from zobi_extensions_cli.types import ExtensionNames
from zobi_extensions_cli.utils import (
...
@click.group(help="CLI for validating and bundling Zobi extensions.")
def app() -> None:
    pass
...
def validate() -> None:
    ...
    if backend_dir.exists():
        ...
        if not build_config.get("include"):
            click.secho(
                "❌ Missing [tool.zobi_extensions.build] section with 'include' patterns in pyproject.toml",
                err=True,
                fg="red",
            )
            sys.exit(1)
    ...
    if mismatches:
        ...
        click.secho(
            "Run `zobi-extensions update` to sync from extension.json.",
            fg="yellow",
        )
        sys.exit(1)
...
def bundle(ctx: click.Context, output: Path | None) -> None:
    """Package the extension into a .zobz file."""
    ctx.invoke(build)
    ...
    default_filename = f"{name}-{version}.zobz"
...
def init(
    ...
    license_ = license_opt or click.prompt("License", default="MIT")
    include_frontend = (
        frontend_opt
