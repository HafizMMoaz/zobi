...
import pytest
from zobi_extensions_cli.cli import app, validate_npm
...
def test_validate_command_success(cli_runner, isolated_filesystem):
    ...
    with patch("zobi_extensions_cli.cli.validate_npm") as mock_validate:
...
def test_validate_fails_on_version_mismatch(
    ...
    assert "zobi-extensions update" in result.output
