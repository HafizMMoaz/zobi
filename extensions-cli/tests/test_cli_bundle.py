...
@pytest.mark.cli
@patch("zobi_extensions_cli.cli.build")
def test_bundle_command_creates_zip_with_default_name(
    ...
    assert "✅ Bundle created: test-extension-1.0.0.zobz" in result.output

    # Verify zip file was created
    zip_path = isolated_filesystem / "test-extension-1.0.0.zobz"
...
def test_bundle_command_with_custom_output_filename(
    ...
    custom_name = "my_custom_bundle.zobz"
...
def test_bundle_command_with_output_directory(
    ...
    expected_path = output_dir / "test-extension-1.0.0.zobz"
...
def test_bundle_command_handles_zip_creation_error(
    ...
    invalid_path = isolated_filesystem / "nonexistent" / "bundle.zobz"
...
def test_bundle_includes_all_files_recursively(
    ...
    zip_path = isolated_filesystem / "complex-extension-2.1.0.zobz"
    assert_file_exists(zip_path)
...
def test_bundle_command_short_option(
    ...
    result = cli_runner.invoke(app, ["bundle", "-o", "short_option.zobz"])

    assert result.exit_code == 0
    assert "✅ Bundle created: short_option.zobz" in result.output
    assert_file_exists(isolated_filesystem / "short_option.zobz")
...
def test_bundle_command_output_options(
    ...
    filename = f"test_{output_option.replace('-', '')}.zobz"
